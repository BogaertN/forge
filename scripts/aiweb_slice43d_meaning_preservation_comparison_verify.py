#!/usr/bin/env python3
"""Visible independent verifier for Slice 43D comparison findings."""
...
EXPECTED_PARENT_HEAD = "6f2cbafc18ef9eff259bca038d189f1bbe7fc4c6"
EXPECTED_PARENT_TREE = "c378cb1cd0be715160a9f919ea01815799ee4f56"
...
PROTECTED_PREDECESSOR_MANIFEST = Path(
    "scripts/AIWEB_SLICE43D_PROTECTED_PREDECESSOR_SHA256SUMS.txt"
)
BEHAVIOR_TEST = Path(
...
INHERITED_VERIFIER = Path(
    "scripts/"
    "aiweb_slice43c_authorized_meaning_proposed_expression_admission_verify.py"
)
EXPECTED_PREDECESSOR_COUNT = 1696
...
def verify_source_contract(repository: Path, ledger: Ledger) -> None:
    ...
    ledger.check(
        ...
        "comparison package has no nested runtime directory",
    )
    ...
    ledger.check(
        ...
        "all required comparison definitions",
    )
...
def run_inherited(
    ...
    try:
        with tempfile.TemporaryDirectory(
            prefix="aiweb_slice43d_slice43c_"
        ) as temporary:
            checkout = Path(temporary) / "slice43c"
...
def main() -> int:
    ...
    print("=== AI.WEB SLICE 43D INDEPENDENT VERIFIER ===")
    print(f"repository={repository}")
    print(f"mode={args.mode}")

    ledger.check(repository.is_dir(), "repository exists")
    if not repository.is_dir():
        print("AI.WEB SLICE 43D VERIFIER: FAIL")
        return 1
    ...
    executable_paths = {
        str(BEHAVIOR_TEST),
        str(Path("scripts") / "aiweb_slice43d_meaning_preservation_comparison_verify.py"),
    }
    ...
    for protected in (
        "aiweb_language_core_bootstrap/rmc_echo_runtime/"
        "authorized_source_admission/schema.py",
        "aiweb_language_core_bootstrap/rmc_echo_runtime/"
    ...
    ledger.check(
        ...
        "Slice 43D package additive at parent",
    )
    ...
    with tempfile.TemporaryDirectory(
        ...
        ledger.check(
            "AI.WEB SLICE 43D BEHAVIOR TEST: PASS" in behavior_output,
            "current behavior verdict",
        )
        print("\n=== INHERITED SLICE 43C VERIFIER ===")
        inherited_rc, inherited_output = run_inherited(repository, env)
        ledger.check(inherited_rc == 0, "inherited Slice 43C verifier")
        ledger.check(
            "AI.WEB SLICE 43C VERIFIER: PASS" in inherited_output,
            "inherited Slice 43C verdict",
        )
    ...
    print(f"slice43d_files={len(expected_paths)}")
    print("meaning_preservation_comparison=1")
    print("dimension_specific_findings_created=13")
    ...
    print("finding_outcomes_preserved_changed_missing_unsupported_conflicted_indeterminate=1")
    ...
    print(f"AI.WEB SLICE 43D VERIFIER: {verdict}")
