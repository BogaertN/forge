#!/usr/bin/env python3
"""Visible sequential verifier for AI.Web Slice 40F."""
...
EXPECTED_HEAD = "2727dc72cbaa436a7c31eec4bb916452c1261c8e"
EXPECTED_TREE = "8a55315ab2a7a2f13a42e4cf26a60e908301c67a"
EXPECTED_SUBJECT = "Slice 40E deterministic connectedness gate runtime"
EXPECTED_COMMITTED_SUBJECT = "Slice 40F deterministic intended-purport recoverable-purpose runtime"
EXPECTED_PROTECTED_COUNT = 544
EXPECTED_PAYLOAD_COUNT = 13
EXPECTED_BEHAVIOR_CHECKS = 144
EXPECTED_MALFORMED_CASES = 78
CURRENT_TEST = "scripts/test_aiweb_slice40f_recoverable_purpose_runtime.py"
PARENT_CONTEXT_TEST = "scripts/test_aiweb_slice40e_connectedness_gate_runtime.py"
...
PROHIBITED_EVALUATOR_FUNCTIONS = {
    "evaluate_intended_purport",
    "compose_gate_outcomes",
    "select_meaning",
}
...
def static_source_checks(repository: Path, verification: Verification) -> None:
    ...
    congruity = parent / "congruity_gate"
    verification.check(
        tuple(sorted(path.name for path in congruity.glob("*.py")))
        == ("__init__.py", "canonical.py", "evaluator.py", "identity.py", "schema.py", "validation.py"),
        "accepted Slice 40D congruity files unchanged",
    )
    connectedness = parent / "connectedness_gate"
    verification.check(
        tuple(sorted(path.name for path in connectedness.glob("*.py")))
        == ("__init__.py", "canonical.py", "evaluator.py", "identity.py", "schema.py", "validation.py"),
        "accepted Slice 40E connectedness files unchanged",
    )
    package = parent / "recoverable_purpose_gate"
    files = tuple(sorted(package.glob("*.py")))
    verification.check(
        tuple(path.name for path in files)
        == ("__init__.py", "canonical.py", "evaluator.py", "identity.py", "schema.py", "validation.py"),
        "exact Slice 40F recoverable_purpose files",
    )
    ...
    for path in files:
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.Import):
                ...
                if node.name == "evaluate_recoverable_purpose":
    ...
    verification.check(observed_evaluator, "recoverable_purpose evaluator installed")
...
def main() -> int:
    ...
    print("AI.WEB SLICE 40F VISIBLE INDEPENDENT VERIFIER")
    ...
    protected_path = repository / "scripts" / "AIWEB_SLICE40F_PROTECTED_PREDECESSOR_SHA256SUMS.txt"
    try:
    ...
    exact_path_file = repository / "scripts" / "AIWEB_SLICE40F_EXACT_PAYLOAD_PATHS.txt"
    ...
    for index, relative in enumerate(tests, 1):
        ...
        if relative == PARENT_CONTEXT_TEST:
            parent_context_pass = "AI.WEB SLICE 40E CONNECTEDNESS GATE BEHAVIOR TEST: PASS" in output
    ...
    if args.mode != "source":
        verification.check(parent_context_pass, "accepted Slice 40E context pass marker")

    required_markers = (
        'AI.WEB SLICE 40F RECOVERABLE PURPOSE BEHAVIOR TEST: PASS',
        'distinction_kinds=9',
        'communicative_act_kinds=18',
        'finding_kinds=7',
        'overall_states=6',
        'recoverable_purpose_evaluator_installed=1',
        'mention_not_request=1',
        'inspect_not_execute=1',
        'propose_not_install=1',
        'simulate_not_alter_live_state=1',
        'report_not_prove=1',
        'retrieve_memory_not_write_memory=1',
        'ask_permission_not_possess_permission=1',
        'verify_request_not_verified_status=1',
        'delivery_meaning_not_delivery_authority=1',
        'recoverable_result=1',
        'unrecoverable_result=1',
        'ambiguous_result=1',
        'unsupported_result=1',
        'conflicted_result=1',
        'indeterminate_result=1',
        'exact_candidate_records_used=1',
        'approved_discourse_ancestry_only=1',
        'authorized_reference_state_only=1',
        'exact_active_context_only=1',
        'hidden_intent_inference_used=0',
        'capability_existence_inference_used=0',
        'prior_conversation_habit_used=0',
        'assistant_intuition_used=0',
        'psychological_inference_used=0',
        'emotional_interpretation_used=0',
        'raw_text_only_inference_used=0',
        'purpose_conflation_used=0',
        'automatic_purpose_collapse_used=0',
        'candidate_structure_mutated=0',
        'clarification_required_created=0',
        'rejection_created=0',
        'composed_gate_outcome_created=0',
        'candidate_disposition_created=0',
    ...
    print()
    print("=== SLICE 40F VERIFIER SUMMARY ===")
    ...
    if verification.failures or test_failures:
        print("SLICE 40F VISIBLE INDEPENDENT VERIFIER: FAIL")
        return 1
    print("SLICE 40F VISIBLE INDEPENDENT VERIFIER: PASS")
    print("protected_predecessor_files=544")
    print(f"inherited_tests={len(inherited)}")
    print(f"visible_total_tests={len(tests)}")
    print(f"slice40f_files={EXPECTED_PAYLOAD_COUNT}")
    print(f"behavior_checks={observed_checks}")
    print(f"malformed_validation_cases={observed_malformed}")
    print('distinction_kinds=9')
    print('communicative_act_kinds=18')
