#!/usr/bin/env python3
"""Visible sequential verifier for AI.Web Slice 41B.

The verifier proves the Slice 41B additive schema-only payload, protects every
accepted predecessor file captured by the source-authority packet, and runs the
current behavior test plus the inherited accepted language-core tests in
applied or committed mode.  It never stages, commits, pushes, modifies Forge,
or suppresses visible test output.
"""
...
EXPECTED_HEAD = "9c9a9135d07991446a720b845611bf3c153db522"
EXPECTED_TREE = "781a691f863aba4defc2dab12df192be6c18b075"
EXPECTED_SUBJECT = "Slice 41A selected meaning runtime core schema and authority contract"
EXPECTED_COMMITTED_SUBJECT = "Slice 41B deterministic validation identity versioning lifecycle"
EXPECTED_PROTECTED_COUNT = 599
EXPECTED_PAYLOAD_COUNT = 15
CURRENT_TEST = "scripts/test_aiweb_slice41b_deterministic_validation_identity_versioning_lifecycle.py"
PARENT_CONTEXT_TEST = "scripts/test_aiweb_slice41a_selected_meaning_runtime_core_schema.py"
...
def run_commit_context(
    ...
    with tempfile.TemporaryDirectory(prefix="aiweb_slice41b_pre39g_") as temporary:
        root = Path(temporary)
        try:
        ...
        return run_visible([python_executable, "-B", str(root / relative), str(root)], root, env)
...
def run_source_only(
    ...
    with tempfile.TemporaryDirectory(prefix="aiweb_slice41b_slice38h_") as temporary:
        root = Path(temporary)
        try:
...
def verify_source_contract(repository: Path, ledger: VerificationLedger) -> None:
    ...
    parent_files = tuple(sorted(path.name for path in parent.iterdir() if path.is_file())) if parent.is_dir() else ()
    ledger.check(parent_files == ("__init__.py", "authority.py", "identity.py", "schema.py"), "accepted Slice 41A parent files unchanged")
    ...
    ledger.check(actual == expected_package_files, "exact Slice 41B governed-lifecycle file set")
    ...
    prohibited_functions = {
        "evaluate_selection_eligibility", "select_candidate", "rank_candidate",
        "choose_candidate", "resolve_ambiguity", "create_selected_meaning",
        "integrate_selected_meaning_into_manifest", "render", "deliver",
    }
    prohibited_tokens = (
        "SelectedGovernedMeaningRecord(", "integrate_selected_meaning(",
        "evaluate_selection_eligibility(", "select_candidate(",
        "rank_candidate(",
    )
    for name in expected_package_files:
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.Import):
                ...
                ledger.check(node.name not in prohibited_functions, f"no prohibited function {name}:{node.name}")
            elif isinstance(node, ast.Call):
                if isinstance(node.func, ast.Name):
                    ledger.check(node.func.id not in prohibited_calls, f"no prohibited call {name}:{node.func.id}")
                elif isinstance(node.func, ast.Attribute):
                    ledger.check(node.func.attr not in prohibited_attributes, f"no prohibited attribute call {name}:{node.func.attr}")
        ledger.check(imported_roots.isdisjoint(prohibited_import_roots), "no prohibited imports in " + name)
        for token in prohibited_tokens:
            ledger.check(token not in source, f"no prohibited implementation token {name}:{token}")
...
def main() -> int:
    ...
    exact_paths_file = repository / "scripts" / "AIWEB_SLICE41B_EXACT_PAYLOAD_PATHS.txt"
    protected_file = repository / "scripts" / "AIWEB_SLICE41B_PROTECTED_PREDECESSOR_SHA256SUMS.txt"
    try:
        payload_paths = tuple(sorted(line for line in exact_paths_file.read_text(encoding="utf-8").splitlines() if line))
    ...
    for relative in payload_paths:
        ...
        ledger.check(path.is_file() and stat.S_IMODE(path.stat().st_mode) == expected_mode, "payload mode " + relative)
    ...
    if arguments.mode != "source":
        ...
        ledger.check(branch.returncode == 0 and branch.stdout.strip() == EXPECTED_BRANCH, "branch")
        if arguments.mode == "applied":
            ...
            ledger.check(tuple(sorted(untracked.stdout.splitlines())) == payload_paths, "applied exact untracked paths")
        else:
            ...
            ledger.check(tuple(sorted(committed.stdout.splitlines())) == payload_paths, "committed exact paths")
            ledger.check(not staged.stdout.strip() and not tracked.stdout.strip() and not untracked.stdout.strip(), "committed tree clean")
    ...
    for index, relative in enumerate(tests, 1):
        ...
        if relative == PRE_SLICE39G_CONTEXT_TEST:
            return_code, output = run_commit_context(repository, relative, PRE_SLICE39G_CONTEXT_COMMIT, python_executable, env, ledger)
        elif relative == SLICE38H_INHERITED_TEST:
            return_code, output = run_source_only(repository, relative, python_executable, env, ledger)
        else:
            return_code, output = run_visible([python_executable, "-B", str(repository / relative), str(repository)], repository, env)
        ...
        if relative == PARENT_CONTEXT_TEST:
            ledger.check("AI.WEB SLICE 41A BEHAVIOR TEST: PASS" in output, "Slice 41A context marker")

    markers = (
        "AI.WEB SLICE 41B BEHAVIOR TEST: PASS",
        "record_types=15",
        "lifecycle_stages=11",
        "lifecycle_transition_rules=35",
        "canonical_serialization=1",
        "deterministic_sha256_identities=1",
        "exact_schema_version_custody=1",
        "exact_predecessor_reference_validation=1",
        "duplicate_rejection=1",
        "identity_collision_rejection=1",
        "unknown_version_rejection=1",
        "immutable_successor_records=1",
        "malformed_record_rejection=1",
        "cross_record_consistency_checks=1",
        "valid_record_is_valid_candidate_meaning=0",
        "valid_record_is_successful_gate_result=0",
        "valid_record_is_selection_eligibility=0",
        "selection_lifecycle_is_selected_meaning=0",
        "selection_eligibility_evaluated=0",
        "candidate_ranked=0",
        "selection_performed=0",
        "selected_meaning_created=0",
        "msm_v1_modified=0",
        "bootstrap_integration_enabled=0",
    ...
    if ledger.failures or failed_tests:
        print("SLICE 41B VISIBLE INDEPENDENT VERIFIER: FAIL")
        return 1
    print("SLICE 41B VISIBLE INDEPENDENT VERIFIER: PASS")
    print("protected_predecessor_files=599")
    print("inherited_tests=" + str(len(tests) - 1))
    print("visible_total_tests=" + str(len(tests)))
    print("slice41b_files=15")
    print("record_types=15")
    print("validation_only=1")
