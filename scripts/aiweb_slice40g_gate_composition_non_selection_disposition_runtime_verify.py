#!/usr/bin/env python3
"""Visible sequential verifier for AI.Web Slice 40G."""
...
EXPECTED_HEAD = "8dd471f4e20024a3b64e5eb9ffac39815090fb39"
EXPECTED_TREE = "1d8b5ac529f6af78e17d69c264aabc4471481909"
EXPECTED_SUBJECT = "Slice 40F deterministic intended-purport recoverable-purpose runtime"
EXPECTED_COMMITTED_SUBJECT = "Slice 40G deterministic gate composition non-selection disposition runtime"
EXPECTED_PROTECTED_COUNT = 557
EXPECTED_PAYLOAD_COUNT = 13
EXPECTED_BEHAVIOR_CHECKS = 182
EXPECTED_MALFORMED_CASES = 87
CURRENT_TEST = "scripts/test_aiweb_slice40g_gate_composition_non_selection_disposition_runtime.py"
PARENT_CONTEXT_TEST = "scripts/test_aiweb_slice40f_recoverable_purpose_runtime.py"
...
PROHIBITED_EVALUATOR_FUNCTIONS = {
    "select_meaning", "accept_candidate", "authorize_action",
    "create_outward_refusal", "route_candidate", "invoke_tool",
}
...
def verify_git_state(repository: Path, mode: str, verification: Verification) -> None:
    ...
    verification.check(
        all(item.returncode == 0 for item in (branch, head, tree, subject, staged, tracked, untracked)),
        "Git inspection",
    )
    if mode == "applied":
        ...
        verification.check(branch.stdout.strip() == EXPECTED_BRANCH, "committed branch")
        verification.check(subject.stdout.strip() == EXPECTED_COMMITTED_SUBJECT, "committed subject")
        verification.check(actual == PAYLOAD_PATHS, "committed exact payload")
...
def static_source_checks(repository: Path, verification: Verification) -> None:
    ...
    expected_existing = {
        "governed_lifecycle": ("__init__.py", "canonical.py", "identity.py", "lifecycle.py", "rules.py", "schema.py", "validation.py"),
        "expectancy_gate": ("__init__.py", "canonical.py", "evaluator.py", "identity.py", "schema.py", "validation.py"),
        "congruity_gate": ("__init__.py", "canonical.py", "evaluator.py", "identity.py", "schema.py", "validation.py"),
        "connectedness_gate": ("__init__.py", "canonical.py", "evaluator.py", "identity.py", "schema.py", "validation.py"),
        "recoverable_purpose_gate": ("__init__.py", "canonical.py", "evaluator.py", "identity.py", "schema.py", "validation.py"),
    }
    for directory, names in expected_existing.items():
        observed = tuple(sorted(path.name for path in (parent / directory).glob("*.py")))
        verification.check(observed == names, f"accepted {directory} files unchanged")
    package = parent / "gate_composition"
    ...
    verification.check(
        ...
        "exact Slice 40G gate_composition files",
    )
    ...
    for path in files:
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.Import):
                ...
                if node.name == "evaluate_gate_composition":
                    observed_evaluator = True
                verification.check(node.name not in PROHIBITED_EVALUATOR_FUNCTIONS, f"no selection/action function {path.name}:{node.name}")
    ...
    verification.check(observed_evaluator, "gate composition evaluator installed")
    decision = (repository / "scripts/AIWEB_SLICE40G_GATE_COMPOSITION_NON_SELECTION_DISPOSITION_RUNTIME_DECISION.md").read_text(encoding="utf-8")
    verification.check("candidate_supported_for_later_selection_review" in decision, "positive disposition source-grounded")
    verification.check("not selected meaning" in decision.lower(), "positive disposition non-selection boundary")
...
def stream_commit_context_test(repository: Path, relative: str, commit: str, label: str, *, python_executable: str, env: dict[str, str], verification: Verification) -> tuple[int, str]:
    archive = subprocess.run(
        ["git", "-C", str(repository), "archive", "--format=tar", commit, "aiweb_language_core_bootstrap", "scripts"],
        stdout=subprocess.PIPE,
        stderr=subprocess.PIPE,
        check=False,
    )
    ...
    with tempfile.TemporaryDirectory(prefix=f"aiweb_slice40g_{label}_") as temporary:
...
def stream_source_only_inherited_test(repository: Path, relative: str, label: str, *, python_executable: str, env: dict[str, str], verification: Verification) -> tuple[int, str]:
    with tempfile.TemporaryDirectory(prefix=f"aiweb_slice40g_{label}_") as temporary:
        source_only_repository = Path(temporary)
        try:
            os.symlink(repository / "aiweb_language_core_bootstrap", source_only_repository / "aiweb_language_core_bootstrap", target_is_directory=True)
            os.symlink(repository / "scripts", source_only_repository / "scripts", target_is_directory=True)
...
def main() -> int:
    ...
    print("AI.WEB SLICE 40G VISIBLE INDEPENDENT VERIFIER")
    ...
    protected_path = repository / "scripts/AIWEB_SLICE40G_PROTECTED_PREDECESSOR_SHA256SUMS.txt"
    try:
    ...
    exact_path_file = repository / "scripts/AIWEB_SLICE40G_EXACT_PAYLOAD_PATHS.txt"
    ...
    for index, relative in enumerate(tests, 1):
        ...
        if relative == PRE_SLICE39G_CONTEXT_TEST:
            return_code, output = stream_commit_context_test(
                ...
                "accepted_pre_slice39g_source", python_executable=python_executable,
                env=env, verification=verification,
            )
        ...
        if relative == PARENT_CONTEXT_TEST:
            parent_context_pass = "AI.WEB SLICE 40F RECOVERABLE PURPOSE BEHAVIOR TEST: PASS" in output
    ...
    if args.mode != "source":
        verification.check(parent_context_pass, "accepted Slice 40F context pass marker")

    required_markers = (
        "AI.WEB SLICE 40G GATE COMPOSITION BEHAVIOR TEST: PASS",
        "disposition_kinds=7", "finding_kinds=7", "composition_statuses=5",
        "gate_composition_evaluator_installed=1", "all_four_gate_results_preserved=1",
        "no_gate_substitution=1", "composition_by_preservation_not_collapse=1",
        "material_ambiguity_preserved=1", "clarification_relevant=1",
        "unsupported_disposition=1", "refusal_relevant=1", "held_disposition=1",
        "blocked_progression=1", "candidate_supported_for_later_selection_review=1",
        "multiple_candidates_automatic_ambiguity=0", "missing_role_automatic_clarification=0",
        "unsupported_automatic_refusal=0", "refusal_relevant_outward_refusal=0",
        "gate_supported_candidate_selected_meaning=0", "candidate_structure_mutated=0",
        "truth_evidence_permission_execution=0", "route_tool_action_memory_rendering_delivery=0",
    )
    for marker in required_markers:
        verification.check(marker in current_output, f"behavior marker {marker}")

    print()
    print("=== SLICE 40G VERIFIER SUMMARY ===")
    ...
    if verification.failures or test_failures:
        print("SLICE 40G VISIBLE INDEPENDENT VERIFIER: FAIL")
        return 1
    print("SLICE 40G VISIBLE INDEPENDENT VERIFIER: PASS")
    print("protected_predecessor_files=557")
    print(f"inherited_tests={len(inherited)}")
    print(f"visible_total_tests={len(tests)}")
    print(f"slice40g_files={EXPECTED_PAYLOAD_COUNT}")
    print(f"behavior_checks={observed_checks}")
    print(f"malformed_validation_cases={observed_malformed}")
    print("disposition_kinds=7")
    print("finding_kinds=7")
    print("composition_statuses=5")
