#!/usr/bin/env python3
"""Independent verifier for Slice 37A controlled concept-authority schema."""
...
EXPECTED_PARENT_HEAD = "5bd8a39b91e7ead06523e7fd0aa3ee057c795f74"
EXPECTED_PARENT_SUBJECT = "Slice 36H bounded bootstrap integration and Slice 36 closeout"
EXPECTED_COMMIT_SUBJECT = "Slice 37A controlled concept authority schema contract"
EXACT_PATHS = ('aiweb_language_core_bootstrap/controlled_concept_sense_registry/__init__.py',
 'aiweb_language_core_bootstrap/controlled_concept_sense_registry/schema.py',
 'aiweb_language_core_bootstrap/controlled_concept_sense_registry/identity.py',
 'aiweb_language_core_bootstrap/controlled_concept_sense_registry/authority.py',
 'aiweb_language_core_bootstrap/controlled_concept_sense_registry/validation.py',
 'scripts/AIWEB_SLICE37A_CONTROLLED_CONCEPT_AUTHORITY_SCHEMA_RUNTIME_SPEC.md',
 'scripts/AIWEB_SLICE37A_SCHEMA_ONLY_AND_SLICE8_PRESERVATION_DECISION.md',
 'scripts/README_aiweb_slice37a_controlled_concept_authority_schema.md',
 'scripts/test_aiweb_slice37a_controlled_concept_authority_schema.py',
 'scripts/aiweb_slice37a_controlled_concept_authority_schema_verify.py')
...
def main() -> int:
    ...
    print("AI.WEB SLICE 37A INDEPENDENT VERIFIER")
    ...
    if args.mode == "source_only":
        source_only_missing = tuple(
            relative
            for relative in predecessor_mismatches
        ...
        check(not predecessor_mismatches, "all 167 predecessor hashes")
    ...
    for fragment in ("lookup", "resolve", "select", "populate", "traverse", "render", "route", "execute"):
        check(
    ...
    env["PYTHONPYCACHEPREFIX"] = f"/tmp/aiweb_slice37a_verify_cache_{os.getpid()}"

    test_result = run(
        (sys.executable, "-B", "scripts/test_aiweb_slice37a_controlled_concept_authority_schema.py", str(repo)),
        cwd=repo,
        env=env,
    )
    if test_result.returncode != 0:
        print("--- Slice 37A behavior test stdout ---")
        print(test_result.stdout)
        print("--- Slice 37A behavior test stderr ---")
        print(test_result.stderr)
    check(test_result.returncode == 0, "Slice 37A behavior test")
    ...
    if args.mode != "source_only":
        ...
        if args.mode == "precommit":
            ...
            check(status.returncode == 0 and actual == expected, "exact precommit ten-file state")
            ...
            check(changed.returncode == 0 and actual_changed == tuple(sorted(EXACT_PATHS)), "exact committed ten-file set")
        ...
        for relative in INHERITED_COMMANDS:
            ...
            if result.returncode != 0:
                ...
                print(result.stdout)
                print(result.stderr)
        check(not inherited_failures, "all 20 inherited behavior tests")
    ...
    if failures:
        ...
        print("SLICE 37A INDEPENDENT VERIFIER: FAIL")
        return 1

    print(f"pass_count={passes}")
    print("failure_count=0")
    print("SLICE 37A INDEPENDENT VERIFIER: PASS")
    print(f"protected_predecessor_files={len(PROTECTED_HASHES)}")
    print(f"inherited_tests={len(INHERITED_COMMANDS)}")
    print(f"slice37a_files={len(EXACT_PATHS)}")
    print("resource_record_kinds=11")
    print("lifecycle_states=15")
    print("schema_contract_registry_entries=0")
