#!/usr/bin/env python3
"""Visible sequential verifier for AI.Web Slice 41F.

The verifier proves the disabled explicit fixture-only selected-meaning closeout, protects
every accepted predecessor file, and visibly runs the current behavior test plus all
...
EXPECTED_HEAD = '1aa5513e14593e4e2d510161f3204a38536d87ea'
EXPECTED_TREE = 'aca30bba4b2b52f8cac6f61f697185a91c534c3d'
EXPECTED_SUBJECT = 'Slice 41E MSM-v1 selected meaning integration and custody'
EXPECTED_COMMITTED_SUBJECT = 'Slice 41F disabled bootstrap integration and Slice 41 closeout'
EXPECTED_PROTECTED_COUNT = 659
EXPECTED_PAYLOAD_COUNT = 15
CURRENT_TEST = 'scripts/test_aiweb_slice41f_disabled_bootstrap_integration_and_slice41_closeout.py'
PARENT_CONTEXT_TEST = 'scripts/test_aiweb_slice41e_msm_selected_meaning_integration_and_custody.py'
...
def run_commit_context(
    ...
    with tempfile.TemporaryDirectory(prefix="aiweb_slice41f_pre39g_") as temporary:
        root = Path(temporary)
        try:
...
def run_source_only(
    ...
    with tempfile.TemporaryDirectory(prefix="aiweb_slice41f_slice38h_") as temporary:
        root = Path(temporary)
        try:
            os.symlink(
...
def verify_source_contract(repository: Path, ledger: VerificationLedger) -> None:
    selected_parent = (
        repository / "aiweb_language_core_bootstrap" / "selected_meaning_runtime"
    )
    parent_files = (
        tuple(sorted(path.name for path in selected_parent.iterdir() if path.is_file()))
        if selected_parent.is_dir() else ()
    )
    ledger.check(
    ...
    for package_name, expected_files in expected_selected_packages.items():
        package = selected_parent / package_name
        actual = (
            tuple(sorted(path.name for path in package.iterdir() if path.is_file()))
            if package.is_dir() else ()
        )
        ledger.check(actual == expected_files, "accepted package unchanged " + package_name)

    package = (
        repository
        / "aiweb_language_core_bootstrap"
        / "disabled_selected_meaning_closeout"
    )
    ...
    ledger.check(
        ...
        "exact Slice 41F package file set",
    )
    ...
    prohibited_attributes = {
        ...
        "unlink", "mkdir", "makedirs", "rename", "rmdir", "remove",
        "touch", "connect", "send", "recv",
    }
    prohibited_functions = {
        ...
        "render", "deliver", "invoke_tool", "write_memory", "start_slice42",
    }
    prohibited_tokens = (
        "semantic_similarity(", "nearest_known(", "confidence_score(",
        "probability_score(", "model.predict(", "runpy.", "subprocess.",
        "manifest.candidate_meanings.append(",
        "manifest.non_selection_outcomes.append(",
        "manifest.selected_governed_meanings.append(",
        "GovernedOutwardMeaningRecord(", "ExpressionLinkRecord(",
        "ValidationLinkRecord(", "DeliveryContainmentLinkRecord(",
    )
    ...
    fixtures_source = (package / "fixtures.py").read_text(encoding="utf-8")

    ledger.check(
        "run_disabled_selected_meaning_closeout" in integration_source,
        "disabled selected-meaning closeout operation installed",
    )
    ledger.check(
        "integrate_selected_meaning_into_manifest" in integration_source,
        "accepted Slice 41E integration API invoked",
    )
    ledger.check(
        "validate_integration_input" in integration_source
        and "validate_integration_result" in integration_source,
        "exact Slice 41E validation required",
    )
    ledger.check(
        "MeaningStructureManifestV1(" not in integration_source,
        "MSM-v1 schema is not reconstructed or replaced",
    )
    ledger.check(
        "Slice41AcceptanceRecord" in schema_source
        and "Slice41RollbackMetadata" in schema_source,
        "closeout and rollback contracts installed",
    )
    ledger.check(
        "slice42_started=False" in integration_source,
        "Slice 42 remains unstarted",
    )
    ledger.check(
        "selected_meaning_bounded_semantic_custody_only" in schema_source
        and "unresolved_state_preserved" in schema_source,
        "bounded custody and unresolved preservation explicit",
    )
    ledger.check(
        "SLICE41_INCREMENT_LABELS" in authority_source
        and '"41F"' in authority_source,
        "fixed Slice 41 sequence recorded",
    )
    ledger.check(
        "expected_unresolved_outcome_count=1" in fixtures_source,
        "closed fixture requires unresolved state",
    )
    ledger.check(
        "PUBLIC_VALIDATORS" in validation_source,
        "fail-closed public validators installed",
    )
    ledger.check(
        "runtime_self_grants_acceptance=False" in integration_source,
        "runtime self-acceptance prohibited",
    )
...
def main() -> int:
    ...
    exact_paths_file = repository / "scripts/AIWEB_SLICE41F_EXACT_PAYLOAD_PATHS.txt"
    protected_file = repository / "scripts/AIWEB_SLICE41F_PROTECTED_PREDECESSOR_SHA256SUMS.txt"
    try:
    ...
    for index, relative in enumerate(tests, 1):
        ...
        if relative == PARENT_CONTEXT_TEST:
            ledger.check(
                "AI.WEB SLICE 41E BEHAVIOR TEST: PASS" in output,
                "Slice 41E context marker",
            )

    markers = (
        "AI.WEB SLICE 41F DISABLED BOOTSTRAP INTEGRATION AND SLICE 41 CLOSEOUT TEST",
        "slice41a_through_41f_completed=1",
        "exact_slice41e_integration_required=1",
        "selected_meaning_bounded_semantic_custody_only=1",
        "alternatives_preserved=1",
        "unresolved_state_preserved=1",
        "disabled_by_default=1",
        "explicit_invocation_required=1",
        "accepted_static_fixture_only=1",
        "offline_in_memory_deterministic=1",
        "final_slice41_acceptance_record_created=1",
        "pre_slice41_tree_recovery=1",
        "exact_staged_path_containment=1",
        "slice42_started=0",
        "truth_evidence_permission_execution_authority=0",
        "outward_expression_authority=0",
        "route_api_network_filesystem_memory_tool_action_rendering_delivery=0",
        "language_model_embedding_vector_rag_similarity=0",
        "failure_count=0",
        "AI.WEB SLICE 41F BEHAVIOR TEST: PASS",
    )
    for marker in markers:
        ledger.check(marker in current_output, "current behavior marker " + marker)

    print("\n=== SLICE 41F VERIFIER SUMMARY ===")
    ...
    if ledger.failures or failed_tests:
        print("SLICE 41F VISIBLE INDEPENDENT VERIFIER: FAIL")
        return 1
    print("SLICE 41F VISIBLE INDEPENDENT VERIFIER: PASS")
    print("protected_predecessor_files=659")
    print("inherited_tests=" + str(len(tests) - 1))
    print("visible_total_tests=" + str(len(tests)))
    print("slice41f_files=15")
    print("disabled_selected_meaning_closeout_runtime=1")
