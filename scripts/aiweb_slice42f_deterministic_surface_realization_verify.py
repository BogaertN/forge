#!/usr/bin/env python3
"""Visible independent verifier for AI.Web Slice 42F."""
...
EXPECTED_HEAD = "48f4b6d698350461eea3aec95b7b2cc8ec08b204"
EXPECTED_TREE = "734b9d55e5341b8d60de982ad9b3f6ca7d425c98"
EXPECTED_SUBJECT = "Slice 42E controlled expression plan construction"
COMMIT_SUBJECT = "Slice 42F deterministic surface realization"

SCRIPT_DIR = Path(__file__).resolve().parent
PAYLOAD_MANIFEST = SCRIPT_DIR / "AIWEB_SLICE42F_EXACT_PAYLOAD_PATHS.txt"
PROTECTED_MANIFEST = SCRIPT_DIR / "AIWEB_SLICE42F_PROTECTED_PREDECESSOR_SHA256SUMS.txt"
...
def run(command: Sequence[str], *, cwd: Path | None = None, env: dict[str, str] | None = None) -> subprocess.CompletedProcess[str]:
    return subprocess.run(list(command), cwd=str(cwd) if cwd else None, env=env, text=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, check=False)


def git(repo: Path, *arguments: str) -> subprocess.CompletedProcess[str]:
    return run(["git", "-c", f"safe.directory={repo}", "-C", str(repo), *arguments])
...
def read_paths(path: Path) -> tuple[Path, ...]:
    return tuple(Path(line) for line in path.read_text(encoding="utf-8").splitlines() if line)


def main() -> int:
    parser = argparse.ArgumentParser()
    parser.add_argument("repository", nargs="?", default=".")
    parser.add_argument("--mode", choices=("applied", "staged", "committed"), default="applied")
    ...
    untracked = set(Path(item) for item in lines(git(repo, "ls-files", "--others", "--exclude-standard")))
    ...
    if args.mode == "applied":
        ...
        committed = set(Path(item) for item in lines(git(repo, "diff-tree", "--no-commit-id", "--name-only", "-r", "HEAD")))
    ...
    for line in PROTECTED_MANIFEST.read_text(encoding="utf-8").splitlines():
        ...
        try:
            ...
            continue
        path = repo / relative
        check(path.is_file() and sha256_file(path) == expected_digest, "protected predecessor " + relative)
    check(protected_count == 1608, "protected predecessor count")
    ...
    for relative in payload:
        ...
        if not path.is_file():
            continue
        expected_mode = 0o755 if relative.suffix == ".py" and relative.name.startswith(("test_", "aiweb_slice42f_")) else 0o644
        check(stat.S_IMODE(path.stat().st_mode) == expected_mode, "payload mode " + relative.as_posix())

    runtime = repo / "aiweb_language_core_bootstrap/outward_expression_runtime/surface_realization"
    ...
    for path in runtime_files:
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.Import):
                ...
                check(not FORBIDDEN_RUNTIME_IMPORTS.intersection(imported), "forbidden runtime import " + path.name)
            elif isinstance(node, ast.ImportFrom):
                imported = {(node.module or "").split(".")[0]}
                check(not FORBIDDEN_RUNTIME_IMPORTS.intersection(imported), "forbidden runtime import " + path.name)
                check(not any(alias.name == "*" for alias in node.names), "wildcard runtime import " + path.name)
            elif isinstance(node, ast.Call) and isinstance(node.func, ast.Name):
                check(node.func.id not in FORBIDDEN_RUNTIME_CALLS, "forbidden runtime call " + path.name)

    pycache_root = tempfile.mkdtemp(prefix="aiweb42e_verifier_pycache_")
    env = os.environ.copy()
    env["PYTHONDONTWRITEBYTECODE"] = "1"
    env["PYTHONPYCACHEPREFIX"] = pycache_root
    env["PYTHONPATH"] = str(repo)
    test_path = repo / "scripts/test_aiweb_slice42f_deterministic_surface_realization.py"
    current_test = run([sys.executable, "-B", str(test_path), str(repo)], cwd=repo, env=env)
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="aiweb42e_parent_") as temp_dir:
            clone = Path(temp_dir) / "repo"
            clone_result = run(["git", "clone", "--no-hardlinks", "--quiet", str(repo), str(clone)])
            ...
            if clone_result.returncode == 0:
                ...
                inherited_path = clone / "scripts/aiweb_slice42e_controlled_expression_plan_construction_verify.py"
                inherited = run([sys.executable, "-B", str(inherited_path), str(clone), "--mode", "committed"], cwd=clone, env=env)
                print("=== INHERITED VISIBLE VERIFIER: SLICE 42E ===")
                print(inherited.stdout, end="")
                print(inherited.stderr, end="", file=sys.stderr)
                check(inherited.returncode == 0, "inherited Slice 42E verifier")
    finally:
        os.umask(prior_umask)

    print("=== SLICE 42F VERIFIER SUMMARY ===")
    ...
    print("slice42f_files=" + str(len(payload)))
    print("visible_total_tests=64")
    print("exact_slice42e_plan_state_required=1")
    print("separate_surface_realization_authority_required=1")
    print("admitted_deterministic_rules_only=1")
    print("controlled_resources_only=1")
    print("human_readable_expression_candidate_created=1")
    print("authorized_claim_strengthening=0")
    print("certainty_upgrade=0")
    print("evidence_status_upgrade=0")
    print("caveats_refusal_unresolved_visible=1")
    print("deterministic_realization_trace_created=1")
    print("deterministic_realization_receipt_created=1")
