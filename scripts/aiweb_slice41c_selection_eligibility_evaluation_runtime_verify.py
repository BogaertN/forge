#!/usr/bin/env python3
"""Visible sequential verifier for AI.Web Slice 41C.
...
EXPECTED_HEAD = "f48ed086c19f1a24fd23710cd2d852326ccee229"
EXPECTED_TREE = "0b9a9af60ab5de8c71841dbbe7e5a30402fd38fd"
EXPECTED_SUBJECT = "Slice 41B deterministic validation identity versioning lifecycle"
EXPECTED_COMMITTED_SUBJECT = "Slice 41C deterministic selection eligibility evaluation runtime"
EXPECTED_PROTECTED_COUNT = 614
EXPECTED_PAYLOAD_COUNT = 15
CURRENT_TEST = "scripts/test_aiweb_slice41c_selection_eligibility_evaluation_runtime.py"
PARENT_CONTEXT_TEST = "scripts/test_aiweb_slice41b_deterministic_validation_identity_versioning_lifecycle.py"
...
def run_commit_context(
    ...
    with tempfile.TemporaryDirectory(prefix="aiweb_slice41c_pre39g_") as temporary:
        root = Path(temporary)
        try:
...
def run_source_only(
    ...
    with tempfile.TemporaryDirectory(prefix="aiweb_slice41c_slice38h_") as temporary:
        root = Path(temporary)
        try:
            os.symlink(
...
def verify_source_contract(repository: Path, ledger: VerificationLedger) -> None:
    ...
    predecessor = parent / "governed_lifecycle"
    predecessor_files = (
        tuple(sorted(path.name for path in predecessor.iterdir() if path.is_file()))
        if predecessor.is_dir() else ()
    )
    ledger.check(
        predecessor_files == (
    ...
    package = parent / "eligibility_evaluation"
    ...
    ledger.check(actual == expected_package_files, "exact Slice 41C package file set")
    ...
    prohibited_functions = {
        "select_candidate", "rank_candidate", "choose_candidate",
        "resolve_ambiguity", "create_selected_meaning",
        "integrate_selected_meaning_into_manifest", "render", "deliver",
        "score_candidate", "classify_with_model",
    }
    prohibited_tokens = (
        "SelectedGovernedMeaningRecord(",
        "integrate_selected_meaning(",
        "select_candidate(",
        "rank_candidate(",
        "semantic_similarity(",
        "nearest_known(",
...
def main() -> int:
    ...
    exact_paths_file = repository / "scripts/AIWEB_SLICE41C_EXACT_PAYLOAD_PATHS.txt"
    protected_file = repository / "scripts/AIWEB_SLICE41C_PROTECTED_PREDECESSOR_SHA256SUMS.txt"
    try:
    ...
    for index, relative in enumerate(tests, 1):
        ...
        if relative == PARENT_CONTEXT_TEST:
            ledger.check(
                "AI.WEB SLICE 41B BEHAVIOR TEST: PASS" in output,
                "Slice 41B context marker",
            )

    markers = (
        "AI.WEB SLICE 41C BEHAVIOR TEST: PASS",
        "eligibility_outcomes=8",
        "approved_selection_authority_profiles=1",
        "exact_msm_v1_candidate_required=1",
        "exact_slice40h_gate_custody_required=1",
        "all_four_gate_family_results_required=1",
        "exact_slice40g_composition_required=1",
        "explicit_candidate_specific_support_required=1",
        "unresolved_and_alternative_custody_preserved=1",
        "eligible_for_selected_meaning_construction=1",
        "held_pending_authority=1",
        "materially_unresolved=1",
        "clarification_dependent=1",
        "unsupported=1",
        "conflicted=1",
        "indeterminate=1",
        "not_eligible=1",
        ...
        "msm_v1_modified=0",
        "confidence_probability_similarity_nearest_known=0",
        "language_model_hidden_classifier=0",
        "refusal_blocked_alternatives_erased=0",
    ...
    if ledger.failures or failed_tests:
        print("SLICE 41C VISIBLE INDEPENDENT VERIFIER: FAIL")
        return 1
    print("SLICE 41C VISIBLE INDEPENDENT VERIFIER: PASS")
    print("protected_predecessor_files=614")
    print("inherited_tests=" + str(len(tests) - 1))
    print("visible_total_tests=" + str(len(tests)))
    print("slice41c_files=15")
    print("eligibility_evaluation_runtime=1")
    print("eligibility_outcomes=8")
    print("candidate_specific_support_required=1")
    print("selection_eligibility_evaluated=1")
