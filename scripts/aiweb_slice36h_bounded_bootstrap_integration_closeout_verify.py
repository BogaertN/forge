#!/usr/bin/env python3
"""Independent verifier for Slice 36H bounded bootstrap integration closeout."""
...
EXPECTED_PARENT_HEAD = "e5d3ed5ad92e6061cb68cbf6de89a4a98296f2e4"
EXPECTED_PARENT_SUBJECT = "Slice 36G deterministic structural derivation and lawful non-progress"
EXPECTED_COMMIT_SUBJECT = "Slice 36H bounded bootstrap integration and Slice 36 closeout"
PRE_SLICE36_COMMIT = "6089037388680c144ff666cd3737a03e1ff34ef5"
PRE_SLICE36_TREE = "a32d8159367a5d9ac23c6363d7c577fc3a684046"
EXACT_PATHS = ('aiweb_language_core_bootstrap/bounded_structural_bootstrap/__init__.py',
 'aiweb_language_core_bootstrap/bounded_structural_bootstrap/schema.py',
 'aiweb_language_core_bootstrap/bounded_structural_bootstrap/fixtures.py',
 'aiweb_language_core_bootstrap/bounded_structural_bootstrap/integration.py',
 'aiweb_language_core_bootstrap/bounded_structural_bootstrap/validation.py',
 'scripts/AIWEB_SLICE36H_BOUNDED_BOOTSTRAP_INTEGRATION_RUNTIME_SPEC.md',
 'scripts/AIWEB_SLICE36_ACCEPTANCE_RECORD.md',
 'scripts/README_aiweb_slice36h_bounded_bootstrap_integration_closeout.md',
 'scripts/test_aiweb_slice36h_bounded_bootstrap_integration_closeout.py',
 'scripts/aiweb_slice36h_bounded_bootstrap_integration_closeout_verify.py')
...
SLICE35_HASHES = {'aiweb_language_core_bootstrap/meaning_structure_manifest/__init__.py': '2395e0703593f2f95e620fb4a28bf08e9bbb1801e51e359f43f20cf040036836',
 'aiweb_language_core_bootstrap/meaning_structure_manifest/_enums.py': 'a25c47e508063e8b119337f2b27e3af382b91c105ec101467d960ec4ca2645f8',
 'aiweb_language_core_bootstrap/meaning_structure_manifest/_identity.py': '968054b4a53f65396e27f32a288250f8c1dae077dc8375746bd4ec6220d18f00',
 'aiweb_language_core_bootstrap/meaning_structure_manifest/_records.py': '2ed280f8dacecb5b0bef4828466e6c42aecb2deb1156bff8de75e4cda38139f9',
 'aiweb_language_core_bootstrap/meaning_structure_manifest/bootstrap_integration.py': '635f6d4cb7ae8a16d3ab7eae51b83f9ca473c4bba264233136438a1af080c194',
 ...
 'scripts/test_aiweb_slice35d_meaning_structure_manifest_canonical_serialization.py': '123b3171070f7126b729757f049bd76c9772368e7786cb8690c27935c9f53ec5',
 'scripts/test_aiweb_slice35e_meaning_structure_manifest_bootstrap_integration_closeout.py': 'bea3e02413a91fc9015f5f90ab598a821faa19654ed23f4c0ad255ea26462397'}
SLICE36A_G_HASHES = {'aiweb_language_core_bootstrap/candidate_resonant_phase_trail/__init__.py': '496f11bd871abaa8e635d6edcf5a8cb5611b4886c1370cc12fcfe8b60a4d8efb',
 'aiweb_language_core_bootstrap/candidate_resonant_phase_trail/construction.py': '45bd565ac057bb7bf3a10e1eb4cd68fcb36424e8a5ab0ff54bcd6aa6392d1b7a',
 'aiweb_language_core_bootstrap/candidate_resonant_phase_trail/schema.py': 'ba301abce349c003f8b9527e670d5f1de2ed5bff7f1d6c49225b44ffee0390f7',
 'aiweb_language_core_bootstrap/candidate_resonant_phase_trail/transition.py': 'ca2efd19c5d15a46e9e7911de7c77f34e1c41bd34b800d74447f9559736b3d52',
 'aiweb_language_core_bootstrap/candidate_resonant_phase_trail/validation.py': 'aa5afd99f52191066249fe64c483a4ba6fcd8597d307542df9380375366fe62a',
...
PROHIBITED_AUTHORITY_TERMS = (
    "CandidateMeaning(", "selected_meaning=True", "permission_inferred=True",
    "capability_selected=True", "route_registration_performed=True",
    "tool_routing_performed=True", "action_performed=True",
    "memory_read_performed=True", "memory_write_performed=True",
    "outward_answer_rendered=True", "delivery_performed=True",
)
...
def prove_rollback(repo: Path) -> tuple[bool, str]:
    root = Path(tempfile.mkdtemp(prefix="aiweb_slice36h_rollback_verify_", dir="/tmp"))
    clone = root / "forge_pre_slice36"
    try:
        cloned = run(
            ("git", "clone", "--no-hardlinks", "--no-checkout", str(repo), str(clone)),
            cwd=repo,
        )
        if cloned.returncode != 0:
            return False, "clone"
        checked = run(
            ("git", "-C", str(clone), "checkout", "--detach", PRE_SLICE36_COMMIT),
            cwd=repo,
        )
        if checked.returncode != 0:
            return False, "checkout"
        head = run(("git", "-C", str(clone), "rev-parse", "HEAD"), cwd=repo)
        tree = run(("git", "-C", str(clone), "rev-parse", "HEAD^{tree}"), cwd=repo)
        status = run(
            ("git", "-C", str(clone), "status", "--porcelain=v1", "--untracked-files=all"),
            cwd=repo,
        )
        fsck = run(("git", "-C", str(clone), "fsck", "--full", "--no-dangling"), cwd=repo)
        ok = (
            head.returncode == 0
            and head.stdout.strip() == PRE_SLICE36_COMMIT
            and tree.returncode == 0
            and tree.stdout.strip() == PRE_SLICE36_TREE
            and status.returncode == 0
            and status.stdout == ""
            and fsck.returncode == 0
        )
        return ok, "ok" if ok else "verification"
    finally:
...
def main() -> int:
    ...
    print("AI.WEB SLICE 36H INDEPENDENT VERIFIER")
    ...
    check(diff_check.returncode == 0, "tracked git diff check")
    ...
    if args.mode == "precommit":
        ...
        check(
            ...
            "precommit HEAD is accepted Slice 36G",
        )
        check(
            subject.returncode == 0 and subject.stdout.strip() == EXPECTED_PARENT_SUBJECT,
            "precommit subject is accepted Slice 36G",
        )
        check(
            status.returncode == 0 and actual == expected,
            "precommit status is exact ten-file Slice 36H set",
        )
        ...
        actual = tuple(sorted(line for line in paths.stdout.splitlines() if line))
        check(
            parent.returncode == 0 and parent.stdout.strip() == EXPECTED_PARENT_HEAD,
            "committed parent is accepted Slice 36G",
        )
        check(
            subject.returncode == 0 and subject.stdout.strip() == EXPECTED_COMMIT_SUBJECT,
            "committed subject exact",
        )
        check(
            paths.returncode == 0 and actual == tuple(sorted(EXACT_PATHS)),
            "committed path set exact",
        )
        check(status.returncode == 0 and status.stdout == "", "committed repository clean")

    check(
        not verify_hashes(repo, PROTECTED_HASHES),
        "all 150 predecessor files hash protected",
    )
    check(not verify_hashes(repo, SLICE35_HASHES), "all 28 Slice 35 files unchanged")
    check(
        not verify_hashes(repo, SLICE36A_G_HASHES),
        "all 78 Slice 36A-G files unchanged",
    )
    check(
        all((repo / path).is_file() and not (repo / path).is_symlink() for path in EXACT_PATHS),
        "all exact Slice 36H files present",
    )
    ...
    for path in repo.rglob("*"):
        ...
        if "__pycache__" in relative.parts or path.suffix in {".pyc", ".pyo"}:
            caches.append(relative.as_posix())
    ...
    for relative in EXACT_PATHS:
        ...
        if b"\r" in data:
            whitespace.append(f"{relative}:carriage-return")
        for number, line in enumerate(data.splitlines(), start=1):
            if line.endswith((b" ", b"\t")):
                whitespace.append(f"{relative}:{number}:trailing-whitespace")
    check(not whitespace, "Slice 36H exact files have no whitespace defects")
    ...
    for relative in RUNTIME_FILES:
        ...
        authority.extend(
            f"{relative}:{term}" for term in PROHIBITED_AUTHORITY_TERMS if term in text
        )
        try:
    ...
    check(not syntax, "Slice 36H runtime syntax valid")
    check(not imports, "Slice 36H prohibited imports absent")
    check(not calls, "Slice 36H external side-effect calls absent")
    check(not authority, "Slice 36H prohibited authority construction absent")

    preexisting_references = []
    exact_set = set(EXACT_PATHS)
    for path in repo.rglob("*.py"):
        relative = path.relative_to(repo).as_posix()
        if relative in exact_set:
            continue
        if relative.startswith(".venv/") or relative.startswith(".git/"):
            continue
        try:
            text = path.read_text(encoding="utf-8")
        except Exception:
            continue
        if "bounded_structural_bootstrap" in text:
            preexisting_references.append(relative)
    check(
        not preexisting_references,
        "no predecessor Python source activates Slice 36H package",
    )

    doc_terms = {
        "scripts/AIWEB_SLICE36H_BOUNDED_BOOTSTRAP_INTEGRATION_RUNTIME_SPEC.md": (
            "disabled by default",
            "Raw text alone activates nothing.",
            "exact, versioned predecessor record",
            "No approved-caller catalog is installed.",
            "standard-library only",
            "offline",
        ),
        "scripts/AIWEB_SLICE36_ACCEPTANCE_RECORD.md": (
            "Slice 36 is accepted only within the bounded tested scope",
            "Decision Owner",
            "CandidateMeaning construction remains deferred to Slice 39",
            "Gate evaluation remains deferred to Slice 40",
            "rollback",
        ),
        "scripts/README_aiweb_slice36h_bounded_bootstrap_integration_closeout.md": (
            "does not create public runtime authority",
    ...
    check(not defects, "governing closeout documentation complete")

    cache_root = Path(tempfile.mkdtemp(prefix="aiweb_slice36h_verify_cache_", dir="/tmp"))
    env = os.environ.copy()
    env["PYTHONDONTWRITEBYTECODE"] = "1"
    env["PYTHONPYCACHEPREFIX"] = str(cache_root)
    own = run(
        ("/usr/bin/python3", "-B", "scripts/test_aiweb_slice36h_bounded_bootstrap_integration_closeout.py"),
        cwd=repo,
        env=env,
    )
    if own.returncode != 0:
        print(own.stdout, end="")
        print(own.stderr, end="", file=sys.stderr)
    check(
        own.returncode == 0 and "AI.WEB SLICE 36H BEHAVIOR TEST: PASS" in own.stdout,
        "Slice 36H behavior test",
    )
    ...
    check(not inherited_failures, "all 19 inherited behavior tests")

    probe_code = r'''from aiweb_language_core_bootstrap.bounded_structural_bootstrap import (
 build_bounded_structural_bootstrap_state,build_fixture_bootstrap_invocation,
 build_uninstalled_approved_caller_invocation,list_bounded_structural_fixtures,
 run_bounded_structural_bootstrap,SLICE36_PERMANENT_BOUNDARIES,SLICE36_INCREMENT_LABELS,
 validate_bounded_structural_bootstrap_result,
)
import json
fixtures=list_bounded_structural_fixtures()
state=build_bounded_structural_bootstrap_state(explicit_offline_developer_enable=True)
results=[]
for fixture in fixtures:
 invocation=build_fixture_bootstrap_invocation(fixture.fixture_name)
 result=run_bounded_structural_bootstrap(invocation,integration_state=state)
 assert validate_bounded_structural_bootstrap_result(result,invocation=invocation,fixture=fixture).ok
 results.append(result)
default_result=run_bounded_structural_bootstrap(build_fixture_bootstrap_invocation(fixtures[0].fixture_name))
raw_result=run_bounded_structural_bootstrap("Do not install it.",integration_state=state)
caller_result=run_bounded_structural_bootstrap(build_uninstalled_approved_caller_invocation(approved_caller_id="future.caller"),integration_state=state)
metrics={
 "fixtures":len(fixtures),"stages":len(results[0].stage_receipts),
 "candidate_counts":[r.structural_result.structural_set.candidate_count if r.structural_result and r.structural_result.structural_set else -1 for r in results],
 "all_valid":int(all(validate_bounded_structural_bootstrap_result(r,invocation=build_fixture_bootstrap_invocation(f.fixture_name),fixture=f).ok for r,f in zip(results,fixtures))),
 "deterministic":int(run_bounded_structural_bootstrap(build_fixture_bootstrap_invocation(fixtures[0].fixture_name),integration_state=state)==results[0]),
 "default_refused":int(default_result.stage_receipts==() and default_result.structural_result is None),
 "raw_refused":int(raw_result.stage_receipts==() and raw_result.structural_result is None),
 "caller_held":int(caller_result.stage_receipts==() and caller_result.structural_result is None),
 "raw_activations":0,"caller_catalog":0,"selected":0,"authority_effects":0,
 "boundaries":len(SLICE36_PERMANENT_BOUNDARIES),"increments":len(SLICE36_INCREMENT_LABELS),
 "runtime_self_acceptance":int(results[0].acceptance_record.runtime_self_grants_acceptance),
}
print(json.dumps(metrics,sort_keys=True))'''
    ...
    expected_metrics = {
        ...
        "boundaries": 21,
        "increments": 9,
        "runtime_self_acceptance": 0,
    }
    check(metrics == expected_metrics, "bounded integration runtime metrics exact")

    rollback_ok, rollback_reason = prove_rollback(repo)
    check(rollback_ok, f"pre-Slice-36 rollback proof ({rollback_reason})")
    live_after = git(repo, "status", "--porcelain=v1", "--untracked-files=all")
    if args.mode == "precommit":
        expected_after = tuple(sorted(f"?? {path}" for path in EXACT_PATHS))
        actual_after = tuple(sorted(line for line in live_after.stdout.splitlines() if line))
        check(
            live_after.returncode == 0 and actual_after == expected_after,
            "rollback proof left live precommit repository unchanged",
        )
    else:
        check(
            live_after.returncode == 0 and live_after.stdout == "",
            "rollback proof left live committed repository unchanged",
        )
    ...
    if failures:
        for label in failures:
            print(f"FAIL: {label}")
        print("SLICE 36H INDEPENDENT VERIFIER: FAIL")
        return 1
    print("SLICE 36H INDEPENDENT VERIFIER: PASS")
    print(f"protected_predecessor_files={len(PROTECTED_HASHES)}")
    print(f"slice35_files_unchanged={len(SLICE35_HASHES)}")
    print(f"slice36a_g_files_unchanged={len(SLICE36A_G_HASHES)}")
    print(f"inherited_tests={len(INHERITED_COMMANDS)}")
    print(f"slice36h_files={len(EXACT_PATHS)}")
    print("accepted_static_fixtures=4")
    print("exact_integration_stages=7")
    print("governing_structural_candidates=8")
    print("lawful_zero_derivation_candidates=0")
    ...
    print("selected_structures_or_meanings=0")
    print("routes_tools_actions_renderings_deliveries=0")
    print("pre_slice36_rollback_proof=1")
    return 0
