#!/usr/bin/env python3
"""Visible sequential verifier for AI.Web Slice 41E.

The verifier proves the additive MSM-v1 selected-meaning integration adapter, protects every
...
EXPECTED_HEAD = '95ba97835634d35f097267dae20d555b2b80bbcd'
EXPECTED_TREE = '7d7d2c1cb40f2fd650ad1358c36bb7f5e2acd16f'
EXPECTED_SUBJECT = 'Slice 41D selected meaning construction alternative preservation'
EXPECTED_COMMITTED_SUBJECT = 'Slice 41E MSM-v1 selected meaning integration and custody'
EXPECTED_PROTECTED_COUNT = 644
EXPECTED_PAYLOAD_COUNT = 15
CURRENT_TEST = 'scripts/test_aiweb_slice41e_msm_selected_meaning_integration_and_custody.py'
PARENT_CONTEXT_TEST = 'scripts/test_aiweb_slice41d_selected_meaning_construction_alternative_preservation.py'
...
def run_commit_context(
    ...
    with tempfile.TemporaryDirectory(prefix="aiweb_slice41e_pre39g_") as temporary:
        root = Path(temporary)
        try:
...
def run_source_only(
    ...
    with tempfile.TemporaryDirectory(prefix="aiweb_slice41e_slice38h_") as temporary:
        root = Path(temporary)
        try:
            os.symlink(
...
def verify_source_contract(repository: Path, ledger: VerificationLedger) -> None:
    ...
    for package_name, expected_files in expected_packages.items():
        package = parent / package_name
        actual = (
            tuple(sorted(path.name for path in package.iterdir() if path.is_file()))
            if package.is_dir() else ()
        )
        ledger.check(actual == expected_files, "exact package file set " + package_name)

    package = parent / "msm_selected_meaning_integration"
    expected_package_files = expected_packages["msm_selected_meaning_integration"]
    ...
    prohibited_functions = {
        ...
        "render", "deliver", "invoke_tool", "write_memory",
    }
    prohibited_tokens = (
        ...
        "manifest.external_authority_references.append(",
        "manifest.semantic_transition_traces.append(",
    ...
    ledger.check(
        ...
        "MSM selected-meaning integration API installed",
    )
    ledger.check(
        "SelectedGovernedMeaningRecord(" in integration_source,
        "integrated selected governed meaning constructed",
    )
    ledger.check(
        "append_lifecycle_successor(" in integration_source,
        "accepted MSM-v1 lifecycle append used",
    )
    ledger.check(
        "selection_receipt.receipt_id" in integration_source,
        "exact Slice 41D selection receipt bound",
    )
    ledger.check(
        "MeaningStructureManifestV1(" not in integration_source,
        "accepted MSM-v1 schema not reconstructed or replaced",
    )
    ledger.check(
        "candidate_meanings=source.candidate_meanings" not in integration_source,
        "candidate collection not manually replaced",
    )
    ledger.check(
        "SLICE41E_REQUIRED_EMPTY_SUCCESSOR_SECTIONS" in validation_source,
        "downstream sections explicitly protected",
    )
    ledger.check(
        "candidate_meanings" in validation_source
        and "non_selection_outcomes" in validation_source,
        "candidate and non-selection retention validated",
    )
...
def main() -> int:
    ...
    exact_paths_file = repository / "scripts/AIWEB_SLICE41E_EXACT_PAYLOAD_PATHS.txt"
    protected_file = repository / "scripts/AIWEB_SLICE41E_PROTECTED_PREDECESSOR_SHA256SUMS.txt"
    try:
    ...
    for index, relative in enumerate(tests, 1):
        ...
        if relative == PARENT_CONTEXT_TEST:
            ledger.check(
                "AI.WEB SLICE 41D BEHAVIOR TEST: PASS" in output,
                "Slice 41D context marker",
            )

    markers = (
        "AI.WEB SLICE 41E BEHAVIOR TEST: PASS",
        "source_candidate_meanings=2",
        "source_non_selection_outcomes=1",
        "successor_selected_meanings=1",
        "exact_slice40h_result_required=1",
        "exact_slice40h_custody_companion_required=1",
        "exact_slice41d_input_and_package_required=1",
        "exact_selected_candidate_reference=1",
        "exact_selection_receipt_authority=1",
        "immutable_msm_v1_successor=1",
        "candidate_and_gate_ancestry_preserved=1",
        "all_candidate_meanings_retained=1",
        "all_non_selection_outcomes_retained=1",
        "slice40h_custody_companion_retained=1",
        "lawful_semantic_transition_trace=1",
        "complete_successor_manifest_validated=1",
        "msm_v1_schema_modified=0",
        "automatic_migration_performed=0",
        "candidate_or_non_selection_deleted=0",
        "governed_result_or_outward_meaning_created=0",
        "expression_validation_delivery_links=0",
        "truth_evidence_permission_execution=0",
        "route_tool_action_memory_rendering_delivery=0",
        "filesystem_network_external_resource=0",
        "language_model_embedding_vector_rag_similarity=0",
    )
    for marker in markers:
        ledger.check(marker in current_output, "current behavior marker " + marker)

    print("\n=== SLICE 41E VERIFIER SUMMARY ===")
    ...
    if ledger.failures or failed_tests:
        print("SLICE 41E VISIBLE INDEPENDENT VERIFIER: FAIL")
        return 1
    print("SLICE 41E VISIBLE INDEPENDENT VERIFIER: PASS")
    print("protected_predecessor_files=644")
    print("inherited_tests=" + str(len(tests) - 1))
    print("visible_total_tests=" + str(len(tests)))
    print("slice41e_files=15")
    print("msm_selected_meaning_integration_runtime=1")
    print("exact_slice40h_custody_required=1")
    print("exact_slice41d_package_required=1")
    print("selected_meaning_integrated=1")
    print("exact_selection_receipt_bound=1")
