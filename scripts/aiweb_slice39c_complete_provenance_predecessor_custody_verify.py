#!/usr/bin/env python3
"""Visible sequential verifier for AI.Web Slice 39C."""
...
EXPECTED_HEAD = "2ecaf057dc4e25f664eceda01adaf6698209c940"
EXPECTED_TREE = "a10320c4093db3c4fc8766859542de1153e3caa3"
EXPECTED_SUBJECT = "Slice 39B deterministic validation identity versioning lifecycle"
EXPECTED_COMMITTED_SUBJECT = (
    "Slice 39C complete provenance predecessor custody"
)
EXPECTED_PREDECESSOR_COUNT = 391
EXPECTED_PAYLOAD_COUNT = 13
EXPECTED_BEHAVIOR_CHECKS = 3098
EXPECTED_MALFORMED_CASES = 250
EXPECTED_REJECTION_CASES = 96
EXPECTED_SOURCE_SPAN_REFERENCES = 27
EXPECTED_STRUCTURAL_RULE_REFERENCES = 17
EXPECTED_OPERATOR_REFERENCES = 2
EXPECTED_REGISTRY_RESOURCE_REFERENCES = 22
EXPECTED_STAGE_RECEIPTS = 8
EXPECTED_REGISTRY_RESOURCE_KINDS = 10
...
PAYLOAD_PATHS = (
    "aiweb_language_core_bootstrap/candidate_meaning_construction/predecessor_custody/__init__.py",
    "aiweb_language_core_bootstrap/candidate_meaning_construction/predecessor_custody/authority.py",
    "aiweb_language_core_bootstrap/candidate_meaning_construction/predecessor_custody/binding.py",
    "aiweb_language_core_bootstrap/candidate_meaning_construction/predecessor_custody/canonical.py",
    "aiweb_language_core_bootstrap/candidate_meaning_construction/predecessor_custody/identity.py",
    "aiweb_language_core_bootstrap/candidate_meaning_construction/predecessor_custody/schema.py",
    "aiweb_language_core_bootstrap/candidate_meaning_construction/predecessor_custody/validation.py",
    ...
    "scripts/AIWEB_SLICE39C_PROTECTED_PREDECESSOR_SHA256SUMS.txt",
    "scripts/README_aiweb_slice39c_complete_provenance_predecessor_custody.md",
    "scripts/aiweb_slice39c_complete_provenance_predecessor_custody_verify.py",
...
def stream_source_only_inherited_test(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(
        prefix="aiweb_slice39c_inherited_source_"
    ) as temporary:
...
def main() -> int:
    ...
    print("AI.WEB SLICE 39C VISIBLE INDEPENDENT VERIFIER")
    ...
    protected_path = (
        repository
        / "scripts"
        / "AIWEB_SLICE39C_PROTECTED_PREDECESSOR_SHA256SUMS.txt"
    )
    try:
    ...
    current_test = (
        "scripts/test_aiweb_slice39c_complete_provenance_"
        "predecessor_custody.py"
    )
    ...
    observed_source_span_references: int | None = None
    observed_structural_rule_references: int | None = None
    observed_operator_references: int | None = None
    observed_registry_resource_references: int | None = None
    observed_stage_receipts: int | None = None
    observed_registry_resource_kinds: int | None = None
    ...
    for index, relative in enumerate(tests, 1):
        ...
        if relative == current_test:
            ...
            observed_source_span_references = parse_single_integer_marker(
                output, "source_span_references"
            )
            observed_structural_rule_references = parse_single_integer_marker(
                output, "structural_rule_references"
            )
            observed_operator_references = parse_single_integer_marker(
                output, "operator_references"
            )
            observed_registry_resource_references = parse_single_integer_marker(
                output, "registry_resource_references"
            )
            observed_stage_receipts = parse_single_integer_marker(
                output, "stage_receipts"
            )
            observed_registry_resource_kinds = parse_single_integer_marker(
                output, "registry_resource_kinds"
            )
            for observed, expected, label in (
                ...
                (observed_source_span_references, EXPECTED_SOURCE_SPAN_REFERENCES, "source span reference count"),
                (observed_structural_rule_references, EXPECTED_STRUCTURAL_RULE_REFERENCES, "structural rule reference count"),
                (observed_operator_references, EXPECTED_OPERATOR_REFERENCES, "operator reference count"),
                (observed_registry_resource_references, EXPECTED_REGISTRY_RESOURCE_REFERENCES, "registry resource reference count"),
                (observed_stage_receipts, EXPECTED_STAGE_RECEIPTS, "stage receipt count"),
                (observed_registry_resource_kinds, EXPECTED_REGISTRY_RESOURCE_KINDS, "registry resource kind count"),
            ):
                verification.check(observed == expected, label)
            verification.check(
                "AI.WEB SLICE 39C BEHAVIOR TEST: PASS" in output,
                "Slice 39C pass marker",
            )
    ...
    print()
    print("=== SLICE 39C VERIFIER SUMMARY ===")
    ...
    if verification.failures or test_failures:
        print("SLICE 39C VISIBLE INDEPENDENT VERIFIER: FAIL")
        return 1

    print("SLICE 39C VISIBLE INDEPENDENT VERIFIER: PASS")
    ...
    print(f"slice39c_files={EXPECTED_PAYLOAD_COUNT}")
    ...
    print(f"source_span_references={observed_source_span_references}")
    print(f"structural_rule_references={observed_structural_rule_references}")
    print(f"operator_references={observed_operator_references}")
    print(f"registry_resource_references={observed_registry_resource_references}")
    print(f"stage_receipts={observed_stage_receipts}")
    print(f"registry_resource_kinds={observed_registry_resource_kinds}")
    ...
    print("canonical_record_types=9")
    print("custody_stages=8")
    print("exact_source_event_checksum_custody=1")
    print("exact_source_span_reconstruction=1")
    print("slice36_structural_operator_phase_scope_attachment_reference_custody=1")
    print("slice37_concept_sense_snapshot_custody=1")
    print("slice38_action_predicate_role_frame_effect_capability_snapshot_custody=1")
    print("cross_lineage_merge_allowed=0")
    print("generated_substitute_ancestry_allowed=0")
