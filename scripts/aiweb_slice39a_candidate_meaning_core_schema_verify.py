#!/usr/bin/env python3
"""Visible sequential verifier for AI.Web Slice 39A."""
...
EXPECTED_HEAD = "bb22f0fff6b64deaeeae8285dfabdbdd586d8473"
EXPECTED_TREE = "12131cc607c1dd293b3e741443d42ad69ba83063"
EXPECTED_SUBJECT = (
    "Slice 38H disabled bootstrap integration and Slice 38 closeout"
)
EXPECTED_COMMITTED_SUBJECT = "Slice 39A candidate meaning core schema"
EXPECTED_PREDECESSOR_COUNT = 361
EXPECTED_PAYLOAD_COUNT = 10
EXPECTED_BEHAVIOR_CHECKS = 1507
EXPECTED_MALFORMED_CASES = 768
...
PAYLOAD_PATHS = (
    "aiweb_language_core_bootstrap/candidate_meaning_construction/__init__.py",
    "aiweb_language_core_bootstrap/candidate_meaning_construction/authority.py",
    "aiweb_language_core_bootstrap/candidate_meaning_construction/identity.py",
    "aiweb_language_core_bootstrap/candidate_meaning_construction/schema.py",
    "scripts/AIWEB_SLICE39A_CANDIDATE_MEANING_CORE_SCHEMA_RUNTIME_SPEC.md",
    "scripts/AIWEB_SLICE39A_PROTECTED_PREDECESSOR_SHA256SUMS.txt",
    "scripts/AIWEB_SLICE39A_SCHEMA_ONLY_AND_DEFERRED_SCOPE_DECISION.md",
    "scripts/README_aiweb_slice39a_candidate_meaning_core_schema.md",
    "scripts/aiweb_slice39a_candidate_meaning_core_schema_verify.py",
...
def static_source_checks(
    ...
    verification.check(len(files) == 4, "schema package file count")
    verification.check(
    ...
    for path in files:
        ...
        verification.check(
            ...
            f"standard-library import boundary {path.name}",
        )
        if path.name in {"authority.py", "identity.py", "schema.py"}:
            verification.check(not functions, f"schema-only function boundary {path.name}")
        for token in PROHIBITED_TOKENS:
            verification.check(
                token not in text,
                f"no runtime wire token {path.name} {token}",
            )
...
def stream_source_only_inherited_test(
    ...
    *,
    ...
    """Run a predecessor closeout behavior test without stale live-Git claims.

    Slice 38H's behavior test contains pre-commit Git-containment assertions
    that are valid only during the Slice 38H application window.  After Slice
    38H is committed, its runtime behavior remains inherited, but those
    historical pre-commit assertions must not be reinterpreted against the
    newer Slice 39A working tree.

    A temporary source-only view preserves the exact current tracked and
    untracked source bytes through directory symlinks while deliberately
    omitting ``.git``.  The predecessor test therefore executes its explicit
    source-only branch.  Slice 39A separately proves the live Git state,
    protected predecessor hashes, and exact untracked payload containment.
    """
    ...
    for root_name in required_roots:
        ...
        if not source_root.is_dir() or source_root.is_symlink():
            return (
                1,
                f"FAIL: unsafe or missing source-only root: {source_root}\n",
            )

    with tempfile.TemporaryDirectory(
        prefix="aiweb_slice39a_inherited_source_",
    ) as temporary:
        ...
        try:
            ...
            return (
                1,
                f"FAIL: source-only inherited view creation: {error}\n",
            )
...
def main() -> int:
    ...
    print("AI.WEB SLICE 39A VISIBLE INDEPENDENT VERIFIER")
    ...
    protected_path = (
        repository
        / "scripts"
        / "AIWEB_SLICE39A_PROTECTED_PREDECESSOR_SHA256SUMS.txt"
    )
    ...
    for index, relative in enumerate(tests, 1):
        ...
        if relative == (
            ...
            verification.check(
                ...
                "malformed constructor case count",
            )
    ...
    print()
    print("=== SLICE 39A VERIFIER SUMMARY ===")
    ...
    if verification.failures or test_failures:
        print("SLICE 39A VISIBLE INDEPENDENT VERIFIER: FAIL")
        return 1

    print("SLICE 39A VISIBLE INDEPENDENT VERIFIER: PASS")
    ...
    print(f"slice39a_files={EXPECTED_PAYLOAD_COUNT}")
    print(f"behavior_checks={observed_behavior_checks}")
    print(f"malformed_constructor_cases={observed_malformed_cases}")
