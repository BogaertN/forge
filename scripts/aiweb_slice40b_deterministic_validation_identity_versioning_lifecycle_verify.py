#!/usr/bin/env python3
"""Visible sequential verifier for AI.Web Slice 40B."""
...
EXPECTED_HEAD = "09a0d20c91994b72edcd63c15780592d56394225"
EXPECTED_TREE = "435de181f15b94824d1204a45d3f4c7d7f244f7b"
EXPECTED_SUBJECT = "Slice 40A verbal cognition gate core schema"
EXPECTED_COMMITTED_SUBJECT = (
    "Slice 40B deterministic gate validation identity versioning lifecycle"
)
EXPECTED_PROTECTED_COUNT = 491
EXPECTED_PAYLOAD_COUNT = 14
EXPECTED_BEHAVIOR_CHECKS = 492
EXPECTED_MALFORMED_CASES = 187
CURRENT_TEST = "scripts/test_aiweb_slice40b_deterministic_validation_identity_versioning_lifecycle.py"
PARENT_CONTEXT_TEST = "scripts/test_aiweb_slice40a_verbal_cognition_gate_core_schema.py"
...
def static_source_checks(repository: Path, verification: Verification) -> None:
    ...
    verification.check(
        ...
        "exact Slice 40B governed-lifecycle files",
    )
    ...
    for path in files:
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.Import):
                ...
                verification.check(
                    ...
                    f"no gate evaluator function {path.name}:{node.name}",
                )
        ...
        verification.check(
            "meaning_structure_manifest import" not in text,
...
def main() -> int:
    ...
    print("AI.WEB SLICE 40B VISIBLE INDEPENDENT VERIFIER")
    ...
    protected_path = repository / "scripts" / "AIWEB_SLICE40B_PROTECTED_PREDECESSOR_SHA256SUMS.txt"
    try:
    ...
    exact_path_file = repository / "scripts" / "AIWEB_SLICE40B_EXACT_PAYLOAD_PATHS.txt"
    ...
    for index, relative in enumerate(tests, 1):
        ...
        if relative == PARENT_CONTEXT_TEST:
            parent_context_pass = "AI.WEB SLICE 40A BEHAVIOR TEST: PASS" in output
    ...
    if args.mode != "source":
        verification.check(parent_context_pass, "accepted Slice 40A context pass marker")

    required_markers = (
        "AI.WEB SLICE 40B BEHAVIOR TEST: PASS",
        "record_types=13",
        "gate_family_count=4",
        "lifecycle_stage_count=10",
        "lifecycle_transition_rules=29",
        "supported_gate_versions=1",
        "supported_gate_profile_versions=1",
        "canonical_serialization=1",
        "deterministic_identifiers=1",
        "schema_version_custody=1",
        "gate_profile_version_custody=1",
        "immutable_successor_records=1",
        "explicit_lifecycle_transitions=1",
        "unknown_version_rejection=1",
        "malformed_record_rejection=1",
        "exact_provenance_validation=1",
        "valid_record_is_valid_candidate_meaning=0",
        "valid_record_is_successful_gate_result=0",
        "lifecycle_is_selected_meaning_progression=0",
    ...
    print()
    print("=== SLICE 40B VERIFIER SUMMARY ===")
    ...
    if verification.failures or test_failures:
        print("SLICE 40B VISIBLE INDEPENDENT VERIFIER: FAIL")
        return 1
    print("SLICE 40B VISIBLE INDEPENDENT VERIFIER: PASS")
    print("protected_predecessor_files=491")
    print(f"inherited_tests={len(inherited)}")
    print(f"visible_total_tests={len(tests)}")
    print(f"slice40b_files={EXPECTED_PAYLOAD_COUNT}")
    print(f"behavior_checks={observed_checks}")
    print(f"malformed_validation_cases={observed_malformed}")
    print("record_types=13")
    print("gate_family_count=4")
    print("lifecycle_stage_count=10")
    print("lifecycle_transition_rules=29")
