#!/usr/bin/env python3
"""Visible independent verifier for AI.Web Slice 39F."""
...
EXPECTED_HEAD = "a79b4b3c557520564436e94f90e792d6540a7c04"
EXPECTED_TREE = "929c7f917ac0162fedacd19852aae5bc8c1acd48"
EXPECTED_SUBJECT = "Slice 39B-E roadmap continuity correction"
EXPECTED_COMMITTED_SUBJECT = "Slice 39F deterministic candidate meaning constructor"
EXPECTED_PROTECTED_COUNT = 439
EXPECTED_BEHAVIOR_CHECKS = 878
EXPECTED_MALFORMED_CASES = 16
EXPECTED_REJECTIONS = 16

PACKAGE_PATHS = (
    "aiweb_language_core_bootstrap/candidate_meaning_construction/deterministic_constructor/__init__.py",
    "aiweb_language_core_bootstrap/candidate_meaning_construction/deterministic_constructor/authority.py",
    "aiweb_language_core_bootstrap/candidate_meaning_construction/deterministic_constructor/canonical.py",
    "aiweb_language_core_bootstrap/candidate_meaning_construction/deterministic_constructor/constructor.py",
    "aiweb_language_core_bootstrap/candidate_meaning_construction/deterministic_constructor/identity.py",
    "aiweb_language_core_bootstrap/candidate_meaning_construction/deterministic_constructor/schema.py",
    "aiweb_language_core_bootstrap/candidate_meaning_construction/deterministic_constructor/validation.py",
)
SCRIPT_PATHS = (
    "scripts/AIWEB_SLICE39F_AUTHORITY_AND_DEFERRED_SCOPE_DECISION.md",
    "scripts/AIWEB_SLICE39F_DETERMINISTIC_CANDIDATE_MEANING_CONSTRUCTOR_RUNTIME_SPEC.md",
    "scripts/AIWEB_SLICE39F_PROTECTED_PREDECESSOR_SHA256SUMS.txt",
    "scripts/README_aiweb_slice39f_deterministic_candidate_meaning_constructor.md",
    "scripts/aiweb_slice39f_deterministic_candidate_meaning_constructor_verify.py",
...
CURRENT_TEST = "scripts/test_aiweb_slice39f_deterministic_candidate_meaning_constructor.py"
...
PROHIBITED_IMPORT_ROOTS = {
    "requests", "numpy", "torch", "tensorflow", "transformers", "sentence_transformers",
...
def verify_git_state(repository: Path, mode: str, verification: Verification) -> None:
    ...
    verification.check(all(item.returncode == 0 for item in (head, tree, subject, staged, tracked, untracked)), "Git inspection")
    if mode == "applied":
...
def static_source_checks(repository: Path, verification: Verification) -> None:
    package = repository / "aiweb_language_core_bootstrap" / "candidate_meaning_construction" / "deterministic_constructor"
    files = tuple(sorted(package.glob("*.py")))
    verification.check(tuple(path.name for path in files) == ("__init__.py", "authority.py", "canonical.py", "constructor.py", "identity.py", "schema.py", "validation.py"), "exact Slice 39F package files")
    for path in files:
        try:
            text = path.read_text(encoding="utf-8")
            tree = ast.parse(text, filename=str(path))
        except Exception as error:
            verification.check(False, f"parse {path.name}: {error}")
            continue
        roots: set[str] = set()
        for node in ast.walk(tree):
            if isinstance(node, ast.Import):
                ...
                if isinstance(node.func, ast.Name):
                    verification.check(node.func.id not in PROHIBITED_DIRECT_NAMES, f"no direct side-effect call {path.name}:{node.func.id}")
                elif isinstance(node.func, ast.Attribute):
                    verification.check(node.func.attr not in PROHIBITED_DIRECT_NAMES, f"no attribute side-effect call {path.name}:{node.func.attr}")
        ...
        for token in PROHIBITED_SOURCE_TOKENS:
            verification.check(token not in text, f"no raw-input constructor token {path.name}:{token}")
...
def stream_source_only_inherited_test(repository: Path, relative: str, *, env: dict[str, str], verification: Verification) -> tuple[int, str]:
    with tempfile.TemporaryDirectory(prefix="aiweb_slice39f_inherited_") as temporary:
...
def main() -> int:
    ...
    print("AI.WEB SLICE 39F VISIBLE INDEPENDENT VERIFIER")
    ...
    protected_path = repository / "scripts" / "AIWEB_SLICE39F_PROTECTED_PREDECESSOR_SHA256SUMS.txt"
    try:
    ...
    for index, relative in enumerate(tests, 1):
        ...
        if relative == CURRENT_TEST:
            ...
            verification.check("AI.WEB SLICE 39F BEHAVIOR TEST: PASS" in output, "Slice 39F pass marker")
            for marker in (
                "constructor_inputs=accepted_typed_slice37_slice38_records_only",
                "zero_candidate_states=0",
                "typed_zero_predecessor_inputs=1",
                "one_candidate_states=1",
                "multiple_candidate_states=2",
                "raw_text_inspection=0",
                "similarity_nearest_known_hidden_repair=0",
                "gate_outcome_selected_meaning=0",
                "manifest_bootstrap_closeout=0",
            ):
                verification.check(marker in output, f"Slice 39F marker {marker}")

    print()
    print("=== SLICE 39F VERIFIER SUMMARY ===")
    ...
    if verification.failures or test_failures:
        print("SLICE 39F VISIBLE INDEPENDENT VERIFIER: FAIL")
        return 1

    print("SLICE 39F VISIBLE INDEPENDENT VERIFIER: PASS")
    ...
    print("zero_one_many_candidate_states_verified=1")
    print("deterministic_construction_receipts_verified=1")
