#!/usr/bin/env python3
"""Visible independent verifier for Slice 43B deterministic RMC Echo governance.

The verifier protects every accepted predecessor path, verifies the exact
additive Slice 43B payload and runtime source contract, runs the current Slice
43B behavior test visibly, and runs the accepted Slice 43A verifier visibly
from an exact isolated checkout of the accepted Slice 43A commit.
"""
...
EXPECTED_PARENT_HEAD = "32719319e3df8dcde42f3ececcb14863d2c541b8"
EXPECTED_PARENT_TREE = "d84b9a8e9f612d1ed461bb3785aef52d2acabbef"
...
EXACT_PAYLOAD_MANIFEST = Path(
    "scripts/AIWEB_SLICE43B_EXACT_PAYLOAD_PATHS.txt"
)
PROTECTED_PREDECESSOR_MANIFEST = Path(
    "scripts/AIWEB_SLICE43B_PROTECTED_PREDECESSOR_SHA256SUMS.txt"
)
BEHAVIOR_TEST = Path(
    "scripts/test_aiweb_slice43b_deterministic_validation_identity_versioning_lifecycle.py"
)
INHERITED_VERIFIER = Path(
    "scripts/aiweb_slice43a_rmc_echo_core_schema_and_authority_boundary_verify.py"
)
EXPECTED_PREDECESSOR_COUNT = 1665
...
def verify_source_contract(repository: Path, ledger: Ledger) -> None:
    ...
    ledger.check(
        ...
        "governed lifecycle package has no nested runtime directory",
    )
    ...
    for name in EXACT_PACKAGE_FILES:
        ...
        for token in prohibited_tokens:
            ledger.check(
                token not in source,
                f"no prohibited token {name}:{token}",
            )
    ledger.check(
        required_definitions.issubset(found_definitions),
        "all required governed lifecycle definitions",
    )
...
def run_inherited(
    ...
    try:
        with tempfile.TemporaryDirectory(
            prefix="aiweb_slice43b_slice43a_"
        ) as temporary:
            checkout = Path(temporary) / "slice43a"
...
def main() -> int:
    ...
    print("=== AI.WEB SLICE 43B INDEPENDENT VERIFIER ===")
    print(f"repository={repository}")
    print(f"mode={args.mode}")

    ledger.check(repository.is_dir(), "repository exists")
    if not repository.is_dir():
        print("AI.WEB SLICE 43B VERIFIER: FAIL")
        return 1
    ...
    tracked_new = git(
        ...
        "cat-file",
        "-e",
        "HEAD:aiweb_language_core_bootstrap/rmc_echo_runtime/governed_lifecycle/schema.py",
    )
    ledger.check(
        (tracked_new.returncode != 0) if args.mode == "applied" else True,
        "package additive at parent",
    )
    ...
    with tempfile.TemporaryDirectory(
        ...
        ledger.check(
            "AI.WEB SLICE 43B BEHAVIOR TEST: PASS" in behavior_output,
            "current behavior verdict",
        )
        print("\n=== INHERITED SLICE 43A VERIFIER ===")
        inherited_rc, inherited_output = run_inherited(repository, env)
        ledger.check(inherited_rc == 0, "inherited Slice 43A verifier")
        ledger.check(
            "SLICE 43A VERIFIER: PASS" in inherited_output,
            "inherited Slice 43A verdict",
        )
    ...
    print(f"slice43b_files={len(expected_paths)}")
    ...
    print("duplicate_and_collision_rejection=1")
    print("lifecycle_transition_law=1")
    print("malformed_record_rejection=1")
    print("cross_record_consistency_checks=1")
    print("slice43c_source_admission=0")
    ...
    print(f"AI.WEB SLICE 43B VERIFIER: {verdict}")
