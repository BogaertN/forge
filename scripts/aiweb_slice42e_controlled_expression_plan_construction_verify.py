#!/usr/bin/env python3
"""Visible independent verifier for AI.Web Slice 42E."""
...
EXPECTED_HEAD = "64bd7a1fbb4e8d374d5a49808de60e69ba4adc25"
EXPECTED_TREE = "6cb77aab753d2d6164ee07372bab6d09d47acf89"
EXPECTED_SUBJECT = "Slice 42D preservation obligation projection"
COMMIT_SUBJECT = "Slice 42E controlled expression plan construction"

SCRIPT_DIR = Path(__file__).resolve().parent
PAYLOAD_MANIFEST = SCRIPT_DIR / "AIWEB_SLICE42E_EXACT_PAYLOAD_PATHS.txt"
PROTECTED_MANIFEST = SCRIPT_DIR / "AIWEB_SLICE42E_PROTECTED_PREDECESSOR_SHA256SUMS.txt"
...
def main() -> int:
    ...
    check(protected_count == 1593, "protected predecessor count")
    ...
    for relative in payload:
        ...
        if not path.is_file():
            continue
        expected_mode = 0o755 if relative.suffix == ".py" and relative.name.startswith(("test_", "aiweb_slice42e_")) else 0o644
        check(stat.S_IMODE(path.stat().st_mode) == expected_mode, "payload mode " + relative.as_posix())

    runtime = repo / "aiweb_language_core_bootstrap/outward_expression_runtime/expression_plan_construction"
    ...
    test_path = repo / "scripts/test_aiweb_slice42e_controlled_expression_plan_construction.py"
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="aiweb42e_parent_") as temp_dir:
            ...
            if clone_result.returncode == 0:
                ...
                inherited_path = clone / "scripts/aiweb_slice42d_preservation_obligation_projection_verify.py"
                inherited = run([sys.executable, "-B", str(inherited_path), str(clone), "--mode", "committed"], cwd=clone, env=env)
                print("=== INHERITED VISIBLE VERIFIER: SLICE 42D ===")
                print(inherited.stdout, end="")
                print(inherited.stderr, end="", file=sys.stderr)
                check(inherited.returncode == 0, "inherited Slice 42D verifier")
    finally:
        os.umask(prior_umask)

    print("=== SLICE 42E VERIFIER SUMMARY ===")
    ...
    print("slice42e_files=" + str(len(payload)))
    print("visible_total_tests=63")
    print("exact_slice42d_state_required=1")
    print("separate_plan_authority_required=1")
    print("deterministic_expression_plan_constructed=1")
    print("blocked_refusal_unresolved_containment_preserved=1")
    print("all_slice42d_obligations_preserved=1")
    print("structural_ordering_determined=1")
    print("modifiers_qualifications_caveats_refusal_preserved=1")
    print("lower_order_override=0")
    print("selected_meaning_ancestry_preserved=1")
    print("governed_outward_meaning_created=0")
    print("human_readable_text_or_expression_candidate=0")
