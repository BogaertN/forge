#!/usr/bin/env python3
"""
forge/scripts/patch278_verify.py

Patch 278 verifier — MEA Replay Engine.
Verifies the additive read-only operator registry and replay gate without
...
PATCH = "Patch 278 — MEA Replay Engine"
...
REQUIRED_FILES = [
    ...
    "scripts/patch278_verify.py",
    "scripts/test_patch278_mea_replay_engine.py",
    "scripts/README_278.md",
    "SHA256SUMS.txt",
]
...
def main() -> int:
    ...
    r = Results()
    print("PATCH 278 VERIFIER — MEA Replay Engine")
    ...
    try:
        ...
        registry_summary = mea.operator_registry_summary()
        ...
        candidate = mea.apply_registered_operator(fixture, "hypothesize", theta)
        expected = mea.canonical_hash(candidate)
        replay_ok = mea.replay_candidate(fixture, "hypothesize", theta, expected_candidate_hash=expected)
        mutated = dict(theta)
        mutated["hypothesis_text"] = "144 Hz is already empirically verified in myelin."
        replay_bad = mea.replay_candidate(fixture, "hypothesize", mutated, expected_candidate_hash=expected)
        kernel_probe = mea.kernel_foundation_probe()
        r.check("mea_import_patch278_exports", True)
        r.check("replay_law_formula", mea.REPLAY_LAW_FORMULA == "Replay(H(M_t), O_k, theta_k) = H(c_i)")
        r.check("registry_operator_count", registry_summary.get("operator_count", 0) >= 14, str(registry_summary.get("operator_count")))
        r.check("registry_replayable_count", registry_summary.get("replayable_operator_count", 0) >= 12, str(registry_summary.get("replayable_operator_count")))
        r.check("registry_has_external_search_cost_only", "external_search" in registry_summary.get("operator_ids", []))
        r.check("noop_replay_confirmed", noop.replay_confirmed is True)
        r.check("hypothesis_replay_confirmed", replay_ok.replay_confirmed is True)
        r.check("mutated_theta_mismatch_blocks_replay", replay_bad.tamper_detected is True and replay_bad.replay_confirmed is False)
        r.check("kernel_stage_patch278", kernel_probe["identity"]["kernel_stage"] == "replay_foundation_read_only_patch278")
        r.check("kernel_operator_registry_visible", kernel_probe["identity"].get("operator_registry_visible") is True)
        r.check("kernel_replay_engine_visible", kernel_probe["identity"].get("replay_engine_visible") is True)
        r.check("kernel_sealing_inactive", kernel_probe["identity"].get("sealing_active") is False)
    except Exception as exc:
        r.check("mea_import_and_patch278_replay", False, repr(exc)[:240])
    ...
    r.check("foundation_status_reports_patch278", PATCH in main_text)
    r.check("endpoint_reports_operator_registry", "operator_registry" in main_text)
    r.check("endpoint_reports_replay_engine", "replay_engine" in main_text)
    r.check("endpoint_reports_replay_law", "replay_law_formula" in main_text)
    ...
    behavior_script = rel("scripts/test_patch278_mea_replay_engine.py")
    ...
    print(f"RESULT: PATCH_278_VERIFY {'PASS' if r.failed == 0 else 'FAIL'}  Total:{r.passed + r.failed} Passed:{r.passed} Failed:{r.failed}")
