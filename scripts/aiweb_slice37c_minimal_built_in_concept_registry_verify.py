#!/usr/bin/env python3
"""Independent verifier for AI.Web Slice 37C."""
...
EXPECTED_PARENT_HEAD = 'f4cfdd0e3f20c5ff3d72ed335b6ddd155b0f36fd'
EXPECTED_PARENT_SUBJECT = 'Slice 37B deterministic validation identity and lifecycle law'
EXPECTED_COMMIT_SUBJECT = 'Slice 37C minimal built-in concept registry'
EXACT_PATHS = ('aiweb_language_core_bootstrap/controlled_concept_sense_registry/built_in_registry/__init__.py',
 'aiweb_language_core_bootstrap/controlled_concept_sense_registry/built_in_registry/authority.py',
 'aiweb_language_core_bootstrap/controlled_concept_sense_registry/built_in_registry/identity.py',
 'aiweb_language_core_bootstrap/controlled_concept_sense_registry/built_in_registry/records.py',
 'aiweb_language_core_bootstrap/controlled_concept_sense_registry/built_in_registry/registry.py',
 'aiweb_language_core_bootstrap/controlled_concept_sense_registry/built_in_registry/schema.py',
 'aiweb_language_core_bootstrap/controlled_concept_sense_registry/built_in_registry/validation.py',
 'scripts/AIWEB_SLICE37C_SOURCE_INSPECTION_RECORD.md',
 'scripts/AIWEB_SLICE37C_MINIMAL_BUILT_IN_CONCEPT_REGISTRY_RUNTIME_SPEC.md',
 'scripts/AIWEB_SLICE37C_CONCEPT_SELECTION_AND_DEFERRED_SCOPE_DECISION.md',
 'scripts/README_aiweb_slice37c_minimal_built_in_concept_registry.md',
 'scripts/test_aiweb_slice37c_minimal_built_in_concept_registry.py',
 'scripts/aiweb_slice37c_minimal_built_in_concept_registry_verify.py')
...
PROHIBITED_CALL_NAMES = (
    "open", "exec", "eval", "compile", "__import__", "os.system",
    "os.popen", "subprocess.run", "subprocess.Popen", "Path.open",
    "Path.read_text", "Path.read_bytes", "Path.write_text", "Path.write_bytes",
)
ALLOWED_TOP_LEVEL_CALLS = (
    "BuiltInConceptDefinition", "tuple", "_provenance", "_namespace",
    "_concept_history", "zip", "_authority", "_transition",
    "_concept_authorities_and_transitions", "with_expected_batch_id",
    "ConceptGovernanceBatch", "with_expected_manifest_id",
    "BuiltInConceptRegistryManifest", "expected_resource_lineage_id",
    "BuiltInConceptRegistry",
)
REGISTRY_PUBLIC_FUNCTIONS = (
    "all_admitted_concepts", "built_in_registry", "concept_by_id",
    "concept_by_key", "contains_concept_id", "current_namespace",
    "registry_manifest",
)
...
def main() -> int:
    ...
    parser.add_argument(
        "--mode", required=True,
        choices=("precommit", "committed", "source_only"),
    )
    ...
    print("AI.WEB SLICE 37C INDEPENDENT VERIFIER")
    print(f"mode={args.mode}")

    package_root = (
        repo / "aiweb_language_core_bootstrap" /
        "controlled_concept_sense_registry" / "built_in_registry"
    )
    for relative in EXACT_PATHS:
        path = repo / relative
        check(path.is_file() and not path.is_symlink(), f"missing or unsafe: {relative}")

    predecessor_mismatches = verify_hashes(repo, PROTECTED_HASHES)
    if args.mode == "source_only":
        missing = tuple(p for p in predecessor_mismatches if not (repo / p).exists())
        changed = tuple(p for p in predecessor_mismatches if (repo / p).exists())
        check(not changed, "all available predecessor hashes")
        print(f"source_only_missing_protected_files={len(missing)}")
    else:
        check(not predecessor_mismatches, "all 189 predecessor hashes")

    source_files = sorted(package_root.glob("*.py"))
    check(len(source_files) == 7, "exact seven package source files")
    prohibited_imports = []
    prohibited_calls = []
    unexpected_top_level_calls = []
    route_decorators = []
    ...
    for path in source_files:
        ...
        for node in tree.body:
            ...
            for value in values:
                for candidate in ast.walk(value):
                    if isinstance(candidate, ast.Call):
                        top_level_call_ids.add(id(candidate))
            ...
            if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
                for decorator in node.decorator_list:
                    ...
                    if any(token in name.lower() for token in ("route", "get", "post", "put", "patch", "delete", "websocket")):
        ...
        for node in ast.walk(tree):
            ...
            if id(node) in top_level_call_ids and name not in ALLOWED_TOP_LEVEL_CALLS:
                unexpected_top_level_calls.append(f"{path.name}:{node.lineno}:{name}")

    check(not prohibited_imports, "no prohibited imports")
    check(not prohibited_calls, "no prohibited side-effect calls")
    check(not unexpected_top_level_calls, "only approved deterministic top-level constructors")
    check(not route_decorators, "no route decorators")

    registry_tree = ast.parse((package_root / "registry.py").read_text(encoding="utf-8"))
    public_functions = tuple(sorted(
        node.name for node in registry_tree.body
        if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef))
        and not node.name.startswith("_")
    ))
    check(public_functions == REGISTRY_PUBLIC_FUNCTIONS, "exact read-only registry function surface")
    for forbidden in ("surface", "lexical", "normalize", "fuzzy", "similar", "alias", "occurrence", "select", "map_term", "populate"):
        check(not any(forbidden in name.lower() for name in public_functions), f"no {forbidden} registry function")

    parent_init = repo / "aiweb_language_core_bootstrap/controlled_concept_sense_registry/__init__.py"
    check(
        parent_init.is_file() and "built_in_registry" not in parent_init.read_text(encoding="utf-8"),
        "parent package does not auto-import Slice 37C",
    )

    env = os.environ.copy()
    env["PYTHONDONTWRITEBYTECODE"] = "1"
    env["PYTHONPYCACHEPREFIX"] = f"/tmp/aiweb_slice37c_verify_cache_{os.getpid()}"
    behavior = run(
        (sys.executable, "-B", "scripts/test_aiweb_slice37c_minimal_built_in_concept_registry.py"),
        cwd=repo, env=env,
    )
    if behavior.returncode != 0:
        print("--- Slice 37C behavior test stdout ---")
        print(behavior.stdout)
        print("--- Slice 37C behavior test stderr ---")
        print(behavior.stderr)
    check(behavior.returncode == 0, "Slice 37C behavior test")
    ...
    if args.mode != "source_only":
        ...
        if args.mode == "precommit":
            ...
            check(untracked_state or staged_state, "exact precommit thirteen-file state")
            ...
            check(changed.returncode == 0 and actual_changed == tuple(sorted(EXACT_PATHS)), "exact committed thirteen-file set")
        ...
        for relative in INHERITED_COMMANDS:
            ...
            if result.returncode != 0:
                ...
                print(result.stdout)
                print(result.stderr)
        check(not inherited_failures, "all 22 inherited behavior tests")
    ...
    for base in (repo / "aiweb_language_core_bootstrap", repo / "scripts"):
        ...
        for path in base.rglob("*"):
            ...
            if "__pycache__" in rel.parts or path.suffix in {".pyc", ".pyo"}:
                cache_artifacts.append(rel.as_posix())
    ...
    if failures:
        ...
        print("SLICE 37C INDEPENDENT VERIFIER: FAIL")
        return 1

    print("SLICE 37C INDEPENDENT VERIFIER: PASS")
    print("protected_predecessor_files=189")
    print("inherited_tests=22")
    print("slice37c_files=13")
    print("concept_count=4")
    print("namespace_count=1")
    print("provenance_records=5")
    print("governed_resource_versions=15")
    print("lifecycle_transitions=10")
    print("exact_identity_lookup_functions=2")
    print("surface_lookup_mapping_sense_class_relation_integration=0")
    print("external_resources_loaded=0")
    print("routes_tools_memory_actions_rendering_delivery=0")
    return 0
