#!/usr/bin/env python3
"""Visible sequential verifier for AI.Web Slice 40E."""
...
EXPECTED_HEAD = "b9b5e835e7506bc2b7849d3221b0328227add7fd"
EXPECTED_TREE = "cd26ca5243fe76c0a7a12e2ee53e471538796eee"
EXPECTED_SUBJECT = "Slice 40D deterministic congruity gate runtime"
EXPECTED_COMMITTED_SUBJECT = "Slice 40E deterministic connectedness gate runtime"
EXPECTED_PROTECTED_COUNT = 531
EXPECTED_PAYLOAD_COUNT = 13
EXPECTED_BEHAVIOR_CHECKS = 169
EXPECTED_MALFORMED_CASES = 88
CURRENT_TEST = "scripts/test_aiweb_slice40e_connectedness_gate_runtime.py"
PARENT_CONTEXT_TEST = "scripts/test_aiweb_slice40d_congruity_gate_runtime.py"
...
PROHIBITED_EVALUATOR_FUNCTIONS = {
    "evaluate_recoverable_purpose", "evaluate_intended_purport",
    "compose_gate_outcomes", "select_meaning",
}
...
def static_source_checks(repository: Path, verification: Verification) -> None:
    ...
    package = parent / "connectedness_gate"
    files = tuple(sorted(package.glob("*.py")))
    verification.check(
        tuple(path.name for path in files)
        == ("__init__.py", "canonical.py", "evaluator.py", "identity.py", "schema.py", "validation.py"),
        "exact Slice 40E connectedness files",
    )
    ...
    for path in files:
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.Import):
                ...
                if node.name == "evaluate_connectedness":
    ...
    verification.check(observed_evaluator, "connectedness evaluator installed")
...
def main() -> int:
    ...
    print("AI.WEB SLICE 40E VISIBLE INDEPENDENT VERIFIER")
    ...
    protected_path = repository / "scripts" / "AIWEB_SLICE40E_PROTECTED_PREDECESSOR_SHA256SUMS.txt"
    try:
    ...
    exact_path_file = repository / "scripts" / "AIWEB_SLICE40E_EXACT_PAYLOAD_PATHS.txt"
    ...
    for index, relative in enumerate(tests, 1):
        ...
        if relative == PARENT_CONTEXT_TEST:
            parent_context_pass = "AI.WEB SLICE 40D CONGRUITY GATE BEHAVIOR TEST: PASS" in output
    ...
    if args.mode != "source":
        verification.check(parent_context_pass, "accepted Slice 40D context pass marker")

    required_markers = (
        "AI.WEB SLICE 40E CONNECTEDNESS GATE BEHAVIOR TEST: PASS",
        "assertion_kinds=7", "finding_kinds=7", "overall_states=6",
        "connectedness_evaluator_installed=1",
        "source_span_connectedness=1",
        "structural_ancestry_connectedness=1",
        "scope_connectedness=1",
        "attachment_connectedness=1",
        "operator_trail_connectedness=1",
        "predicate_frame_connectedness=1",
        "candidate_lineage_connectedness=1",
        "connected_result=1", "disconnected_result=1",
        "ambiguous_result=1", "unsupported_result=1",
        "conflicted_result=1", "indeterminate_result=1",
        "same_expression_only_connection=0",
        "same_manifest_only_connection=0",
        "implicit_transitive_connection=0",
        "source_gap_bridged=0", "ancestry_gap_bridged=0",
        "scope_rewritten=0", "attachment_reassigned=0",
        "operator_trail_rewritten=0", "predicate_frame_rewired=0",
        "candidate_lineage_merged=0", "candidate_structure_mutated=0",
        "similarity_fallback_used=0", "hidden_model_judgment_used=0",
    ...
    print()
    print("=== SLICE 40E VERIFIER SUMMARY ===")
    ...
    if verification.failures or test_failures:
        print("SLICE 40E VISIBLE INDEPENDENT VERIFIER: FAIL")
        return 1
    print("SLICE 40E VISIBLE INDEPENDENT VERIFIER: PASS")
    print("protected_predecessor_files=531")
    print(f"inherited_tests={len(inherited)}")
    print(f"visible_total_tests={len(tests)}")
    print(f"slice40e_files={EXPECTED_PAYLOAD_COUNT}")
    print(f"behavior_checks={observed_checks}")
    print(f"malformed_validation_cases={observed_malformed}")
    print("assertion_kinds=7")
