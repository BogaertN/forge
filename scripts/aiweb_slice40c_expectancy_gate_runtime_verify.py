#!/usr/bin/env python3
"""Visible sequential verifier for AI.Web Slice 40C."""
...
EXPECTED_HEAD = "5ad63716f4da2833a23758d083671a7ee92ae22a"
EXPECTED_TREE = "77ac09bc0c10460918537094ddd1eef106ca5287"
EXPECTED_SUBJECT = "Slice 40B deterministic gate validation identity versioning lifecycle"
EXPECTED_COMMITTED_SUBJECT = "Slice 40C deterministic expectancy gate runtime"
EXPECTED_PROTECTED_COUNT = 505
EXPECTED_PAYLOAD_COUNT = 13
EXPECTED_BEHAVIOR_CHECKS = 148
EXPECTED_MALFORMED_CASES = 56
CURRENT_TEST = "scripts/test_aiweb_slice40c_expectancy_gate_runtime.py"
PARENT_CONTEXT_TEST = "scripts/test_aiweb_slice40b_deterministic_validation_identity_versioning_lifecycle.py"
...
PROHIBITED_EVALUATOR_FUNCTIONS = {
    "evaluate_congruity", "evaluate_connectedness",
...
def static_source_checks(repository: Path, verification: Verification) -> None:
    ...
    parent_files = tuple(sorted(path.name for path in parent.glob("*.py")))
    verification.check(
    ...
    package = parent / "expectancy_gate"
    files = tuple(sorted(package.glob("*.py")))
    verification.check(
        tuple(path.name for path in files)
        == (
            "__init__.py", "canonical.py", "evaluator.py", "identity.py",
            "schema.py", "validation.py",
        ),
        "exact Slice 40C expectancy files",
    )
    ...
    for path in files:
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.Import):
                ...
                if node.name == "evaluate_expectancy":
                    observed_evaluator = True
                verification.check(
                    node.name not in PROHIBITED_EVALUATOR_FUNCTIONS,
                    f"no later gate or selection function {path.name}:{node.name}",
                )
        ...
        verification.check(
            "meaning_structure_manifest" not in text,
            f"MSM not imported {path.name}",
        )
    verification.check(observed_evaluator, "expectancy evaluator installed")
...
def main() -> int:
    ...
    print("AI.WEB SLICE 40C VISIBLE INDEPENDENT VERIFIER")
    ...
    protected_path = repository / "scripts" / "AIWEB_SLICE40C_PROTECTED_PREDECESSOR_SHA256SUMS.txt"
    try:
    ...
    exact_path_file = repository / "scripts" / "AIWEB_SLICE40C_EXACT_PAYLOAD_PATHS.txt"
    ...
    for index, relative in enumerate(tests, 1):
        ...
        if relative == PARENT_CONTEXT_TEST:
            parent_context_pass = "AI.WEB SLICE 40B BEHAVIOR TEST: PASS" in output
    ...
    if args.mode != "source":
        verification.check(parent_context_pass, "accepted Slice 40B context pass marker")

    required_markers = (
        "AI.WEB SLICE 40C EXPECTANCY GATE BEHAVIOR TEST: PASS",
        "requirement_kinds=5",
        "finding_kinds=7",
        "overall_states=3",
        "required_role_missing=1",
        "required_relation_missing=1",
        "required_complement_missing=1",
        "required_purpose_information_missing=1",
        "optional_detail_omitted=1",
        "structurally_complete_for_gate=1",
        "indeterminate_required_authority_absent=1",
        "candidate_structure_mutated=0",
        "missing_role_filled=0",
        "referent_invented=0",
        "unstated_participant_inferred=0",
    ...
    print()
    print("=== SLICE 40C VERIFIER SUMMARY ===")
    ...
    if verification.failures or test_failures:
        print("SLICE 40C VISIBLE INDEPENDENT VERIFIER: FAIL")
        return 1
    print("SLICE 40C VISIBLE INDEPENDENT VERIFIER: PASS")
    print("protected_predecessor_files=505")
    print(f"inherited_tests={len(inherited)}")
    print(f"visible_total_tests={len(tests)}")
    print(f"slice40c_files={EXPECTED_PAYLOAD_COUNT}")
    print(f"behavior_checks={observed_checks}")
    print(f"malformed_validation_cases={observed_malformed}")
    print("requirement_kinds=5")
    print("finding_kinds=7")
    print("overall_states=3")
    print("expectancy_evaluator_installed=1")
