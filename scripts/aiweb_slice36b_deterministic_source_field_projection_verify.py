#!/usr/bin/env python3
"""Independent verifier for Slice 36B deterministic source-field projection."""
...
EXPECTED_PARENT_HEAD = "e8432f293b199fdad913fdd53f801f37dbab6fdc"
EXPECTED_COMMIT_SUBJECT = "Slice 36B deterministic source-field projection"
EXACT_PATHS = ('aiweb_language_core_bootstrap/source_field_projection/__init__.py', 'aiweb_language_core_bootstrap/source_field_projection/schema.py', 'aiweb_language_core_bootstrap/source_field_projection/projection.py', 'aiweb_language_core_bootstrap/source_field_projection/reconstruction.py', 'aiweb_language_core_bootstrap/source_field_projection/validation.py', 'scripts/AIWEB_SLICE36B_DETERMINISTIC_SOURCE_FIELD_PROJECTION_RUNTIME_SPEC.md', 'scripts/AIWEB_SLICE36B_GRAPHEME_BOUNDARY_PROFILE_DECISION.md', 'scripts/README_aiweb_slice36b_deterministic_source_field_projection.md', 'scripts/test_aiweb_slice36b_deterministic_source_field_projection.py', 'scripts/aiweb_slice36b_deterministic_source_field_projection_verify.py')
...
EXPECTED_EXPORTS = ('ABSOLUTE_MAX_PROJECTION_CODE_POINTS', 'ABSOLUTE_MAX_PROJECTION_OBSERVATIONS', 'DEFAULT_MAX_PROJECTION_CODE_POINTS', 'DEFAULT_MAX_PROJECTION_OBSERVATIONS', 'GRAPHEME_PROFILE_ID', 'PROJECTION_RULESET_ID', 'PROJECTION_RULESET_VERSION', 'PROJECTION_SCHEMA_VERSION', 'PROJECTION_SPEC_ID', 'PROJECTION_SPEC_VERSION', 'SOURCE_FIELD_SCHEMA_ID', 'UNICODE_DATABASE_VERSION', 'GraphemeBoundaryStatus', 'GraphemeProfileStatus', 'SourceBoundaryRecord', 'SourceCodePointRecord', 'SourceFieldProjectionLimits', 'SourceFieldProjectionRecord', 'SourceFieldProjectionResult', 'SourceFieldProjectionStatus', 'SourceFieldReconstructionResult', 'SourceFieldSupportStatus', 'SourceObservationKind', 'SourceObservationRecord', 'build_source_field_projection_limits', 'default_source_field_projection_limits', 'project_source_field', 'reconstruct_source_field', 'validate_source_boundary_record', 'validate_source_code_point_record', 'validate_source_field_projection', 'validate_source_field_projection_limits', 'validate_source_field_projection_result', 'validate_source_field_reconstruction_result', 'validate_source_observation_record')
INHERITED_COMMANDS = (
...
ALLOWED_IMPORT_ROOTS = {
    ...
    "unicodedata", "aiweb_language_core_bootstrap"
}
PROHIBITED_IMPORT_ROOTS = {
    "rmc_engine_v1", "os", "pathlib", "socket", "subprocess", "urllib",
    "requests", "httpx", "aiohttp", "openai", "anthropic", "ollama",
    "torch", "tensorflow", "transformers", "langchain", "chromadb",
    "sqlite3", "importlib", "pkgutil"
}
PROHIBITED_CALLS = {
...
def run_py(repo: Path, script: str) -> subprocess.CompletedProcess[str]:
    env = {
        ...
        "PYTHONPYCACHEPREFIX": "/tmp/aiweb_slice36b_verifier_cache",
        "PATH": "/usr/bin:/bin",
    }
    return subprocess.run(
...
def main() -> int:
    ...
    for rel in EXACT_PATHS:
        if not (repo / rel).is_file():
            failures.append("Slice 36B file missing: " + rel)
    if not any("Slice 36B file missing" in item for item in failures):
        passes.append(f"Slice 36B exact files present={len(EXACT_PATHS)}")
    ...
    if "source_field_projection" in root_init:
        failures.append("bootstrap root auto-imports Slice 36B")
    else:
        passes.append("bootstrap root unchanged and does not auto-import Slice 36B")

    probe_code = (
        "import sys; "
        "before=tuple(sorted(x for x in sys.modules if x=='rmc_engine_v1' or x.startswith('rmc_engine_v1.'))); "
        "import aiweb_language_core_bootstrap.source_field_projection as m; "
    ...
    probe = subprocess.run(
        ...
        env={
            "PYTHONDONTWRITEBYTECODE": "1",
            "PYTHONPYCACHEPREFIX": "/tmp/aiweb_slice36b_import_cache",
            "PATH": "/usr/bin:/bin",
        },
        check=False,
    )
    ...
    for marker in (
        "UAX #29",
        "exact grapheme boundaries only under a closed ASCII profile",
        "marked `unavailable` rather than being guessed",
    ):
        if marker.lower() in decision.lower():
            passes.append("grapheme decision marker: " + marker)
        else:
            failures.append("grapheme decision marker missing: " + marker)

    command_set = REHEARSAL_COMMANDS if args.mode == "rehearsal" else INHERITED_COMMANDS
    for script_name in command_set + ("scripts/test_aiweb_slice36b_deterministic_source_field_projection.py",):
    ...
    print("AI.WEB SLICE 36B INDEPENDENT VERIFIER")
    ...
    for failure in failures:
        print("FAIL - " + failure)
    if failures:
        print("SLICE 36B INDEPENDENT VERIFIER: FAIL")
        return 1
    print("SLICE 36B INDEPENDENT VERIFIER: PASS")
    print(f"protected_predecessor_files={len(PROTECTED_HASHES)}")
    print(f"inherited_commands={len(command_set)}")
    print(f"slice36b_files={len(EXACT_PATHS)}")
    print("legacy_rmc_imports=0")
    print("operator_applications=0")
    print("token_concept_predicate_phase_meaning_effects=0")
    return 0
