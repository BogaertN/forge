#!/usr/bin/env python3
"""Independent verifier for Slice 36E candidate resonant phase trails."""
...
EXPECTED_PARENT_HEAD = "9a6540d0503e4292afa46aeebc9dbc6b1be9eaa7"
EXPECTED_COMMIT_SUBJECT = "Slice 36E candidate resonant phase-trail construction"
EXACT_PATHS = ('aiweb_language_core_bootstrap/candidate_resonant_phase_trail/__init__.py',
 'aiweb_language_core_bootstrap/candidate_resonant_phase_trail/schema.py',
 'aiweb_language_core_bootstrap/candidate_resonant_phase_trail/transition.py',
 'aiweb_language_core_bootstrap/candidate_resonant_phase_trail/construction.py',
 'aiweb_language_core_bootstrap/candidate_resonant_phase_trail/validation.py',
 'scripts/AIWEB_SLICE36E_CANDIDATE_RESONANT_PHASE_TRAIL_RUNTIME_SPEC.md',
 'scripts/AIWEB_SLICE36E_RSOC_CORE_APPLICATION_AND_PHASE_TRANSITION_BOUNDARY_DECISION.md',
 'scripts/README_aiweb_slice36e_candidate_resonant_phase_trail.md',
 'scripts/test_aiweb_slice36e_candidate_resonant_phase_trail.py',
 'scripts/aiweb_slice36e_candidate_resonant_phase_trail_verify.py')
...
OWN_TEST = "scripts/test_aiweb_slice36e_candidate_resonant_phase_trail.py"

ALLOWED_IMPORT_ROOTS = {
    "__future__", "collections", "dataclasses", "enum", "hashlib",
    "itertools", "typing", "aiweb_language_core_bootstrap"
}
PROHIBITED_IMPORT_ROOTS = {
    ...
    "chromadb", "sqlite3", "importlib", "pkgutil", "re", "random"
}
PROHIBITED_CALLS = {
...
def run_python(repository: Path, script: str) -> subprocess.CompletedProcess[str]:
    environment = {
        "PYTHONDONTWRITEBYTECODE": "1",
        "PYTHONPYCACHEPREFIX": "/tmp/aiweb_slice36e_verifier_cache",
        "PATH": "/usr/bin:/bin",
    }
    return subprocess.run(
...
def run_probe(repository: Path, code: str) -> subprocess.CompletedProcess[str]:
    environment = {
        "PYTHONDONTWRITEBYTECODE": "1",
        "PYTHONPYCACHEPREFIX": "/tmp/aiweb_slice36e_probe_cache",
        "PATH": "/usr/bin:/bin",
    }
    return subprocess.run(
...
def verify_runtime_source(repository: Path) -> tuple[str, ...]:
    failures: list[str] = []
    for relative in RUNTIME_FILES:
        path = repository / relative
        if not path.is_file():
            failures.append("runtime file missing:" + relative)
            continue
        try:
            ...
            failures.append(
                f"runtime syntax failure:{relative}:{type(error).__name__}"
            )
            continue
            ...
            if isinstance(node, ast.Import):
                for alias in node.names:
                    ...
                    if root in PROHIBITED_IMPORT_ROOTS or root not in ALLOWED_IMPORT_ROOTS:
                        failures.append(f"runtime import prohibited:{relative}:{alias.name}")
            elif isinstance(node, ast.ImportFrom) and node.level == 0:
                root = (node.module or "").split(".", 1)[0]
                if root in PROHIBITED_IMPORT_ROOTS or root not in ALLOWED_IMPORT_ROOTS:
                    failures.append(f"runtime import prohibited:{relative}:{node.module}")
            elif isinstance(node, ast.Call):
                name = call_name(node)
                if name in PROHIBITED_CALLS:
                    failures.append(f"runtime call prohibited:{relative}:{name}")
        lowered = source.lower()
        for token in (
            "llm_renderer", "chroma_connector", "embedding", "vector similarity",
            "nearest_neighbor", "web search", "requests.", "openai", "anthropic",
        ):
            if token in lowered:
                failures.append(f"runtime forbidden token:{relative}:{token}")
...
def main() -> int:
    ...
    elif arguments.mode != "rehearsal" and not (repository / ".git").is_dir():
        failures.append("git repository missing")

    if not failures and arguments.mode == "precommit":
        ...
        tracked = run_git(repository, "diff", "--name-only")
        ...
        if head.returncode == 0 and head.stdout.strip() == EXPECTED_PARENT_HEAD:
            passes.append("precommit base exact")
        else:
            failures.append("precommit base mismatch:" + head.stdout.strip())
        if status.returncode == 0 and set(status.stdout.splitlines()) == {
            f"?? {path}" for path in EXACT_PATHS
        }:
    ...
    if not failures and arguments.mode == "committed":
        branch = run_git(repository, "branch", "--show-current")
        ...
        if changed.returncode == 0 and set(changed.stdout.splitlines()) == set(EXACT_PATHS):
    ...
    for relative, expected_digest in PROTECTED_HASHES.items():
        path = repository / relative
        if not path.is_file() or path.is_symlink():
            failures.append("protected predecessor missing or unsafe:" + relative)
            protected_failures += 1
        elif sha256_file(path) != expected_digest:
            failures.append("protected predecessor hash mismatch:" + relative)
            protected_failures += 1
    if protected_failures == 0:
        passes.append(f"protected predecessor hashes preserved={len(PROTECTED_HASHES)}")

    missing_slice_files = [
        relative for relative in EXACT_PATHS
        if not (repository / relative).is_file() or (repository / relative).is_symlink()
    ]
    if missing_slice_files:
        failures.extend("Slice 36E file missing or unsafe:" + value for value in missing_slice_files)
    else:
        passes.append(f"Slice 36E exact files present={len(EXACT_PATHS)}")

    runtime_failures = verify_runtime_source(repository)
    ...
    if cache_paths:
        ...
        passes.append("project-controlled Python cache artifacts absent")
    ...
    probe_code = (
        ...
        "from aiweb_language_core_bootstrap.candidate_resonant_phase_trail import build_default_phase_trail_policy,construct_candidate_resonant_phase_trails,PhaseTrailConstructionStatus;"
        "p=build_default_phase_trail_policy();"
        "assert p.immutable_successor_required and not p.fixed_phase_sequence_required;"
        "assert not p.core_rsoc_operator_application_authorized;"
        "c=capture_input_event('Do not install it.',source_id='verify',channel_id='verify',sequence_number=1);"
        "s=project_source_field(c.event);b=bind_resonant_operator_candidates(s);r=construct_candidate_resonant_phase_trails(s,b);"
        "assert r.status is PhaseTrailConstructionStatus.MULTIPLE_PHASE_TRAILS;"
        "assert r.phase_trail_set is not None and r.phase_trail_set.trail_count==8;"
        "assert all(t.core_rsoc_operator_applications==0 and not t.selected_trail for t in r.phase_trail_set.trails);"
        "assert not r.selected_phase and not r.selected_meaning and not r.permission_inferred and not r.action_performed;"
        "assert not any(name.startswith('rmc_engine_v1') for name in sys.modules);"
        "print('PROBE_PASS')"
    )
    probe = run_probe(repository, probe_code)
    if probe.returncode == 0 and probe.stdout.strip() == "PROBE_PASS":
        passes.append("runtime contract and governing-example probe passed")
    else:
        failures.append(
            "runtime contract probe failed:"
            + (probe.stderr.strip() or probe.stdout.strip())
        )

    own_test = run_python(repository, OWN_TEST)
    if own_test.returncode == 0 and "SLICE 36E BEHAVIOR TEST: PASS" in own_test.stdout:
        passes.append("Slice 36E behavior test passed")
    else:
        failures.append(
            "Slice 36E behavior test failed:"
            + (own_test.stderr.strip() or own_test.stdout.strip())
        )

    inherited_count = 0
    if arguments.mode in {"precommit", "committed"}:
        for command in INHERITED_COMMANDS:
            path = repository / command
            if not path.is_file():
                failures.append("inherited command missing:" + command)
                continue
            result = run_python(repository, command)
            inherited_count += 1
        ...
        if inherited_count == len(INHERITED_COMMANDS) and not any(
            value.startswith("inherited command") for value in failures
        ):
    ...
    print("AI.WEB SLICE 36E INDEPENDENT VERIFIER")
    ...
    if failures:
        for value in failures:
            print("FAIL:", value)
        print("SLICE 36E INDEPENDENT VERIFIER: FAIL")
        return 1

    print("SLICE 36E INDEPENDENT VERIFIER: PASS")
    print(f"protected_predecessor_files={len(PROTECTED_HASHES)}")
    print(
        "inherited_commands="
        + str(inherited_count if arguments.mode != "rehearsal" else len(INHERITED_COMMANDS))
    )
    print(f"slice36e_files={len(EXACT_PATHS)}")
    print("governing_example_phase_trails=8")
    print("immutable_successor_law=1")
    print("fixed_phase_sequences_forced=0")
    print("arbitrary_neighbor_compositions=0")
    print("core_rsoc_operator_applications=0")
    print("selected_trails=0")
    print("selected_phases=0")
