"""Deterministic identity and version helpers for Slice 38E records."""
...
from .schema import PredicateFrameGovernedResource
...
def with_expected_id(record: T) -> T:
    field_name = next(
        (
            name
            for name in (
                "provenance_id", "frame_id", "constraint_id", "compatibility_id",
                "policy_id", "authority_id", "transition_id", "manifest_id",
                "namespace_id",
            )
    ...
    if field_name is None:
        raise TypeError("unsupported Slice 38E record type")
    return replace(record, **{field_name: record_id(record)})


def resource_lineage_body(record: PredicateFrameGovernedResource) -> dict[str, object]:
...
def expected_lineage_id(record: PredicateFrameGovernedResource) -> str:
    from ...schema import stable_record_id

    return stable_record_id("slice38e_frame_lineage", resource_lineage_body(record))
