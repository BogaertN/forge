"""Canonical UTF-8 JSON and deterministic SHA-256 helpers for Slice 43E."""
...
class DriftClassificationCanonicalizationError(ValueError):
    pass
...
def _normalize(value: Any) -> Any:
    ...
    if type(value) is dict:
        if not all(type(key) is str for key in value):
            raise DriftClassificationCanonicalizationError(
...
def stable_identifier(
    ...
    *,
    ...
    if (
        ...
        raise DriftClassificationCanonicalizationError("invalid namespace")
    return (
        f"{namespace}:"
...
__all__ = (
    "SUPPORTED_RECORD_TYPES",
    "DriftClassificationCanonicalizationError",
