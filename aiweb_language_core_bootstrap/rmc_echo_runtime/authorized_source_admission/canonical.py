"""Canonical UTF-8 JSON and deterministic SHA-256 helpers for Slice 43C."""
...
class SourceAdmissionCanonicalizationError(ValueError):
    pass
...
def _normalize(value: Any) -> Any:
    ...
    if type(value) is dict:
        if not all(type(key) is str for key in value):
            raise SourceAdmissionCanonicalizationError(
...
def canonical_record_bytes(
    ...
    *,
    ...
    value = {
        ...
        if name not in set(exclude_fields)
    }
    return json.dumps(
        value,
        ensure_ascii=False,
...
def stable_identifier(
    ...
    *,
    ...
    if (
        ...
        raise SourceAdmissionCanonicalizationError("invalid namespace")
    return (
        f"{namespace}:"
...
__all__ = (
    "SUPPORTED_RECORD_TYPES",
    "SourceAdmissionCanonicalizationError",
