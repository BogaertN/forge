"""Canonical serialization and stable identity helpers for Slice 40F."""
...
class RecoverablePurposeCanonicalizationError(ValueError):
    pass
...
def _normalize(value: Any) -> Any:
    ...
    if isinstance(value, dict):
        if not all(isinstance(key, str) for key in value):
            raise RecoverablePurposeCanonicalizationError(
...
def with_expected_id(record: Any, field_name: str, prefix: str) -> Any:
    ...
    return replace(
        record,
        **{field_name: stable_identifier(prefix, payload)},
    )
