"""Canonical deterministic serialization for Slice 41C records."""
...
CANONICAL_FIELD_ORDER_VERSION = "aiweb-slice41c-canonical-field-order-v1"
...
class SelectionEligibilityCanonicalizationError(ValueError):
    pass
...
def canonical_field_order(record_type: type[Any]) -> tuple[str, ...]:
    try:
        ...
        raise SelectionEligibilityCanonicalizationError(
...
def stable_identifier(
    ...
    *,
    ...
    if not isinstance(namespace, str) or not namespace or namespace.strip() != namespace:
        raise SelectionEligibilityCanonicalizationError("invalid namespace")
    digest = deterministic_record_digest(record, exclude_fields=exclude_fields)
...
__all__ = (
    ...
    "SUPPORTED_RECORD_TYPES",
    "SelectionEligibilityCanonicalizationError",
