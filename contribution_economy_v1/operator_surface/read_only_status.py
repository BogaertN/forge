...
CONTROLLED_WRITE_BUILD_ID = "CE-CONTROLLED-CONTRIBUTION-EVENT-BUILD-004"
STATUS_SCHEMA_VERSION = "ce_operator_surface_status_v2_build004"
MEA_PREVIEW_SCHEMA_VERSION = "ce_operator_surface_mea_preview_v1_build003"
CONTROLLED_EVENT_SCHEMA_VERSION = "ce_operator_surface_controlled_event_v1_build004"
CONTROLLED_CAPSULE_SCHEMA_VERSION = "ce_operator_surface_controlled_capsule_candidate_v1_build004"
...
_IDENTITY_TABLES = (
    ...
    "contributor_internal_persistence_consent_events_v1",
)
_LEDGER_DATA_TABLES = (
...
_CONTROLLED_AUDIT_TABLES = (
    "contribution_event_evidence_manifests_v1",
    "controlled_write_authorizations_v1",
    "contribution_event_payloads_v1",
    "memory_capsule_candidate_payloads_v1",
    "controlled_write_receipts_v1",
)
...
def _identity_authority_summary(identity_database: Path) -> dict[str, Any]:
    ...
    persistence_scope_events = int(counts["row_counts"].get("contributor_internal_persistence_consent_events_v1", 0))
    return {
        ...
        "internal_persistence_scope_event_count": persistence_scope_events,
        "tables": counts,
    }
...
def _core_store_summary(core_database: Path) -> dict[str, Any]:
    ...
    controlled = _safe_table_counts(core_database, _CONTROLLED_AUDIT_TABLES)
    ...
    return {
        ...
        "controlled_write_tables": controlled,
        "controlled_write_executed": counts["row_counts"].get("contribution_events", 0) == 1 and counts["row_counts"].get("memory_capsule_candidates", 0) == 1,
        "controlled_write_scope": "CE-EVT-000001 only",
        "total_governed_records": sum(counts["row_counts"].values()),
        "general_live_event_persistence_enabled": bool(metadata.get("live_event_persistence_enabled", False)),
...
def build_operator_status(*, forge_root: Path, identity_vault_root: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "installed_layers": {
            ...
            "controlled_first_event": CONTROLLED_WRITE_BUILD_ID,
        },
        ...
        "operator_visibility": {
            ...
            "read_only_routes": [
                ...
                "/api/contribution-economy/controlled-event/latest",
                "/api/contribution-economy/capsule-candidate/latest",
            ],
            "contribution_mutation_routes_enabled": False,
            "build002_core_mutation_routes_remain_disabled": True,
            "visibility_activation_scope": "Build 004 read-only visibility of one controlled persisted event/candidate",
        },
...
def _payload_from_core_table(forge_root: Path, table: str, key_field: str, key_value: str, payload_field: str) -> dict[str, Any] | None:
    core_database = Path(forge_root).resolve() / "memory" / "contribution_economy_v1" / "core" / "contribution_economy_core.db"
    with _read_only_connection(core_database) as connection:
        tables = {str(row[0]) for row in connection.execute("SELECT name FROM sqlite_master WHERE type='table'").fetchall()}
        if table not in tables:
            return None
        row = connection.execute(f"SELECT {payload_field} FROM {table} WHERE {key_field} = ?", (key_value,)).fetchone()
    return json.loads(row[0]) if row else None


def build_controlled_event_latest_status(*, forge_root: Path) -> dict[str, Any]:
    event = _payload_from_core_table(forge_root, "contribution_event_payloads_v1", "event_id", "CE-EVT-000001", "event_payload_json")
    safe_event = None
    if event:
        safe_event = {key: event.get(key) for key in ("event_id", "event_title", "identity_disclosure_class", "action_type", "contribution_type", "difficulty_class", "influence_type", "classification_status", "contributor_action_proof_hash", "source_artifact_proof_hash", "timestamp_utc", "event_status", "validation_record_id", "capsule_finalization_authorized", "ct_minting_authorized", "ledger_write_authorized", "public_attribution_authorized")}
    return {"status": "OK", "schema_version": CONTROLLED_EVENT_SCHEMA_VERSION, "api_contract": API_CONTRACT, "endpoint": "/api/contribution-economy/controlled-event/latest", "build_id": CONTROLLED_WRITE_BUILD_ID, "read_only": True, "event": safe_event, "boundary": _read_only_boundary()}


def build_controlled_capsule_candidate_latest_status(*, forge_root: Path) -> dict[str, Any]:
    candidate = _payload_from_core_table(forge_root, "memory_capsule_candidate_payloads_v1", "capsule_id", "CE-CAP-CAND-000001", "candidate_payload_json")
    safe_candidate = None
    if candidate:
        safe_candidate = {key: candidate.get(key) for key in ("capsule_id", "capsule_status", "finalized", "contribution_event_id", "principal_id", "contributor_action_proof_hash", "proof_hash_role", "source_artifact_proof_hash", "validation_status", "ct_reward_calculation_preview_reference_only", "ct_minting_status", "ct_minted_milli", "influence_ledger_write_authorized", "investment_ledger_write_authorized", "public_output_authorized", "top_level_hash")}
    return {"status": "OK", "schema_version": CONTROLLED_CAPSULE_SCHEMA_VERSION, "api_contract": API_CONTRACT, "endpoint": "/api/contribution-economy/capsule-candidate/latest", "build_id": CONTROLLED_WRITE_BUILD_ID, "read_only": True, "candidate": safe_candidate, "boundary": _read_only_boundary()}
