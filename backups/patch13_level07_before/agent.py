...
class ForgeAgent:
    ...
    def __init__(self, session_id: str, session_memory: SessionMemory):
        ...
        self._current_question: str = ""   # set at start of each ask()
    ...
    def _execute_tool_call(self, tool_name: str, tool_args: dict) -> str:
        ...
        # propose_command writes its own audit entry inside proposals.py
        # All other tools get audited here
        if tool_name == "propose_command":
            # Audit already written inside proposals.propose_command()
            result_str = result.get("data", result.get("error", "Proposal processed."))
            self.memory.record_tool_call(
                tool=tool_name,
                args=tool_args,
                result=result_str,
                audit_hash="(see proposals audit entry)"
            )
            return result_str
