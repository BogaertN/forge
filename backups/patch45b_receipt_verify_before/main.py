...
def _print_banner(session_id: str, scope: list[str]):
    ...
    print("│  Ingestion Integrity Hardening.                     │")
...
FORGE_EXPECTED_COMMANDS = [
    ...
    "ingest-history", "ingest-receipt-show",
    "context-duplicates", "context-reset-plan", "context-export-manifest",
]
...
def _already_ingested_record(corpus_id: str) -> Optional[dict]:
    ...
    if receipt_matches:
        ...
        return {
            ...
            "chunk_count": receipt.get("chunk_count_verified", receipt.get("chunk_count_planned", receipt.get("chunk_count", "?"))),
...
def cmd_ingest_history(session_id: str) -> None:
    ...
    for rpath in receipts:
        ...
        print(f"      chunks    : {r.get('chunk_count_verified', r.get('chunk_count_planned', '?'))}")
...
def cmd_ingest_receipt_show(receipt_arg: str, session_id: str) -> None:
    ...
    print(f"  chunk count        : {receipt.get('chunk_count_verified', receipt.get('chunk_count_planned', '?'))}")
...
def cmd_context_duplicates(session_id: str) -> None:
    """Read-only duplicate checker for context receipts and Chroma metadata. Usage: context-duplicates"""
    ...
    for rpath in _receipt_json_paths():
        try:
            receipts.append((rpath, _load_receipt(rpath)))
        except Exception:
            pass

    corpus_ids = [r.get("corpus_id", "") for _, r in receipts if r.get("corpus_id")]
    source_shas = [r.get("sha256_16", r.get("source_hash", "")) for _, r in receipts if r.get("sha256_16", r.get("source_hash", ""))]
    receipt_ids = [r.get("receipt_id", p.stem) for p, r in receipts]
    dup_corpus = [k for k, v in Counter(corpus_ids).items() if v > 1]
    dup_source = [k for k, v in Counter(source_shas).items() if v > 1]
    dup_receipts = [k for k, v in Counter(receipt_ids).items() if v > 1]

    chroma_chunk_ids: list[str] = []
    chroma_chunk_shas: list[str] = []
    ...
    try:
        ...
        if CONTEXT_LIB_CHROMA.exists():
            ...
            chroma_chunk_ids = list(data.get("ids", []) or [])
            for meta in data.get("metadatas", []) or []:
                if meta:
                    chroma_corpus_ids.append(str(meta.get("corpus_id", "")))
                    chroma_chunk_shas.append(str(meta.get("chunk_sha256_16", "")))
    except Exception as e:
        chroma_error = str(e)

    dup_chunk_ids = [k for k, v in Counter(chroma_chunk_ids).items() if k and v > 1]
    dup_chunk_sha = [k for k, v in Counter(chroma_chunk_shas).items() if k and v > 1]
    chroma_dup_corpus = [k for k, v in Counter(chroma_corpus_ids).items() if k and v > 1]

    write_audit_entry(session_id, "CONTEXT_DUPLICATES_CHECKED", str(CONTEXT_LIB_DIR), "read-only",
                      f"receipts={len(receipts)} dup_corpus={len(dup_corpus)} dup_chunks={len(dup_chunk_ids)}")

    print()
    print(f"── Context Duplicate Check ───────────────────────────")
    print(f"  Receipts checked          : {len(receipts)}")
    print(f"  Duplicate receipt IDs     : {dup_receipts or 'none'}")
    print(f"  Duplicate receipt corpus  : {dup_corpus or 'none'}")
    print(f"  Duplicate source SHA      : {dup_source or 'none'}")
    print()
    print(f"  Chroma collection         : {CONTEXT_COLLECTION}")
    if chroma_error:
        print(f"  Chroma metadata check     : unavailable ({chroma_error})")
    else:
        print(f"  Chroma chunks checked     : {len(chroma_chunk_ids)}")
        print(f"  Duplicate chunk IDs       : {dup_chunk_ids or 'none'}")
        print(f"  Duplicate chunk SHA       : {dup_chunk_sha or 'none'}")
        print(f"  Repeated corpus IDs       : {chroma_dup_corpus or 'none'}")
    if corpus_ids:
        print()
        print(f"  Ingested corpus IDs:")
        for cid in sorted(set(corpus_ids)):
            n = corpus_ids.count(cid)
            marker = "already ingested" if n >= 1 else ""
            print(f"    • {cid} ({n} receipt{'s' if n != 1 else ''}) {marker}")
    print(f"  [Read-only. No writes performed.]")
    print(f"──────────────────────────────────────────────────────")
    print()
...
def _context_export_payload() -> dict:
    ...
    for rpath in _receipt_json_paths():
        ...
        receipts_payload.append({
            ...
            "chunk_count": r.get("chunk_count_verified", r.get("chunk_count_planned")),
