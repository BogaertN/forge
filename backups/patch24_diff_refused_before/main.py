...
def _print_banner(session_id: str, scope: list[str]):
    print()
    print("┌─────────────────────────────────────────────────────┐")
    print("│  FORGE  —  Local Coding Assistant  —  Level 2.1    │")
    print("│  Patch Proposal + Diff Preview.                     │")
    print("│  Forge proposes. Forge previews. Nic applies.       │")
    print("└─────────────────────────────────────────────────────┘")
...
def cmd_patch_review(
    ...
    # Must be inside ~/forge/proposed_patches/
    patches_dir = FORGE_ROOT / "proposed_patches"
    try:
        proposal_path.relative_to(patches_dir)
    except ValueError:
        print(f"[forge] ERROR: Proposal file must be inside ~/forge/proposed_patches/")
    ...
    try:
        while True:
            ...
            if user_input.lower() == "help":
                ...
                print("    patch-propose <target_file>   — create a patch proposal (bypasses LLM)")
                print("    patch-review <proposal_file>  — show unified diff preview of a proposal")
                ...
                print()
                print("  Level 2.1 patch workflow:")
                print("    forge> patch-propose ~/aiweb/agents/agent.py   ← create proposal")
                print("    forge> patch-review 2026-05-09_PATCH_agent_py  ← preview diff")
                print("    [Nic edits the file manually using the proposal + diff as guide]")
                print()
