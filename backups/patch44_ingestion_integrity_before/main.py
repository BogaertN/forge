...
def _print_banner(session_id: str, scope: list[str]):
    ...
    print("│  Dual-Brain Single-Document Ingestion.              │")
    print("│  ingest-one | context-status | context-search-test  │")
    print("└─────────────────────────────────────────────────────┘")
...
FORGE_EXPECTED_COMMANDS = [
    ...
    "context-status", "context-search-test",
]
...
def _run_ingest_gates(corpus_id: str, rows: list[dict],
    ...
    # Gate 7: dry-run exists for this corpus_id
    dry_runs = sorted(INGESTION_PLANS_DIR.glob(f"*_dry_run_{corpus_id}.json"), reverse=True)
    gates.append(("dry-run plan exists", bool(dry_runs),
                  dry_runs[0].name if dry_runs else f"run 'ingest-dry-run {corpus_id}' first"))

    # Gate 8: dry-run validation
    if dry_runs:
        try:
            dr = _json.loads(dry_runs[0].read_text(encoding="utf-8"))
            chunk_count = dr.get("chunk_count", 0)
            dr_ok = chunk_count >= 1
            gates.append(("dry-run valid (chunks >= 1)", dr_ok,
                          f"chunk_count={chunk_count}"))
        except Exception as e:
            gates.append(("dry-run valid", False, str(e)))
    else:
        gates.append(("dry-run valid", False, "no dry-run to validate"))
...
def cmd_ingest_one(corpus_id: str, session_id: str) -> None:
    ...
    # ── Extract text ──────────────────────────────────────────────────────────
    result = extract_text(fpath, max_chars=300_000)
    if not result.ok or not result.text.strip():
        write_audit_entry(session_id, "INGEST_ONE_FAILED", corpus_id, "-",
                          f"extraction failed: {result.error or 'empty'}")
        print(f"[forge] INGEST_ONE_FAILED: Extraction error: {result.error}")
        return
    ...
    if not chunks:
        write_audit_entry(session_id, "INGEST_ONE_FAILED", corpus_id, "-", "zero chunks produced")
        print(f"[forge] INGEST_ONE_FAILED: No chunks produced.")
        return
    ...
    # ── Verify chunk count ────────────────────────────────────────────────────
    expected_n = len(ids)
    if post_count < expected_n:
        write_audit_entry(session_id, "INGEST_ONE_FAILED", corpus_id, f"{post_count}/{expected_n}",
                          "chunk count mismatch after write")
        print(f"[forge] INGEST_ONE_FAILED: Expected {expected_n} chunks, Chroma reports {post_count}")
        return
    ...
    receipt = {
        ...
        "chunk_count_verified": post_count,
        ...
        "verification_ok":    post_count >= expected_n,
    }
    ...
    write_audit_entry(session_id, "INGEST_ONE_COMPLETED", corpus_id,
                      f"{post_count} chunks",
                      f"collection={CONTEXT_COLLECTION} sha={corpus_sha} embedding={ef_name[:40]}")

    print(f"\n[forge] INGEST_ONE_COMPLETED")
    print(f"  corpus_id : {corpus_id}")
    print(f"  Chunks    : {post_count} written and verified")
    print(f"  Collection: {CONTEXT_COLLECTION}")
...
def cmd_ingest_verify(receipt_arg: str, session_id: str) -> None:
    """
    Verify an ingestion receipt against the context library.
    ...
    # Check 3: collection exists and has expected chunks
    try:
        import chromadb as _chroma
        client = _chroma.PersistentClient(path=str(CONTEXT_LIB_CHROMA))
        ...
        if col_exists:
            ...
            count = col.count()
            expected_n = receipt.get("chunk_count_verified", 0)
            checks.append(("chunk count >= planned", count >= expected_n,
                            f"collection has {count}, planned {expected_n}"))

            # Check 4: sample chunk has both brain fields
            if count > 0:
                sample = col.peek(limit=1)
                if sample.get("metadatas"):
                    meta = sample["metadatas"][0]
                    has_brain1 = "corpus_id" in meta and "sha256_16" in meta
                    has_brain2 = "rpmc_phase_tags" in meta and "symbolic_operators" in meta
                    checks.append(("chunk has Brain 1 (provenance)", has_brain1,
                                   f"corpus_id={meta.get('corpus_id','?')}"))
                    checks.append(("chunk has Brain 2 (RPMC symbolic)", has_brain2,
                                   f"phases={meta.get('rpmc_phase_tags','?')[:40]}"))
    except Exception as e:
        checks.append(("Chroma accessible", False, str(e)[:80]))

    all_ok = all(c[1] for c in checks)
    audit_tool = "INGEST_VERIFY_OK" if all_ok else "INGEST_VERIFY_FAILED"
    write_audit_entry(session_id, audit_tool, str(rpath), "-",
                      f"corpus_id={corpus_id} checks={len(checks)} passed={sum(1 for c in checks if c[1])}")
    ...
    for name, passed, detail in checks:
        print(f"  {'✓' if passed else '✗'} {name:<50} {detail[:50]}")
    print()
    print(f"  Result: {audit_tool}")
    print(f"──────────────────────────────────────────────────────")
    print()
...
def cmd_context_search_test(query: str, session_id: str) -> None:
    ...
    write_audit_entry(session_id, "CONTEXT_SEARCH_TEST_RUN", query[:80], f"{n_results} results",
    ...
    print(f"  [No writes performed. Context library unchanged.]")
    print(f"──────────────────────────────────────────────────────")
    print()
