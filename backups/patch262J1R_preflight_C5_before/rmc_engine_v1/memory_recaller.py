...
ENGINE_VERSION = "rmc_memory_recaller_v1_patch262J1R_preflight_B6"
ENGINE_MODE = "read_only_rmc_memory_recaller_trace_spine"
...
def _score_node(node: dict[str, Any], input_text: str, phase_report: dict[str, Any]) -> dict[str, Any]:
    ...
    retrieval_weight = round(min(1.0, semantic * 0.42 + phase_relevance + ancestry_score + drift_relation + active_loop), 4)
    evidence = []
    if overlap:
...
def memory_recaller_boundary() -> dict[str, Any]:
    return {
        ...
        "note": "This module builds read-only I_t/M_t/Φ_t/D_t trace inputs. It does not generate candidates or render final output.",
    }
