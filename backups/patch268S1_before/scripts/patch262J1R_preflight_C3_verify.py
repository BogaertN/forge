#!/usr/bin/env python3
"""Verifier for Patch 262J1R-Preflight-C3."""
...
def main() -> int:
    ...
    test = ROOT / "scripts" / "test_rmc_correction_naming_C3.py"
    ...
    add("file_exists_test_rmc_correction_naming_C3.py", test.exists(), "scripts/test_rmc_correction_naming_C3.py")
    ...
    add("main_has_C3_label", "PATCH 262J1R-PREFLIGHT-C3" in main_text)
    add("main_delegates_to_correction_naming_engine", "run_correction_and_naming" in main_text and "correction_naming_engine" in main_text)
    ...
    add("module_has_run_correction_and_naming", "def run_correction_and_naming" in mod_text)
    add("module_has_chi_t_and_N_t", "chi_t" in mod_text and "N_t" in mod_text)
    ...
    ok, detail = run_script("scripts/test_rmc_correction_naming_C3.py")
    add("test_rmc_correction_naming_C3.py_passes", ok, detail)
    # Keep key lower-stage tests passing.
    ...
    print("PATCH 262J1R-PREFLIGHT-C3 VERIFY")
    print("─" * 72)
    passed = 0
    ...
    if passed == len(checks):
        print("RESULT: PATCH_262J1R_PREFLIGHT_C3_VERIFY_OK")
        return 0
    print("RESULT: PATCH_262J1R_PREFLIGHT_C3_VERIFY_FAILED")
    return 1
