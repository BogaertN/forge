...
Patch 295 — MEA / Forge Discovery Kernel Controlled Live Candidate API Identity.

This module exposes the Forge Discovery Kernel as a read-only foundation surface
with deterministic replay support. Patch 294 added an approved initial-seed-only persistent problem-manifest store in Forge-owned runtime state. Patch 295 adds a read-only /api/mea/candidates view generated from the verified stored manifest. It cannot execute /api/mea/seal, commit a candidate, advance live state from a candidate, or write memory.
"""
...
KERNEL_STAGE = "controlled_live_candidates_patch295"
KERNEL_PATCH_ID = "Patch 295 — MEA Controlled Live Candidate API"
