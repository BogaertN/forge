...
def _print_banner(session_id: str, scope: list[str]):
    ...
    print("│  Phase D Grounding Attachment Marker Repair.          │")
    print("│  ground first | attach proof | marker-safe   │")
    print("└─────────────────────────────────────────────────────┘")
...
def _proposal_grounding_attachment_text(receipt: dict) -> str:
    ...
    lines = [
        ...
        "Marker repair: Patch 56B requires BEGIN/END sentinels; random user text is ignored.",
        "═" * 66,
...
def _append_grounding_attachment_to_proposal(proposal_path: str | Path, receipt: dict, session_id: str) -> bool:
    """Append the active grounding receipt summary to a Forge proposal file."""
    from agents.forge.memory import write_audit_entry
    try:
        ...
        has_begin = FORGE_GROUNDING_ATTACHMENT_BEGIN in current
        has_end = FORGE_GROUNDING_ATTACHMENT_END in current
        if has_begin and has_end:
            write_audit_entry(
                session_id=session_id,
                tool="PATCH_PROPOSAL_GROUNDING_ALREADY_ATTACHED",
                path=str(path),
                lines="read-only sentinel verified",
        ...
        if has_begin != has_end:
            write_audit_entry(
                ...
                path=str(path),
                lines="partial marker ignored; appending fresh attachment",
                reason=f"begin={has_begin} end={has_end} receipt={receipt.get('receipt_id')}",
            )
