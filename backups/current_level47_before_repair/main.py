...
def _print_banner(session_id: str, scope: list[str]):
    print()
    print("┌─────────────────────────────────────────────────────┐")
    print("│  FORGE  —  Local Coding Assistant  —  Level 4.7    │")
    print("│  Controlled Corpus Preview.                         │")
    print("│  corpus-preview: metadata controls access.          │")
    print("└─────────────────────────────────────────────────────┘")
...
def cmd_corpus_preview(corpus_id: str, session_id: str) -> None:
    ...
    User supplies only the ID — the path is resolved from metadata.
    Never ingests. Never writes to Chroma or vector DB.
    Archive-held items are refused.

    Usage: corpus-preview <corpus_id>
    Example: corpus-preview corpus_0011
    """
    import hashlib
    from agents.forge.memory import write_audit_entry
    ...
    if item is None:
        write_audit_entry(session_id, "CORPUS_PREVIEW_REFUSED", "-", "-",
                          f"id_not_found={corpus_id}")
        print(f"[forge] CORPUS PREVIEW REFUSED: ID '{corpus_id}' not found in manifest.")
        all_ids = [r.get("id", "?") for r in rows]
    ...
    if not eligible or ARCHIVE_MARKER in folder:
        reason = f"held/archive: folder={item.get('corpus_folder','')} eligible={item.get('index_eligible','')}"
        write_audit_entry(session_id, "CORPUS_PREVIEW_REFUSED", "-", "-",
                          f"id={corpus_id} | {reason}")
        print(f"[forge] CORPUS PREVIEW REFUSED: '{corpus_id}' is held from indexing.")
        print(f"  Folder    : {item.get('corpus_folder', '?')}")
        print(f"  Eligible  : {item.get('index_eligible', '?')}")
        print(f"  Archive-held items cannot be previewed in this version.")
        return

    # ── Path resolution and safety check ─────────────────────────────────────
    abs_path_str = item.get("absolute_path", "").strip()
    if not abs_path_str:
        abs_path_str = str(_CORPUS_ROOT / item.get("relative_path", ""))

    abs_path = Path(os.path.expanduser(abs_path_str))

    # Path must be inside CORPUS_ROOT
    if not _corpus_security_check(abs_path, session_id):
        return

    if not abs_path.exists():
        write_audit_entry(session_id, "CORPUS_PREVIEW_REFUSED", str(abs_path), "-",
                          f"id={corpus_id} | file_missing")
        print(f"[forge] CORPUS PREVIEW REFUSED: File not found on disk: {abs_path}")
        return
    ...
    if expected_sha:
        try:
            ...
            actual_sha = hashlib.sha256(raw).hexdigest()[:16]
        except OSError as e:
            write_audit_entry(session_id, "CORPUS_PREVIEW_REFUSED", str(abs_path), "-",
                              f"id={corpus_id} | read_error={e}")
            print(f"[forge] CORPUS PREVIEW REFUSED: Cannot read file: {e}")
            return

        if actual_sha != expected_sha:
            write_audit_entry(session_id, "CORPUS_PREVIEW_HASH_MISMATCH", str(abs_path), "-",
                              f"id={corpus_id} | expected={expected_sha} | actual={actual_sha}")
            print(f"[forge] CORPUS PREVIEW REFUSED: SHA-256 mismatch.")
            print(f"  Expected : {expected_sha}")
            print(f"  Actual   : {actual_sha}")
            print(f"  The file may have changed since the manifest was created.")
            return
    else:
        actual_sha = "unknown"
    ...
    if suffix in (".txt", ".md", ".csv"):
        try:
            preview_text = abs_path.read_text(encoding="utf-8", errors="replace")
        except OSError as e:
            extraction_note = f"Read error: {e}"
        ...
        try:
            ...
            doc = _DocxDoc(str(abs_path))
            paragraphs = [p.text for p in doc.paragraphs if p.text.strip()]
            preview_text = "\n".join(paragraphs)
        except ImportError:
            extraction_note = "PREVIEW_UNSUPPORTED_FORMAT_OR_MISSING_LIBRARY (python-docx not installed)"
        except Exception as e:
            extraction_note = f"docx extraction error: {e}"

    elif suffix == ".pdf":
        try:
            import pdfplumber as _pdfp
            with _pdfp.open(str(abs_path)) as pdf:
                pages_text = []
                for i, page in enumerate(pdf.pages):
                    if i >= 5:  # preview first 5 pages max
                        break
                    t = page.extract_text()
                    if t:
                        pages_text.append(t)
            ...
            try:
                ...
                extraction_note = "PREVIEW_UNSUPPORTED_FORMAT_OR_MISSING_LIBRARY (no PDF library)"
            except Exception as e:
                extraction_note = f"pdf extraction error: {e}"
        except Exception as e:
            extraction_note = f"pdf extraction error: {e}"

    else:
        extraction_note = f"PREVIEW_UNSUPPORTED_FORMAT_OR_MISSING_LIBRARY (unsupported extension: {suffix})"

    # ── Truncate and audit ────────────────────────────────────────────────────
    if extraction_note:
        write_audit_entry(session_id, "CORPUS_PREVIEW_UNSUPPORTED", str(abs_path), "-",
                          f"id={corpus_id} | note={extraction_note[:80]}")
    else:
        truncated = len(preview_text) > PREVIEW_MAX_CHARS
        preview_text = preview_text[:PREVIEW_MAX_CHARS]
        write_audit_entry(
            session_id=session_id,
            tool="CORPUS_PREVIEWED",
            path=str(abs_path),
            lines=f"{len(preview_text)} chars",
            reason=(
                f"id={corpus_id} | "
                f"sha256={actual_sha} | "
                f"truncated={truncated if not extraction_note else 'N/A'}"
            )
        )

    # ── Display ───────────────────────────────────────────────────────────────
    print()
    print(f"── Corpus Preview: {corpus_id} ───────────────────────")
    print(f"  Filename      : {item.get('filename', '?')}")
    print(f"  Folder        : {item.get('corpus_folder', '?')}")
    print(f"  Authority     : {item.get('authority', '?')}")
    print(f"  Status        : {item.get('status', '?')}")
    print(f"  Index eligible: {item.get('index_eligible', '?')}")
    print(f"  File path     : {abs_path}")
    print(f"  SHA-256       : {actual_sha}  ✓ matches manifest")
    print(f"  Format        : {suffix}")
    print()

    if extraction_note:
        print(f"  ⚠ {extraction_note}")
    else:
        char_count = len(preview_text)
        print(f"── Preview ({char_count} chars, max {PREVIEW_MAX_CHARS}) ─────────")
        print()
        for line in preview_text.splitlines()[:80]:
            print(f"  {line}")
        if truncated:
            print(f"\n  ... [preview truncated at {PREVIEW_MAX_CHARS} characters]")

    print()
    print(f"  ⚠ PREVIEW ONLY — this document has NOT been indexed or ingested.")
    print(f"──────────────────────────────────────────────────────")
    print()
