...
def _print_banner(session_id: str, scope: list[str]):
    print()
    print("┌─────────────────────────────────────────────────────┐")
    print("│  FORGE  —  Local Coding Assistant  —  Level 1.0    │")
    print("│  Read-Only Command Runner.                          │")
    print("│  Hard-coded allowlist. No shell. No project writes. │")
    print("└─────────────────────────────────────────────────────┘")
    ...
    try:
        while True:
            ...
            if user_input.lower() == "help":
                ...
                print("    propose <command>           — validate and save a proposal (bypasses LLM)")
                ...
                print()
                print("  run allowlist:  nvidia-smi, ollama ps, ollama version,")
                print("                  free -h, df -h, lscpu, lsblk")
                print()
                print("  workflow:  run nvidia-smi  →  Forge executes + analyzes immediately")
                print("             propose <cmd>   →  Forge validates, Nic runs, diag <cmd> to analyze")
                print()
