...
class ForgeAgent:
    ...
    def set_diag_session(self, diag_session_id: str | None):
        """
        Set or clear the active diagnostic session.
        Rebuilds the system prompt with the new session context.
        """
        self.active_diag_session_id = diag_session_id
        if diag_session_id:
            try:
                from agents.forge.diag_session import get_active_session_context
                ctx = get_active_session_context(diag_session_id)
            except Exception:
                ctx = ""
        else:
            ctx = ""
        self.system_prompt = build_system_prompt(self.session_id, self.memory, ctx)
    ...
    def _execute_tool_call(self, tool_name: str, tool_args: dict) -> str:
        ...
        # Execute the tool — pass session_id and user_question for propose_command
        result = dispatch(
            tool_name,
            tool_args,
            session_id=self.session_id,
        ...
        # propose_command, analyze_command_output, and run_safe_command write their own audit entries
        if tool_name in ("propose_command", "analyze_command_output", "run_safe_command"):
            result_str = result.get("data", result.get("error", "Tool processed."))
            self.memory.record_tool_call(
                tool=tool_name,
                args=tool_args,
                result=result_str,
                audit_hash="(see tool audit entry)"
            )

            # Link proposal to active diag session if one is running
            if tool_name == "propose_command" and self.active_diag_session_id:
                try:
                    from agents.forge.diag_session import link_proposal
                    raw = result  # result is already a dict from dispatch
                    # dispatch returns _ok(result["message"]) — we need the inner result
                    # The raw proposal dict is stored in the _proposals module
                    # We access it by checking the message content for the path
                    import re as _re
                    path_match = _re.search(r"/home/\S+\.txt", result_str)
                    if path_match:
                        link_proposal(
                            session_id=self.active_diag_session_id,
                            proposal_path=path_match.group(0),
                            command=tool_args.get("command", ""),
                            allowed="REFUSED" not in result_str,
                            risk_level=tool_args.get("risk_level", ""),
                        )
                except Exception:
                    pass  # session linking is non-critical — never fail the main flow
