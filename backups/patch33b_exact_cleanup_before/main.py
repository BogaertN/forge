...
def _print_banner(session_id: str, scope: list[str]):
    print()
    print("┌─────────────────────────────────────────────────────┐")
    print("│  FORGE  —  Local Coding Assistant  —  Level 4.5    │")
    print("│  Exact Directory Truth Mode.                        │")
    print("│  exact-list and exact-read bypass LLM entirely.     │")
    print("└─────────────────────────────────────────────────────┘")
...
def cmd_exact_list(arg: str, session_id: str) -> None:
    """
    Deterministic directory listing — bypasses the LLM entirely.
    Shows exact filesystem contents: names, sizes, permissions, type.
    Usage: exact-list <directory> [--files-only]
    """
    ...
    for part in parts:
        if part == "--files-only":
            ...
            dir_arg += (" " + part) if dir_arg else part

    dir_arg = dir_arg.strip()
    if not dir_arg:
        print()
        print("  Usage: exact-list <directory> [--files-only]")
        print("  Example: exact-list /home/nic/aiweb/runtime_wrappers/symbolic_cognition_stack")
        print()
        return

    target = os.path.realpath(os.path.abspath(os.path.expanduser(dir_arg)))

    # ── Security checks BEFORE any read ──────────────────────────────────────
    blocked, reason = is_path_blocked(target)
    if blocked:
        write_audit_entry(session_id, "EXACT_LIST_REFUSED", target, "-",
                          f"blocked: {reason[:120]}")
        print(f"[forge] EXACT_LIST_REFUSED: path is blocked: {reason}")
    ...
    if not allowed:
        ...
        print(f"[forge] EXACT_LIST_REFUSED: path is outside approved/session scope: {reason}")
        print(f"[forge] EXACT_LIST_REFUSED logged. No path was read.")
        return

    if not os.path.exists(target):
        print(f"[forge] ERROR: Path does not exist: {target}")
        return

    if not os.path.isdir(target):
        print(f"[forge] ERROR: Not a directory: {target}")
        print(f"  Use exact-read for files.")
        return

    # ── Scan directory ────────────────────────────────────────────────────────
    dirs, files, symlinks, others = [], [], [], []

    try:
        entries = sorted(os.scandir(target), key=lambda e: e.name.lower())
    except PermissionError as e:
        print(f"[forge] ERROR: Permission denied: {e}")
        return
    ...
    total = len(dirs) + len(files) + len(symlinks) + len(others)

    write_audit_entry(
        session_id=session_id,
        tool="EXACT_LIST",
        path=target,
        lines=f"{total} entries",
        reason=f"dirs={len(dirs)} files={len(files)} symlinks={len(symlinks)} files_only={files_only}"
    )

    def _fmt_size(b: int) -> str:
        if b < 1024:        return f"{b:>8}B"
        if b < 1048576:     return f"{b/1024:>7.1f}K"
        if b < 1073741824:  return f"{b/1048576:>7.1f}M"
        return               f"{b/1073741824:>7.1f}G"

    print()
    print(f"── Exact Listing: {target} ──────────────────────────")
    print(f"  (deterministic Python os.scandir — no LLM interpretation)")
    print()

    if not files_only and dirs:
        print(f"  DIRECTORIES ({len(dirs)}):")
        for name, mode, size in dirs:
            print(f"    {mode}  {_fmt_size(size)}  {name}/")
        print()

    if files:
        print(f"  FILES ({len(files)}):")
        for name, mode, size in files:
            print(f"    {mode}  {_fmt_size(size)}  {name}")
        print()

    if not files_only and symlinks:
        print(f"  SYMLINKS ({len(symlinks)}):")
        for name, mode, size in symlinks:
            link_target = ""
            try:
                link_target = f"  → {os.readlink(os.path.join(target, name))}"
            except OSError:
                pass
            print(f"    {mode}  {_fmt_size(size)}  {name}{link_target}")
        print()

    if not files_only and others:
        print(f"  OTHER ({len(others)}):")
        for name, mode, size in others:
            print(f"    {mode}  {_fmt_size(size)}  {name}")
        print()

    print(f"  Total: {total} entries  ({len(dirs)} dirs, {len(files)} files, {len(symlinks)} symlinks)")
    print(f"  Audit: EXACT_LIST logged")
    print(f"──────────────────────────────────────────────────────")
    print()


def cmd_exact_read(arg: str, session_id: str) -> None:
    """
    Deterministic file read — bypasses the LLM entirely.
    Shows exact file path, size, line count, and full content.
    ...
    if not file_arg:
        ...
        print("  Example: exact-read /home/nic/aiweb/runtime_wrappers/symbolic_cognition_stack/README.md")
        print()
        return

    target = os.path.realpath(os.path.abspath(os.path.expanduser(file_arg)))

    # ── Security checks BEFORE any read ──────────────────────────────────────
    blocked, reason = is_path_blocked(target)
    if blocked:
        write_audit_entry(session_id, "EXACT_READ_REFUSED", target, "-",
                          f"blocked: {reason[:120]}")
        print(f"[forge] EXACT_READ_REFUSED: path is blocked: {reason}")
        print(f"[forge] EXACT_READ_REFUSED logged. No path was read.")
        return
    ...
    if not allowed:
        write_audit_entry(session_id, "EXACT_READ_REFUSED", target, "-",
                          f"out of scope: {reason[:120]}")
        print(f"[forge] EXACT_READ_REFUSED: path is outside approved/session scope: {reason}")
        print(f"[forge] EXACT_READ_REFUSED logged. No path was read.")
        return

    if not os.path.exists(target):
        print(f"[forge] ERROR: File not found: {target}")
        return

    if os.path.isdir(target):
        print(f"[forge] ERROR: That is a directory. Use exact-list for directories.")
        return

    if os.path.islink(target) and not os.path.exists(target):
        print(f"[forge] ERROR: Broken symlink: {target}")
        return

    # ── Read file ─────────────────────────────────────────────────────────────
    file_size = os.path.getsize(target)
    MAX_READ  = 512 * 1024  # 512 KB cap

    if file_size > MAX_READ:
        print(f"[forge] ERROR: File too large for exact-read: {file_size:,} bytes (max {MAX_READ:,})")
        print(f"  Use: head -n 200 {target}  to see the first 200 lines.")
        return
    ...
    # Binary detection
    if b"\x00" in raw:
        print(f"[forge] EXACT_READ_REFUSED: file appears to be binary (contains null bytes).")
        write_audit_entry(session_id, "EXACT_READ_REFUSED", target, "-", "binary file")
        return
    ...
    try:
        ...
        try:
            ...
            encoding_note = " (decoded as latin-1)"
        except Exception:
            print(f"[forge] EXACT_READ_REFUSED: file is not readable as text.")
            write_audit_entry(session_id, "EXACT_READ_REFUSED", target, "-", "non-text file")
            return
    else:
        encoding_note = ""
    ...
    write_audit_entry(
        ...
        reason=f"size={file_size} bytes{encoding_note}"
    )

    print()
    print(f"── Exact Read: {os.path.basename(target)} ──────────────────────────────")
    print(f"  Path      : {target}")
    print(f"  Size      : {file_size:,} bytes")
    print(f"  Lines     : {line_count}{encoding_note}")
    print(f"  (deterministic Python read — no LLM interpretation)")
    print(f"──────────────────────────────────────────────────────")
    print()
    ...
    print()
    print(f"──────────────────────────────────────────────────────")
    print(f"  End of file: {os.path.basename(target)} ({line_count} lines, {file_size:,} bytes)")
    print(f"  Audit: EXACT_READ logged")
    print(f"──────────────────────────────────────────────────────")
    print()
...
def run_session():
    ...
    try:
        while True:
            ...
            if user_input.lower().startswith("exact-list ") or user_input.lower() == "exact-list":
                el_arg = user_input[11:].strip() if len(user_input) > 11 else ""
                if not el_arg:
                    print()
                    print("  Usage: exact-list <directory> [--files-only]")
                    print()
                else:
                    cmd_exact_list(el_arg, session_id)
                continue

            if user_input.lower().startswith("exact-read ") or user_input.lower() == "exact-read":
                er_arg = user_input[11:].strip() if len(user_input) > 11 else ""
                if not er_arg:
                    print()
                    print("  Usage: exact-read <file>")
                    print()
                else:
                    cmd_exact_read(er_arg, session_id)
                continue
