...
def _print_banner(session_id: str, scope: list[str]):
    ...
    print("│  Corpus Extraction + RPMC Kernel Planning.          │")
    print("│  rpmc-policy/template/check/source-list.            │")
    print("└─────────────────────────────────────────────────────┘")
...
def cmd_source_gap_report(session_id: str) -> None:
    """
    Hardened source gap report. Uses identity fingerprint matching — not fuzzy keywords.
    ...
    required_gaps   = sum(1 for r in results if r["required_for_full_authority"] and not r["satisfies_full_authority"])
    total_gaps      = sum(1 for r in results if not r["satisfies_full_authority"])

    # Trial: can proceed with warnings even if sources are RELATED_ONLY or MISSING
    # Full authority: requires all required sources to be EXACT or STRONG
    trial_ok        = True  # trial can always proceed with warnings
    ...
    md_lines  = [f"# Source Gap Report", f"",
                 f"**Verdict**: {verdict}  ", f"**Required gaps**: {required_gaps}  ",
                 f"**Trial readiness**: {report['trial_readiness']}  ",
                 f"**Full auth readiness**: {report['full_auth_readiness']}", f"",
                 f"| Title | Status | Confidence | Match type | Satisfies authority |",
                 f"|-------|--------|------------|-----------|---------------------|"]
    for r in results:
        md_lines.append(
            f"| {r['title'][:50]} | {r['status']} | {r['confidence']} | {r['match_type']} | {r['satisfies_full_authority']} |")
    ...
    write_audit_entry(session_id, "SOURCE_GAP_REPORT_CREATED", str(json_path), "-",
                      f"required_gaps={required_gaps} total_gaps={total_gaps} verdict={verdict[:60]}")
    write_audit_entry(session_id, audit_tool, str(json_path), "-",
                      f"required_gaps={required_gaps}")

    print()
    print(f"── Source Gap Report (Hardened) ──────────────────────")
    for r in results:
        ...
        print(f"      Satisfies  : {'YES' if r['satisfies_full_authority'] else 'NO — does not satisfy full authority'}")
        if r.get("evidence_snippet"):
            print(f"      Evidence   : {r['evidence_snippet'][:80]}")
        if r.get("warning"):
...
def cmd_ingest_one(corpus_id: str, session_id: str) -> None:
    """
    DISABLED — real ingestion is not yet enabled.
    Will perform single-document trial ingestion into aiweb_trial_v1 when authorized.
    Usage: ingest-one <corpus_id>
    """
    from agents.forge.memory import write_audit_entry
    corpus_id = corpus_id.strip() or "(none)"
    write_audit_entry(
        session_id=session_id,
        tool="INGEST_ONE_DISABLED",
        path=corpus_id,
        lines="-",
        reason="ingestion not yet authorized — awaiting RPMC source registration and ingestion patch approval"
    )
    print()
    print(f"[forge] INGEST_ONE_DISABLED")
    print(f"  Real ingestion is not enabled until:")
    print(f"    1. RPMC source documents are exact/strong registered in local corpus")
    print(f"    2. An explicit ingestion patch is approved")
    print(f"  No Chroma write. No embeddings. No collection created.")
    print(f"  To check readiness: run 'ingestion-readiness'")
    print(f"  To plan: run 'ingest-dry-run {corpus_id}'")
    print(f"  Audit: INGEST_ONE_DISABLED logged.")
    print()


def cmd_ingest_verify(receipt_arg: str, session_id: str) -> None:
    """
    DISABLED — no ingestion receipts exist because real ingestion is not yet enabled.
    Will verify ingestion receipts when ingest-one is authorized.
    Usage: ingest-verify <receipt_id_or_latest>
    """
    from agents.forge.memory import write_audit_entry
    write_audit_entry(
        session_id=session_id,
        tool="INGEST_VERIFY_DISABLED",
        path=receipt_arg or "latest",
        lines="-",
        reason="no ingestion receipts — ingestion not yet authorized"
    )
    print()
    print(f"[forge] INGEST_VERIFY_DISABLED")
    print(f"  No ingestion receipts exist because real ingestion is not yet enabled.")
    print(f"  Run 'ingestion-readiness' to check pre-ingestion gate status.")
    print(f"  Audit: INGEST_VERIFY_DISABLED logged.")
    print()
...
def cmd_forge_version(session_id: str) -> None:
    ...
    write_audit_entry(session_id, "FORGE_VERSION_SHOWN", "-", "-",
                      f"commands_expected={expected} trust_level=4.9")

    print()
    print(f"── Forge Version ─────────────────────────────────────")
    print(f"  Name          : FORGE — Local Coding Assistant")
    print(f"  Trust level   : 4.9")
...
def cmd_ingest_dry_run_validate(corpus_id: str, session_id: str) -> None:
    ...
    passed = not issues
    write_audit_entry(session_id, "INGEST_DRY_RUN_CREATED" if passed else "INGEST_DRY_RUN_FAILED",
                      str(plans[0]), "-",
                      f"validate: issues={len(issues)} warnings={len(warnings)}")
...
def cmd_ingest_dry_run(corpus_id: str, session_id: str) -> None:
    ...
    if not corpus_id:
        ...
        print("  Example: ingest-dry-run corpus_0011")
        print()
        return
    ...
    if item is None:
        write_audit_entry(session_id, "INGEST_DRY_RUN_REFUSED", "-", "-",
    ...
    if not abs_path.exists():
        write_audit_entry(session_id, "INGEST_DRY_RUN_REFUSED", str(abs_path), "-",
                          f"id={corpus_id} file_missing")
        print(f"[forge] INGEST DRY-RUN REFUSED: File not found: {abs_path}")
        return
    ...
    if expected_sha and actual_sha != expected_sha:
        write_audit_entry(session_id, "INGEST_DRY_RUN_REFUSED", str(abs_path), "-",
                          f"id={corpus_id} hash_mismatch expected={expected_sha} actual={actual_sha}")
        print(f"[forge] INGEST DRY-RUN REFUSED: SHA-256 mismatch.")
        print(f"  Expected: {expected_sha}  Actual: {actual_sha}")
        return

    # Extract text
    result = extract_text(abs_path, max_chars=200_000)
    if result.unsupported or result.error:
        write_audit_entry(session_id, "INGEST_DRY_RUN_FAILED", str(abs_path), "-",
                          f"id={corpus_id} extraction_error: {(result.error or '')[:80]}")
        print(f"[forge] INGEST DRY-RUN FAILED: Extraction error: {result.error}")
        return

    text = result.text
    if not text.strip():
        write_audit_entry(session_id, "INGEST_DRY_RUN_FAILED", str(abs_path), "-",
                          f"id={corpus_id} empty_extraction")
        print(f"[forge] INGEST DRY-RUN FAILED: Extraction returned empty text.")
        return

    # Chunk using hardened chunker
    ...
    tiny_chunks  = [c for c in chunks if c.get("tiny")]
    over_max     = [c for c in chunks if c.get("exceeds_max")]

    # Proposed metadata per chunk
    meta_fields = {
        "corpus_id":    corpus_id,
    ...
    INGESTION_PLANS_DIR.mkdir(parents=True, exist_ok=True)
    ts = datetime.datetime.now().strftime("%Y-%m-%d_%H%M%S")
    plan_name = f"{ts}_dry_run_{corpus_id}.json"
    ...
    plan = {
        ...
        "chunk_policy":        {"chunk_target": CHUNK_TARGET, "chunk_max": CHUNK_MAX,
                                "overlap": CHUNK_OVERLAP, "tiny_min": CHUNK_MIN,
        ...
        "target_collection_proposal": "aiweb_trial_v1",
        "target_db_path":      str(FORGE_ROOT / "memory" / "aiweb_corpus_db"),
    ...
    try:
        ...
        print(f"[forge] WARNING: Could not save dry-run plan: {e}")

    write_audit_entry(session_id, "INGEST_DRY_RUN_CREATED", str(plan_path), f"{len(chunks)} chunks",
                      f"id={corpus_id} chars={len(text)} chunks={len(chunks)} sha={actual_sha}")
    ...
    print(f"  Format      : {abs_path.suffix.lower()}")
    print(f"  Extracted   : {len(text):,} chars")
    print()
    print(f"  Chunk policy: target={CHUNK_TARGET} max={CHUNK_MAX} overlap={CHUNK_OVERLAP}")
    print(f"  Chunk count : {len(chunks)}")
    print(f"  Min chars   : {min_chars:,}" + (f"  ⚠ tiny" if min_chars < CHUNK_MIN else ""))
    print(f"  Max chars   : {max_chars:,}" + (f"  ✗ EXCEEDS HARD MAX {CHUNK_MAX}" if max_chars > CHUNK_MAX else "  ✓"))
    print(f"  Avg chars   : {avg_chars:,}")
    if tiny_chunks:
        print(f"  Tiny chunks : {len(tiny_chunks)}  ⚠ (< {CHUNK_MIN} chars)")
    if over_max:
        print(f"  Over-max    : {len(over_max)}  ✗ SHOULD BE 0")
    print()
    print(f"  Proposed collection: aiweb_trial_v1")
    print(f"  Proposed metadata fields: {list(meta_fields.keys())}")
    print()
    print(f"  First chunk preview:")
    for line in (chunks[0]["text"][:300] if chunks else "").splitlines()[:6]:
        print(f"    {line}")
    print()
