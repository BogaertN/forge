...
def cmd_add(path_arg: str):
    """
    forge add <path>
    Scan a path, show inclusion preview, ask for approval, then index into ChromaDB.

    Uses approved_paths.json + blocked_paths.json only.
    Session scope does not apply to this command.

    Audit entries written:
      KB_SCAN_START        — command begins
      KB_SCAN_PREVIEW      — after scan, with file counts
      KB_INDEX_CANCELLED   — if user declines
      KB_INDEX_APPROVED    — if user approves
      KB_INDEX_FILE        — per successfully indexed file (written inside index_path)
      KB_INDEX_SKIPPED_CURRENT — summary of already-current files (written inside index_path)
      KB_INDEX_ERROR       — per error (written inside index_path)
      KB_INDEX_COMPLETE    — final summary
    """
    ...
    # Generate a KB session ID for this indexing run
    kb_session_id = f"kb_{datetime.datetime.now().strftime('%Y_%m_%d_%H%M%S')}"

    # Check 1: path must be within approved_paths.json (session scope ignored)
    approved = get_approved_paths()
    if not approved:
    ...
    if not _is_under_any(path, approved):
        ...
        print(f"[forge] Approved paths: {approved}")
        print(f"[forge] Run 'python main.py approve {path}' to add it.")
        sys.exit(1)

    # Check 2: path must not be blocked
    ...
    write_audit_entry(
        ...
        path=path,
        lines="-",
        reason=f"forge add initiated | model={EMBED_MODEL}"
    )

    # Scan — temporarily override session scope so scan works outside a session
    import agents.forge.permissions as perms
    orig_session = perms.get_session_paths
    perms.get_session_paths = lambda: [path]

    print(f"[forge] Scanning {path} ...")
    try:
        scan = scan_for_indexing(path)
    finally:
    ...
    if scan.is_empty():
        ...
        if scan.already_current:
            print(f"[forge] {len(scan.already_current)} files are already current in the knowledge base.")
        write_audit_entry(
            session_id=kb_session_id,
    ...
    # AUDIT: KB_INDEX_COMPLETE
    errors = len(scan.will_index) - files_indexed
    write_audit_entry(
        session_id=kb_session_id,
        tool="KB_INDEX_COMPLETE",
        path=path,
        ...
        reason=(
            ...
            f"manifest={MEMORY_DIR / 'forge_kb_manifest.json'}"
        )
    )
...
def cmd_kb():
    """forge kb — show knowledge base status."""
    from agents.forge.knowledge_base import get_kb_stats
    print()
    print(get_kb_stats())
    print()
...
def main():
    ...
    # forge kb
    subparsers.add_parser("kb", help="Show knowledge base stats")
