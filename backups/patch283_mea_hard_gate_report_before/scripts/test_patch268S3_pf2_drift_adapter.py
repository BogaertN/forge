#!/usr/bin/env python3
"""Patch 268S3 — ProtoForge2 Drift Candidate Adapter Tests.
Run: python scripts/test_patch268S3_pf2_drift_adapter.py
"""
...
FAKE_ARBITRATOR_REAL_SIG = '''
class DriftArbitrator:
    """RMC-safe arbitrator with real audited signature."""
    def evaluate(self, text, current_phase, phase_history=None, expected_shape=None, memory_baseline=None):
        assert isinstance(text, str)
        assert isinstance(current_phase, int)
        assert phase_history is None or isinstance(phase_history, list)
        return {
            "epsilon_s": 0.12,
            "entropy": 0.19,
            "verdict": "ALLOW",
...
class DriftReport:
    ...
    def to_dict(self):
        return asdict(self)
class DriftBridge:
    """Sandbox drift bridge with real audited signature."""
    def evaluate(self, text, phase_history):
        assert isinstance(text, str)
        assert isinstance(phase_history, list)
        return DriftReport("DRIFT_WITHIN_BOUNDS", 0.18, 0.2, 0.0, True, False, False)
'''
...
b = boundary()
chk("T1_engine_268S3",          "268S3" in b["engine_version"])
...
r = probe_protoforge2_status("/tmp/pf2_definitely_missing_268S3")
...
# ── T9B: Real audited DriftArbitrator signature is called correctly ──────────
with tempfile.TemporaryDirectory() as td:
    arb_path = _write(td, "laws/aiweb_rmc_drift_detector_reference.py", FAKE_ARBITRATOR_REAL_SIG)
    test_cands = [
        {"path": Path(arb_path), "classification": "rmc_safe_arbitrator",
         "safe_by_default": True, "has_log_side_effects": False,
         "callable_class": "DriftArbitrator", "callable_method": "evaluate",
    ...
    r = preview_drift_call(_test_candidates=test_cands)
    chk("T9B_real_sig_preview_ok", r["status"] == "PREVIEW_OK")
    chk("T9B_called_signature", "signature_mapped_probe" in str(r.get("called_function", "")))
    chk("T9B_live_true", r.get("live_drift_available") is True)
    chk("T9B_normalized", r.get("normalized_result", {}).get("normalized") is True)

# ── T9C: Real audited DriftBridge signature + dataclass to_dict normalizes ────
with tempfile.TemporaryDirectory() as td:
    bridge_path = _write(td, "bridge/drift_bridge.py", FAKE_BRIDGE_REAL_SIG)
    test_cands = [
        {"path": Path(bridge_path), "classification": "sandbox_drift_bridge",
         "safe_by_default": True, "has_log_side_effects": False,
         "callable_class": "DriftBridge", "callable_method": "evaluate",
    ...
    r = preview_drift_call(_test_candidates=test_cands)
    chk("T9C_bridge_real_sig_ok", r["status"] == "PREVIEW_OK")
    chk("T9C_bridge_dataclass_normalized", r.get("normalized_result", {}).get("normalized") is True)
    chk("T9C_bridge_eps", "epsilon_s" in r.get("normalized_result", {}))
...
if failed == 0:
    print("\n  RESULT: patch268S3_tests=PASS"); sys.exit(0)
else:
    print("\n  RESULT: patch268S3_tests=FAIL"); sys.exit(1)
