...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="rmc_c8_test_") as tmp:
        ...
        _assert("actual_files_written_nonempty", len(committed.get("actual_files_written") or []) == 4)
        ...
        feedback_path = Path(committed["target_paths"]["feedback_object"])
        ...
        _assert("feedback_committed_status", feedback.get("feedback_status") == "committed_to_rmc_live_memory")
        _assert("feedback_has_own_id", bool(feedback.get("feedback_id")) and feedback.get("feedback_id") != node.get("memory_node_id"))
        _assert("feedback_has_retrieval_tags", "feedback_t" in (feedback.get("retrieval_tags") or []))
        _assert("receipt_commit_id_matches", receipt.get("commit_id") == committed.get("commit_id"))
        _assert("receipt_links_feedback", receipt.get("feedback_id") == feedback.get("feedback_id"))
        _assert("index_has_row", len(index_lines) == 1 and committed.get("commit_id") in index_lines[0] and feedback.get("feedback_id") in index_lines[0])
