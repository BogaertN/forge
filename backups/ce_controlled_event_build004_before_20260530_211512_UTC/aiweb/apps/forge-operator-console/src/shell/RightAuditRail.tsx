...
import { getContributionEconomyStatus, getIdentityVaultStatus, getOperatorOutputState, getProtoForgeReports } from '../api/forgeClient';
import type { ContributionEconomyStatusResponse, IdentityVaultStatusResponse, OperatorOutputStateResponse, ProtoForgeReportsResponse } from '../api/types';
...
export function RightAuditRail() {
  ...
  const [contributionEconomy, setContributionEconomy] = useState<ContributionEconomyStatusResponse | null>(null);
  const [error, setError] = useState<string>('');
  const [lastRefresh, setLastRefresh] = useState<string>('not loaded');

  const load = useCallback(async () => {
    try {
      const [outputData, protoData, identityData, contributionData] = await Promise.all([
        getOperatorOutputState(),
        getProtoForgeReports().catch(() => null),
        getIdentityVaultStatus().catch(() => null),
        getContributionEconomyStatus().catch(() => null),
      ]);
      setOutputState(outputData);
      setProtoForge(protoData);
      setIdentity(identityData);
      setContributionEconomy(contributionData);
      setError('');
  ...
  return (
    <aside className="right-rail right-rail-runtime-live">
      ...
      <div className="runtime-status-block">
        <div className="runtime-status-header">Contribution Economy</div>
        <MiniMetric label="Status" value={contributionEconomy?.status} tone={contributionEconomy?.status === 'OK' ? 'good' : 'warn'} />
        <MiniMetric label="Principals" value={contributionEconomy?.identity_authority?.registered_principal_count} />
        <MiniMetric label="Events" value={contributionEconomy?.integrated_core?.row_counts?.contribution_events} />
        <MiniMetric label="Capsules" value={contributionEconomy?.integrated_core?.row_counts?.memory_capsule_candidates} />
        <MiniMetric label="CT Mints" value={contributionEconomy?.integrated_core?.row_counts?.ct_mint_events} />
        <BoundaryLine label="CT mint gate" value={contributionEconomy?.integrated_core?.ct_minting_enabled} goodWhenFalse />
        <BoundaryLine label="Ledger writes" value={contributionEconomy?.integrated_core?.ledger_write_enabled} goodWhenFalse />
      </div>

      <p className="rail-note">
        Build 003: right rail reads live runtime and Contribution Economy status only. It does not execute commands, write files, mutate Identity Vault, persist capsules, mint CT, write ledgers, or mutate RMC live memory.
      </p>
    </aside>
  );
}
