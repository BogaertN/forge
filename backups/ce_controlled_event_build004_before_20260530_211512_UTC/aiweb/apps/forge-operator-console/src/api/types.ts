...
export interface AiwebOsBuildManifestResponse {
  status: string;
  endpoint?: string;
  mode?: string;
  created_at_utc?: string;
  read_only?: boolean;
  current_patch?: string;
  last_successful_patch?: string;
  next_recommended_patch?: string;
  identity?: Record<string, unknown>;
  versions?: Record<string, unknown>;
  counts?: Record<string, unknown>;
  active_modules?: Array<Record<string, unknown>>;
  accepted_patch_stack?: string[];
  known_gaps?: Array<Record<string, unknown>>;
  last_receipt_bundle_hint?: Record<string, unknown>;
  dirty_state?: Record<string, unknown>;
  terminus_high_security_ui?: Record<string, unknown>;
  health?: Record<string, unknown>;
  boundary?: Record<string, unknown>;
}


// CE-OPERATOR-SURFACE-BUILD-003 — Read-only integrated Contribution Economy operator surface.
export interface ContributionEconomyStatusResponse {
  status: string;
  ...
  read_consistency: {
    model: string;
    creates_sqlite_sidecars: boolean;
    requires_writer_checkpoint_before_visibility: boolean;
  };
  installed_layers: Record<string, string>;
  identity_authority: {
    registered_principal_count: number;
    private_identity_vault_only_principal_count: number;
    all_principals_private_only: boolean;
    public_display_authorization_event_count: number;
    economic_authorization_event_count: number;
  ...
  identity_authority_schema: {
    verified: boolean;
    v2_row_counts: Record<string, number>;
    missing_tables: string[];
  ...
  integrated_core: {
    ...
    row_counts: Record<string, number>;
    total_governed_records: number;
    live_event_persistence_enabled: boolean;
    capsule_finalization_enabled: boolean;
    ct_minting_enabled: boolean;
  ...
  ledgers: {
    ...
    influence_row_counts: Record<string, number>;
    investment_row_counts: Record<string, number>;
    influence_total_entries: number;
    investment_total_entries: number;
    ct_mint_records_created_by_investment: boolean;
    money_creates_ct: boolean;
    ledgers_permanently_separate: boolean;
  };
  economic_gates: {
    capsule_finalization_enabled?: boolean;
    ct_minting_enabled?: boolean;
    influence_ledger_writes_enabled?: boolean;
    investment_ledger_writes_enabled?: boolean;
    money_creates_ct?: boolean;
    authorization_receipt_alone_never_bypasses_live_gate?: boolean;
    [key: string]: unknown;
  };
  source_artifact_bridge: {
    mea_capsule_preview_endpoint: '/api/contribution-economy/mea-capsule-preview';
    mea_artifact_is_contribution_action_proof: boolean;
    mea_hypothesis_is_validated_contribution: boolean;
  };
  operator_visibility: {
    read_only_routes_enabled: boolean;
    read_only_routes: string[];
    contribution_mutation_routes_enabled: boolean;
    build002_core_mutation_routes_remain_disabled: boolean;
    visibility_activation_scope: string;
  };
...
export interface ContributionEconomyMeaPreviewResponse {
  status: string;
  schema_version: string;
  api_contract: 'forge_operator_console_api_v1';
  ...
  preview?: {
    ...
    seal_packet_hash: string;
    source_artifact_proof_hash: string;
    proof_hash: string | null;
    proof_hash_role: string;
    top_level_hash: string;
    contributor_binding_status: string;
    contributors_count: number;
    breath_validation_status: string;
    ct_minting_status: string;
    persistence_authorized: boolean;
    influence_ledger_write_authorized: boolean;
    investment_ledger_write_authorized: boolean;
    public_output_authorized: boolean;
