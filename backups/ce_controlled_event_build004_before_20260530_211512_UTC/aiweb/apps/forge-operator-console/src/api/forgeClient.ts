import type {
  ...
  ContributionEconomyStatusResponse,
  ContributionEconomyMeaPreviewResponse,
} from './types';
...
// CE-OPERATOR-SURFACE-BUILD-003 — read-only Contribution Economy endpoints.
export async function getContributionEconomyStatus(): Promise<ContributionEconomyStatusResponse> {
  const response = await fetch('/api/contribution-economy/status');
  return readJson<ContributionEconomyStatusResponse>(response);
}

export async function getContributionEconomyMeaPreview(): Promise<ContributionEconomyMeaPreviewResponse> {
  const response = await fetch('/api/contribution-economy/mea-capsule-preview');
  return readJson<ContributionEconomyMeaPreviewResponse>(response);
}
