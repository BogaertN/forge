#!/usr/bin/env python3
"""Patch 262C1 verifier — Audible browser tone + FBSC glyph codex alignment.

Static verifier only. It checks that /api/rmc/cymatic-preview remains read-only while
adding audible browser transposition and codex metadata. It does not start Forge, execute
shell, write memory, touch Identity Vault, or generate audio/image files.
"""
...
checks = {
    ...
    "audible_browser_transposition=True": "audible_browser_transposition" in text and "playback_frequency_hz" in text,
    "symbolic_frequency_preserved=True": "symbolic_frequency_hz" in text and "frequency_hz" in text,
    "glyph_codex_aligned=True": "glyph_codex_aligned" in text and "PHASE_GRACE" in text and "PHASE_RECURSION" in text,
...
if failed:
    print("PATCH262C1_AUDIBLE_TONE_GLYPH_CODEX_VERIFY_FAIL")
    for item in failed:
        print(f"missing={item}")
    sys.exit(1)
print("PATCH262C1_AUDIBLE_TONE_GLYPH_CODEX_VERIFY_PASS")
for item in checks:
    print(item)
