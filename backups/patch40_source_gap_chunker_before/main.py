...
KNOWN_SOURCE_GAPS = [
    {
        "title": "Resonant Phase Memory Calculus: The Law of Recursion, Collapse, and Return — Full Protocol Draft",
    ...
    {
        "title": "RPMC Implementation Blueprint (ProtoForge Dev Book)",
...
def cmd_source_gap_report(session_id: str) -> None:
    """Compare known Drive sources against local corpus manifest. Usage: source-gap-report"""
    import json as _json
    from agents.forge.memory import write_audit_entry

    rows = _corpus_load_json() or _corpus_load_csv()
    local_ids   = {r.get("id",""):  r for r in (rows or [])}
    local_fnames = {r.get("filename","").lower(): r for r in (rows or [])}
    local_titles = {r.get("use_for","").lower(): r for r in (rows or [])}

    results = []
    gap_count = 0
    for source in KNOWN_SOURCE_GAPS:
        # Try to find it in local corpus by title keywords
        title_lower = source["title"].lower()
        keywords = [w for w in title_lower.split() if len(w) > 4]
        found_row = None
        for row in (rows or []):
            row_text = " ".join([str(v).lower() for v in row.values()])
            if any(kw in row_text for kw in keywords[:3]):
                found_row = row
                break

        status = "registered_locally" if found_row else "MISSING_LOCALLY"
        ...
        results.append({
            ...
            "status":     status,
            "local_id":   found_row.get("id","") if found_row else None,
            "required_for_full_authority": source["required_for_full_authority"],
            "warning":    ("Required for full RPMC corpus authority" if source["required_for_full_authority"] and not found_row else ""),
        })
    ...
    report = {
        ...
        "gaps":       gap_count,
        "results":    results,
        "verdict":    "WARN_GAPS_PRESENT" if gap_count > 0 else "ALL_SOURCES_REGISTERED",
    }
    ...
    md_lines = [
        f"# Source Gap Report", f"",
        f"**Timestamp**: {report['timestamp']}  ",
        f"**Gaps**: {gap_count} / {len(results)}  ",
        f"**Verdict**: {report['verdict']}", f"",
        f"| Title | Category | Status | Warning |",
        f"|-------|----------|--------|---------|",
    ]
    for r in results:
        icon = "✓" if r["status"] == "registered_locally" else "⚠"
        md_lines.append(f"| {r['title'][:60]} | {r['category']} | {icon} {r['status']} | {r['warning']} |")
    ...
    audit_tool = "SOURCE_GAP_WARN" if gap_count > 0 else "SOURCE_GAP_OK"
    write_audit_entry(session_id, "SOURCE_GAP_REPORT_CREATED", str(json_path), "-",
                      f"gaps={gap_count} total={len(results)} verdict={report['verdict']}")
    write_audit_entry(session_id, audit_tool, str(json_path), "-",
                      f"gaps={gap_count}")

    print()
    print(f"── Source Gap Report ─────────────────────────────────")
    for r in results:
        icon = "✓" if r["status"] == "registered_locally" else "⚠"
        print(f"  {icon} {r['title'][:70]}")
        print(f"      Category : {r['category']}")
        print(f"      Status   : {r['status']}")
        if r["warning"]:
            print(f"      Warning  : {r['warning']}")
    print()
    print(f"  Verdict: {report['verdict']}  ({gap_count} gaps)")
    print(f"  Report : {json_path}")
    print(f"  Note   : Source gaps block full RPMC authority but NOT trial ingestion.")
    print(f"──────────────────────────────────────────────────────")
    print()


def cmd_source_gap_check(session_id: str) -> None:
    """Show the latest source gap report. Usage: source-gap-check"""
    import json as _json
    from agents.forge.memory import write_audit_entry
    ...
    write_audit_entry(session_id, "SOURCE_GAP_CHECK_SHOWN", str(reports[0]), "-",
                      f"gaps={data.get('gaps','?')} ts={data.get('timestamp','?')}")

    print()
    print(f"── Latest Source Gap Check ───────────────────────────")
    print(f"  Timestamp: {data.get('timestamp','?')}")
    print(f"  Gaps     : {data.get('gaps','?')} / {data.get('total_sources','?')}")
...
def _chunk_text(text: str, chunk_size: int = 1350, overlap: int = 200) -> list[dict]:
    """
    Character-based chunking that respects paragraph boundaries.
    Returns list of dicts with: index, text, char_start, char_end, char_count.
    """
        ...
        if len(current) + len(para) + 2 <= chunk_size:
            ...
            if current:
                chunks.append({
                    ...
                    "char_end":   char_start + len(current),
                    "char_count": len(current),
                })
                idx += 1
                # Overlap: take the tail of the current chunk
                overlap_text = current[-overlap:] if len(current) > overlap else current
                char_start   = char_start + len(current) - len(overlap_text)
                current      = (overlap_text + "\n\n" + para).strip()
            else:
                # Para is larger than chunk_size — force split
                for i in range(0, len(para), chunk_size - overlap):
                    piece = para[i:i + chunk_size]
                    chunks.append({
                        "index":      idx,
                        "text":       piece,
                        "char_start": char_start + i,
                        "char_end":   char_start + i + len(piece),
...
def cmd_ingest_dry_run(corpus_id: str, session_id: str) -> None:
    ...
    plan = {
        ...
        "chunk_policy":        {"chunk_size": CHUNK_SIZE, "overlap": OVERLAP, "method": "paragraph_aware"},
    ...
    print()
    print(f"  Chunk policy: size={CHUNK_SIZE} overlap={OVERLAP} method=paragraph_aware")
...
def cmd_ingestion_readiness(session_id: str) -> None:
    ...
    # 6. Source gap report exists
    gap_reports = sorted(SOURCE_GAP_REPORTS_DIR.glob("*_source_gap.json"), reverse=True)
    if gap_reports:
        try:
            gap_data = _json.loads(gap_reports[0].read_text(encoding="utf-8"))
            gaps = gap_data.get("gaps", 0)
            check("Source gap report exists", True, gap_reports[0].name)
            check("RPMC sources all registered", gaps == 0,
                  f"{gaps} source(s) missing locally (blocks full authority, not trial)", warn=(gaps > 0))
        except Exception:
