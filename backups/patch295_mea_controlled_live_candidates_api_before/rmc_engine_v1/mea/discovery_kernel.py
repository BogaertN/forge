...
Patch 294 — MEA / Forge Discovery Kernel Controlled Problem Manifest Store Identity.

This module exposes the Forge Discovery Kernel as a read-only foundation surface
with deterministic replay support. Patch 294 adds an explicitly approved, initial-seed-only persistent problem-manifest store in Forge-owned runtime state. It cannot execute /api/mea/seal, commit a candidate, advance live state from a candidate, or write memory.
"""
...
KERNEL_STAGE = "problem_manifest_store_patch294"
KERNEL_PATCH_ID = "Patch 294 — MEA Controlled Problem Manifest Store"
