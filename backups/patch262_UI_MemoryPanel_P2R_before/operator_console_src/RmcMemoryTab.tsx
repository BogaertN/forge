...
// RMC Memory Panel Phase 2: canonical route-manifest driven, read-only by default, gated writes only.
...
function RouteAvailability({ manifest }: { manifest: RmcRouteManifest | null }) {
  ...
  const knownGaps = [
    ['context_duplicates', 'Context duplicate status is not exposed as a canonical HTTP route yet.'],
    ['context_export_manifest', 'Context export manifest is not exposed as a canonical HTTP route yet.'],
    ['context_search_tests', 'Context search test history is not exposed as a canonical HTTP route yet.'],
  ];
  return (
...
export function RmcMemoryTab() {
  ...
  const loadOverview = useCallback(async () => {
    ...
    setError(null);
    try {
      const [routeManifest, memoryStatus, chromaStatus, activeLoop, pipelineSummary, promotionStatus, glyphStatus, llmStatus, datasetGrowthStatus, datasetGrowthCoverage] = await Promise.all([
  ...
  const canPromote = selectedCandidateId.length > 0 && promotionApproval === PROMOTION_TOKEN;
      ...
      <Section title="Promotion Path / Review Queue → Stable Memory → Retrieval Index" badge={data.promotionStatus?.status}>
        ...
        <div className="panel-card wide">
          <div className="output-slot-header">
            ...
            <strong>{canPromote ? 'armed' : 'locked'}</strong>
          </div>
          <div className="muted small-note">Promotion is a real write route. It stays locked until the exact approval token is typed.</div>
          <div className="action-row compact-actions">
