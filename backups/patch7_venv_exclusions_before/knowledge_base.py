...
INDEXING_MODES = {
    "core": {
        ...
        "allowed_json_max_kb": 50,   # small JSON config files only
        "excluded_extensions": {
        ...
        "excluded_dir_names": {
            ...
            "venv", "env", "dist", "build", "coverage", "htmlcov",
            ".mypy_cache", ".pytest_cache", ".ruff_cache", ".tox",
            "migrations", "archive", "archives", "backup", "backups",
            ".idea", ".vscode",
        },
    ...
    "all": {
        ...
        "allowed_extensions": None,      # None = defer to filetype_policy.json
        "allowed_filenames_exact": set(),
        "allowed_json_max_kb": 512,
        "excluded_extensions": set(),    # defer to filetype_policy.json
        "excluded_dir_names": {
...
def _mode_allows_file(filepath: str, policy: dict) -> tuple[bool, str]:
    ...
    allowed_exts       = policy.get("allowed_extensions")        # None = all via filetype_policy
    ...
    if ext in excluded_exts:
        return False, f"extension excluded in {policy.get('_description', 'this mode')[:30]}"
    ...
    if ext == ".json":
        ...
        try:
            ...
            if size_kb > max_json_kb:
                return False, f"JSON too large for this mode ({size_kb:.0f}KB > {max_json_kb}KB)"
        except OSError:
            return False, "cannot stat file"
        return True, ""

    # If mode has a strict allowlist (not None), file must match
    if allowed_exts is not None:
        if ext in allowed_exts:
            return True, ""
        if name_lower in allowed_names:
            return True, ""
        return False, f"extension '{ext}' not in {policy.get('_description', 'mode')} allowlist"
...
def scan_for_indexing(path: str, mode: str = "core") -> ScanResult:
    ...
    def _dir_is_excluded(name: str) -> bool:
        name_lower = name.lower()
    ...
    def _walk(current: str):
        ...
        for entry in entries:
            ...
            if entry.is_dir():
                # Always skip hidden dirs except a few useful ones
                if name.startswith(".") and name not in (".github",):
                    result.will_skip.append((entry.path, f"hidden directory: {name}"))
                    continue
                if _dir_is_excluded(name):
                ...
                # Sensitive block first — always wins
                ...
                # Size check from filetype_policy
