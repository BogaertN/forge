...
type PanelMode = 'closed' | 'status' | 'logs' | 'build' | 'action';
...
export function OperatorLifecycleMenu() {
  ...
  const [buildManifest, setBuildManifest] = useState<AiwebOsBuildManifestResponse | null>(null);
  ...
  async function showBuildManifest() {
    setMode('build');
    setOpen(true);
    setPreview(null);
    setConfirmResult(null);
    ...
    setBusy(true);
    try {
      const data = await getAiwebOsBuildManifest();
      setBuildManifest(data);
      setError('');
      setLastLoaded(new Date().toLocaleTimeString());
    } catch (err) {
      setError(err instanceof Error ? err.message : String(err));
      ...
      {open && (
        <section className="operator-life-popover" aria-label="AI.Web OS lifecycle menu">
          ...
          <div className="operator-life-command-row">
            ...
            <button onClick={() => void showBuildManifest()} disabled={busy}>Build Manifest</button>
            {ACTIONS.map((item) => (
              <button
                key={item.action}
          ...
          {mode === 'build' && (
            <div className="operator-life-body operator-life-build-manifest">
              <div className="operator-life-status-grid">
                <div className="operator-life-card">
                  <span>Status</span>
                  <strong className={statusTone(buildManifest?.status)}>{asText(buildManifest?.status)}</strong>
                </div>
                <div className="operator-life-card">
                  <span>Current patch</span>
                  <strong>{asText(buildManifest?.current_patch)}</strong>
                </div>
                <div className="operator-life-card">
                  <span>RMC routes</span>
                  <strong>{asText(buildManifest?.counts?.rmc_canonical_routes)}</strong>
                </div>
                <div className="operator-life-card">
                  <span>Commands</span>
                  <strong>{asText(buildManifest?.counts?.command_surface_expected)}</strong>
                </div>
              </div>
              <div className="operator-life-build-grid">
                <div>Last successful patch: <strong>{asText(buildManifest?.last_successful_patch)}</strong></div>
                <div>Operator console: <strong>{asText((buildManifest?.versions?.operator_console as Record<string, unknown> | undefined)?.version)}</strong></div>
                <div>Terminus hidden by default: <strong className={boundaryClass(buildManifest?.terminus_high_security_ui?.hidden_by_default)}>{asText(buildManifest?.terminus_high_security_ui?.hidden_by_default)}</strong></div>
                <div>Boundary OK: <strong className={boundaryClass(buildManifest?.health?.boundary_ok)}>{asText(buildManifest?.health?.boundary_ok)}</strong></div>
              </div>
              <details className="operator-life-log-file" open>
                <summary>Accepted patch stack</summary>
                <pre>{(buildManifest?.accepted_patch_stack || []).join('\n') || 'No build manifest loaded yet.'}</pre>
              </details>
              <details className="operator-life-log-file">
                <summary>Known gaps / queued hardening</summary>
                <pre>{JSON.stringify(buildManifest?.known_gaps || [], null, 2)}</pre>
              </details>
              <details className="operator-life-log-file">
                <summary>Full build manifest JSON</summary>
          ...
          <p className="operator-life-note">
            Patch 274: build manifest and lifecycle controls are preview-first and token-gated. The browser receives no shell, no arbitrary command input, and no authority over Forge, RMC memory, Chroma, or Identity Vault.
          </p>
        </section>
      )}
    </div>
