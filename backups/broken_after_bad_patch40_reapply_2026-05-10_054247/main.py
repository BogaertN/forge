...
def _print_banner(session_id: str, scope: list[str]):
    print()
    print("┌─────────────────────────────────────────────────────┐")
    print("│  FORGE  —  Local Coding Assistant  —  Level 0.8    │")
    print("│  Diagnostic Session Continuity.                     │")
    print("│  Proposals, outputs, and findings linked in chains. │")
    ...
    print("  'diag <cmd>' — paste output. 'diag-session start <topic>' — open a troubleshooting chain.")
    print()
    ...
    try:
        while True:
            ...
            if user_input.lower() == "help":
                ...
                print()
                print("  diag-session example:")
                print("    forge> diag-session start ollama_gpu_check")
                print("    forge> diag free -h           ← output linked to session")
                print("    forge> diag-session status    ← see the full chain")
                print()
