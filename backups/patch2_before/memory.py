...
def write_audit_entry(
    ...
    try:
        ...
        raise RuntimeError(f"CRITICAL: Cannot write to audit log at {AUDIT_LOG}: {e}")
...
def write_session_end(session_id: str, changed: bool) -> str:
    """Write the session-close audit entry."""
    status = "FILES_UNCHANGED" if not changed else "FILES_CHANGED"
    return write_audit_entry(
        session_id=session_id,
        tool="SESSION_END",
        path="-",
