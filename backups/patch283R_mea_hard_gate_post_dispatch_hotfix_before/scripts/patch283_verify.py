...
PATCH = "Patch 283 — MEA Hard Gate Report / Candidate Gate Engine Preview"
...
def main() -> int:
    print("PATCH 283 VERIFIER — MEA Hard Gate Report / Candidate Gate Engine Preview")
    ...
    try:
        ...
        record(mea.HARD_GATE_REPORT_PATCH_ID == PATCH, "patch283_id_export", getattr(mea, "HARD_GATE_REPORT_PATCH_ID", ""))
    ...
    total = len(results)
    failed = total - passed
    print(f"RESULT: PATCH_283_VERIFY {'PASS' if failed == 0 else 'FAIL'}  Total:{total} Passed:{passed} Failed:{failed}")
