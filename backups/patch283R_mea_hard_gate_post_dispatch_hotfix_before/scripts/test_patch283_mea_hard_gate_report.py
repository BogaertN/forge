...
def main() -> int:
    print("PATCH 283 BEHAVIOR TESTS — MEA Hard Gate Report")
    ...
    record(HARD_GATE_REPORT_PATCH_ID == "Patch 283 — MEA Hard Gate Report / Candidate Gate Engine Preview", "patch_id_locked", HARD_GATE_REPORT_PATCH_ID)
    ...
    total = len(results)
    failed = total - passed
    print(f"RESULT: PATCH_283_BEHAVIOR {'PASS' if failed == 0 else 'FAIL'}  Total:{total} Passed:{passed} Failed:{failed}")
