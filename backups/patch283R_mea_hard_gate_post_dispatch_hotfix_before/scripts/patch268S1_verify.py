#!/usr/bin/env python3
"""Patch 268S1 Verifier. Run: python scripts/patch268S1_verify.py"""
...
b = boundary()
chk("engine_268S1",             "268S1" in ENGINE_VERSION)
...
r = probe_protoforge2_status("/tmp/pf2_268S1_verify_missing")
...
# Behavior tests
r_bt = subprocess.run([sys.executable, "scripts/test_patch268S1_pf2_drift_adapter.py"],
...
print(f"\nPATCH 268S1 VERIFIER")
print("─"*60)
for n,v,d in results:
    print(f"  {'✓' if v==PASS else '✗'} [{v}] {n}" + (f"\n        {d}" if d and v==FAIL else ""))
print("─"*60)
print(f"  Total: {len(results)}  Passed: {passed}  Failed: {failed}")
if failed==0: print("\nRESULT: PATCH_268S1_VERIFY_OK"); sys.exit(0)
else: print(f"\nRESULT: PATCH_268S1_VERIFY_FAIL ({failed})"); sys.exit(1)
