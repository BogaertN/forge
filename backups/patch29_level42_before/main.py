...
def _print_banner(session_id: str, scope: list[str]):
    print()
    print("┌─────────────────────────────────────────────────────┐")
    print("│  FORGE  —  Local Coding Assistant  —  Level 4.1    │")
    print("│  Post-Apply Integrity + Session Write Status.       │")
    print("│  Verify applied state. Track approved writes.       │")
    print("└─────────────────────────────────────────────────────┘")
...
def cmd_patch_apply(
    ...
    if result["ok"]:
        ...
        print()
        return True   # signals approved project write to session loop
    else:
...
def run_session():
    ...
    try:
        ...
        # Write session end — reason distinguishes approved-write sessions
        end_reason = "APPROVED_PATCH_APPLIED" if files_changed else "FILES_UNCHANGED"
        write_session_end(session_id, end_reason)

        print()
        if files_changed:
            print("[forge] Session ended. Approved project file writes occurred this session.")
        else:
