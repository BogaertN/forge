...
ENGINE_VERSION = "rmc_manifest_compiler_kernel_v1_patch262J"


def _stable_digest(value: Any, size: int = 16) -> str:
    return hashlib.sha256(repr(value).encode("utf-8", errors="replace")).hexdigest()[:size]
...
def compile_manifest_dry_run(coherence_report: dict) -> dict:
    ...
    preflight = {
        ...
        "required_before_manifest": [
            ...
            if reason not in ("projection gate must remain blocked during dry-run",)
        ],
        "source_trace": source_trace,
    ...
    if manifest_allowed:
        manifest_packet = {
            ...
            "render_targets": {
                ...
                "cymatic_geometry": {"status": "preview_only_existing_browser_layer", "file_written": False},
