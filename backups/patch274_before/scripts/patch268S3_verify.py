#!/usr/bin/env python3
"""Patch 268S3 Verifier. Run: python scripts/patch268S3_verify.py"""
...
b = boundary()
chk("engine_268S3",             "268S3" in ENGINE_VERSION)
...
r = probe_protoforge2_status("/tmp/pf2_268S3_verify_missing")
...
# Behavior tests
r_bt = subprocess.run([sys.executable, "scripts/test_patch268S3_pf2_drift_adapter.py"],
...
print(f"\nPATCH 268S3 VERIFIER")
print("─"*60)
for n,v,d in results:
    print(f"  {'✓' if v==PASS else '✗'} [{v}] {n}" + (f"\n        {d}" if d and v==FAIL else ""))
print("─"*60)
print(f"  Total: {len(results)}  Passed: {passed}  Failed: {failed}")
if failed==0: print("\nRESULT: PATCH_268S3_VERIFY_OK"); sys.exit(0)
else: print(f"\nRESULT: PATCH_268S3_VERIFY_FAIL ({failed})"); sys.exit(1)
