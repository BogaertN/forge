...
required = [
    ...
    "cmd_forge_agent_activation_preflight_all",
    "cmd_forge_agent_activation_preflight_all_report",
...
if missing:
    print("PATCH231B_VERIFY_FAIL missing markers:")
    for item in missing:
        print(" -", item)
    sys.exit(1)
if "def cmd_forge_agent_activate_manual" in main_text or "forge-agent-activate-manual\")" in main_text:
    print("PATCH231B_VERIFY_FAIL activation command appears installed")
    sys.exit(1)
...
if not (0 <= idx_all < idx_prefix and 0 <= idx_all_report < idx_prefix and 0 <= idx_report < idx_prefix):
    print("PATCH231B_VERIFY_FAIL route order unsafe")
...
for key in ("forge_agent_activation_preflight_all", "forge_agent_activation_preflight_all_report"):
    if key not in tools:
        print(f"PATCH231B_VERIFY_FAIL missing tool registry key {key}")
        sys.exit(1)
print("PATCH231B_VERIFY_PASS")
print("commands=forge-agent-activation-preflight-all, forge-agent-activation-preflight-all-report")
print("boundary=batch read-only preflight; no activation command installed; no RMC memory write; no Identity Vault DB write; no activation")
