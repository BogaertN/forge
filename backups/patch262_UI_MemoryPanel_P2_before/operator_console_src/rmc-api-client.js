...
export async function fetchRouteManifest(force = false) {
  ...
  try {
    routeManifestCache = await readJson(await fetch(FALLBACK_ROUTES.route_manifest));
  } catch (_err) {
    routeManifestCache = { status: 'FALLBACK', lookup: { ...FALLBACK_ROUTES }, routes: [], read_only: true };
  }
