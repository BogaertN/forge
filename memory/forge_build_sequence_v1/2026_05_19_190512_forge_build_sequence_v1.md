...
- **current_phase**: `S19B — Runtime Module Dependency Map`
- **active_patch**: `Patch 156 — Runtime Module Dependency Map`
- **next_patch**: `Patch 157 — Runtime Build Plan / First Safe Module Candidate`
- **recommended_next_patch**: `Patch 157 — Runtime Build Plan / First Safe Module Candidate`
...
- **runtime_module_map_status**: `FORGE_RUNTIME_MODULE_DEPENDENCY_MAP_HAS_PLANNING_BLOCKERS`
- **runtime_module_map_engine_count**: `0`
- **runtime_module_map_domain_count**: `0`
- **runtime_module_map_planning_ready**: `False`
...
- **S17** — `DONE` — Snapshot and forensic replay expansion: Parent roadmap stage for snapshot planning, creation, shadow replay, and forensic diff.
- **S18** — `DONE` — Dashboard v2 / operator control panel: Future operator-facing dashboard expansion after snapshot foundations.
...
- **S17A** — `DONE` — Snapshot / Forensic Replay Plan: Patch 151 defines the snapshot and forensic replay plan. It creates no snapshot and restores nothing.
- **S17B** — `DONE` — Snapshot Create / Verify: Patch 152 should create Forge-owned snapshot artifacts and verify hashes.
- **S17C** — `DONE` — Shadow Restore / Forensic Diff: Future patch should rehearse restore into a shadow directory and produce forensic diff evidence.
...
- **S19B** — `ACTIVE` — Runtime Module Dependency Map: Map AI.Web runtime modules, dependencies, wrappers, tests, and deferred edges before any runtime build patch is proposed.
- **S19C** — `NEXT` — Runtime Build Sandbox Planner: Plan a Forge-owned runtime build sandbox before any AI.Web live runtime writes are eligible.
...
## Next Commands
- `forge-runtime-module-map-build`
- `forge-runtime-module-map-show latest`
- `forge-runtime-module-map-export`
- `forge-dashboard-roadmap-build`
- `forge-status-api-build`
- `forge-dashboard-build`
- `Install Patch 157 when ready`
