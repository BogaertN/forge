...
INDEXING_MODES = {
    ...
    "project": {
        "_description": (
            "Canonical project source only. Walks explicit root directories rather than the full tree. "
            "Skips nested duplicates, generated snapshots, static assets, and external models."
        ),
        # canonical_roots: only these subdirectories of the scan path are walked.
        # Anything at the top level not in this list is ignored entirely.
        # This is what eliminates /aiweb/aiweb/ duplicates and /models/ noise
        # without needing explicit rules for every possible junk directory.
        "canonical_roots": [
            "agents", "engines", "runtime", "memory", "config",
            "interface", "devtools", "docs", "system_docs", "scripts",
            "plugins", "symbolic_layers", "runtime_wrappers", "core",
            "src", "lib", "api", "server", "client",
        ],
        # Same allowed extensions as core
        "allowed_extensions": {
            ".py", ".md", ".txt", ".toml", ".yaml", ".yml",
            ".sh", ".bash", ".rst", ".cfg", ".ini",
            ".html", ".css", ".js", ".ts", ".jsx", ".tsx",
        ...
        "allowed_filenames_exact": {
            "dockerfile", "makefile", "requirements.txt", "readme",
        ...
        "excluded_dir_names": {
            # Venv / packages
            "site-packages", "dist-packages", "__pypackages__",
            ".venv", "venv", "env", "ENV", "aiweb310", "myenv",
            # Node
            "node_modules", ".npm",
            # Build
            "dist", "build", "coverage", "htmlcov",
        ...
        "excluded_dir_patterns": [
            "frozen", "_frozen", "_backup", "_bak", "_old",
            "freeze", "_freeze", "snapshot", "_snapshot",
        ],
    },
}
...
class ScanResult:
    ...
    def __init__(self, mode: str = "core"):
        ...
        self.will_skip:              list[tuple[str, str]] = []
        self.already_current:        list[str] = []
        self.missing_canonical_roots: list[str] = []   # project mode only

    def summary(self) -> str:
        """Short summary shown before approval prompt."""
        lines = []

        if self.missing_canonical_roots:
            lines.append(f"\n  Missing canonical roots (not found, skipped):")
            lines.append(f"    {', '.join(self.missing_canonical_roots)}")
    ...
    def detailed_preview(self) -> str:
        ...
        # ── 0. Missing canonical roots (project mode) ──
        if self.missing_canonical_roots:
            lines.append(f"\n  MISSING CANONICAL ROOTS (not found under scan path, skipped)")
            lines.append(f"    {', '.join(self.missing_canonical_roots)}")
...
def scan_for_indexing(path: str, mode: str = "core") -> ScanResult:
    ...
    mode: one of 'core', 'docs', 'logs', 'all', 'project'

    'project' mode walks only explicit canonical_roots under the scan path.
    All other modes walk the full tree with filtering.
    """
    ...
    # ── PROJECT MODE: walk only canonical roots ───────────────────────────────
    canonical_roots = policy.get("canonical_roots")
    if canonical_roots is not None and os.path.isdir(path):
        roots_found    = []
        roots_missing  = []
        ...
        for root_name in canonical_roots:
            ...
            if os.path.isdir(root_path):
                roots_found.append(root_path)
            else:
                roots_missing.append(root_name)

        if roots_missing:
            result.missing_canonical_roots = roots_missing

        if not roots_found:
            result.will_skip.append((path, "no canonical roots found under scan path"))
            return result

        for root_path in roots_found:
            _walk(root_path)

        return result

    # ── ALL OTHER MODES: walk full tree ───────────────────────────────────────
...
class IndexingProgress:
    ...
    def update(self, filepath: str, chunks: int, error: str = "", empty: bool = False):
        self.done += 1
        if empty:
            self.empty_skipped += 1
            print(f"    ○ [{self.done}/{self.total}] {os.path.basename(filepath)} — empty, skipped")
        elif error:
            self.errors += 1
    ...
    def summary(self) -> str:
        files_indexed = self.done - self.errors - self.empty_skipped
        lines = [
            f"  Files indexed   : {files_indexed}",
            f"  Chunks created  : {self.chunks_total}",
        ]
        if self.empty_skipped:
            lines.append(f"  Empty skipped   : {self.empty_skipped}")
        if self.errors:
            lines.append(f"  True errors     : {self.errors}")
        return "\n".join(lines)
...
def index_path(
    ...
    for filepath in files_to_index:
        ...
        if not chunks:
            progress.update(filepath, 0, empty=True)
            _mem.write_audit_entry(
                session_id=session_id,
                tool="KB_INDEX_SKIPPED_EMPTY",
                path=filepath,
                lines="-",
                reason="empty file, no chunks produced"
            )
            continue
    ...
    files_indexed = progress.done - progress.errors - progress.empty_skipped
    print(f"\n{progress.summary()}")
    return files_indexed, progress.chunks_total, progress.empty_skipped, progress.errors
