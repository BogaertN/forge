...
def _detect_type(filename: str) -> str:
    """
    Detect artifact type from the primary type marker in the filename.
    The marker is the first token after the timestamp prefix.
    e.g., '2026-05-09_133322_DIFF_2026_05_09...' → primary token = 'DIFF' → type = 'DIFF'
    This avoids false positives from DIFF filenames that embed '_PATCH_' later in the name.
    """
    # Strip timestamp prefix if present: YYYY-MM-DD_HHMMSS_
    name = re.sub(r"^\d{4}-\d{2}-\d{2}_\d{6}_", "", filename)
    # The first _-delimited token is the primary type marker
    first_token = name.split("_")[0].upper()
    return ARTIFACT_TYPES.get(first_token, "UNKNOWN")
...
def find_chain(artifact: dict, all_artifacts: list[dict]) -> list[dict]:
    """
    Find all related artifacts using transitive closure:
      Pass 1: same-second timestamps (same preflight run)
      Pass 2: direct proposal_ref / rollback_ref cross-references
      Pass 3: DIFF filenames embed proposal filename (with dashes→underscores)
    Iterate until no new artifacts are added.
    """
    chain_paths: set = {artifact["path"]}
    target = artifact.get("target_file", "")

    def _prop_normalized(filename: str) -> str:
        """Normalize a proposal filename for embedding comparison: replace - with _."""
        return filename.replace("-", "_")

    changed = True
    while changed:
        changed = False
        current = [a for a in all_artifacts if a["path"] in chain_paths]

        for a in current:
            a_ts   = a.get("timestamp", "")
            a_name = a["filename"]

            for c in all_artifacts:
                if c["path"] in chain_paths:
                    continue

                c_ts   = c.get("timestamp", "")
                c_name = c["filename"]

                # Rule 1: same target + same-second timestamp → same preflight run
                if target and c.get("target_file") == target:
                    if a_ts and c_ts and a_ts == c_ts:
                        chain_paths.add(c["path"])
                        changed = True
                        continue

                # Rule 2: this artifact's proposal_ref or rollback_ref names the candidate
                for ref_field in ("proposal_ref", "rollback_ref"):
                    ref = a.get(ref_field, "")
                    if ref and ref == c_name:
                        chain_paths.add(c["path"])
                        changed = True
                        break

                # Rule 3: candidate's proposal_ref or rollback_ref names this artifact
                for ref_field in ("proposal_ref", "rollback_ref"):
                    ref = c.get(ref_field, "")
                    if ref and ref == a_name:
                        chain_paths.add(c["path"])
                        changed = True
                        break

                # Rule 4: DIFF filename embeds the proposal filename
                # DIFF format: {diff_ts}_DIFF_{proposal_ts_with_underscores}_{rest}
                # The proposal filename normalized (dashes→underscores) appears in the diff name
                if a["type"] == "PROPOSAL" and c["type"] == "DIFF":
                    if _prop_normalized(a_name) in c_name:
                        chain_paths.add(c["path"])
                        changed = True
                        continue

                # Rule 5: reverse — artifact is DIFF, candidate is the embedded PROPOSAL
                if a["type"] == "DIFF" and c["type"] == "PROPOSAL":
                    if _prop_normalized(c_name) in a_name:
                        chain_paths.add(c["path"])
                        changed = True
                        continue
